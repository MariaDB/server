...
extern "C" UINT __stdcall CheckDataDirectory(MSIHANDLE hInstall)
{
  ...
  for (auto dir :
       {L"bin\\", L"include\\",
        L"lib\\", L"share\\"})
  {
    /* Extra space for appended dir, wcscat_s aborts on overflow */
    wchar_t localpath[MAX_PATH + 10];
...
extern "C" UINT CheckDBInUse(MSIHANDLE hInstall)
{
  ...
  wchar_t datadir[MAX_PATH];
  /* Extra space for appended \bin, wcscat_s aborts on overflow */
  ...
  /* INSTALLDIR ends with backslash */
...
extern "C" UINT __stdcall CheckServiceUpgrades(MSIHANDLE hInstall)
{
  ...
  if (er != ERROR_SUCCESS)
  {
    Log("MsiGetPropertyW failed, error %u", er);
    goto LExit;
  }
  ...
  if (er != ERROR_SUCCESS)
  {
    Log("MsiGetPropertyA failed, error %u", er);
    goto LExit;
  }
...
/* Fail on purpose, to test rollback. Scheduled with TESTFAIL=1 in msiexec */
extern "C" UINT __stdcall FakeFailure(MSIHANDLE hInstall)
{
