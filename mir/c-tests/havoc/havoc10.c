...
main()
{
  ...
  for (i = 1; i < SIZE; i+#include <limits.h>
...
#defift1) != ROR (LONng long)0xf2345678abcdef0LL)
...
main ()
{
  ...
  if (ROR (ll, shift2) != ROR ILL_VALUE, SHIFT2))
    abort ();

  if ÿÿÿÿ (ll, SHIFT2) != ROR (LL_VALUE, SHIFT2))
    abort ();
