...
public:
  ...
  */
  ...
  ULonglong_null side_effect_ref() override
  {
    return is_null() ? ULonglong_null() : ULonglong_null((ulonglong)val_int());
  }
  void side_effect_detach() override
  {
    if (!is_null())
    {
      THD *thd= get_thd();
      ulonglong offset= (ulonglong) Field_sys_refcursor::val_int();
  ...
  int store_item(THD *thd, Item *item, bool no_conversions)
  {
    ...
    const ULonglong_null old_value= Field_sys_refcursor::side_effect_ref();

    if (item->save_int_in_field(this, no_conversions))
    {
      DBUG_ASSERT(thd->is_error());
      Field_sys_refcursor::side_effect_detach();
      DBUG_RETURN(true);
    }

    const ULonglong_null new_value= Field_sys_refcursor::side_effect_ref();
    cursor_array(thd)->ref_count_update(thd, old_value, new_value);

    item->side_effect_detach(thd, expr_event_t::SPVAR_RIGHT_HAND_DESTRUCT);
...
public:
  ...
  void side_effect_attach(THD *thd, ulonglong offset) const override
  {
                                 ...
                                 const st_value *val) const override
  {
    ...
    Field_sys_refcursor::cursor_array(thd)->ref_count_update(thd, old_value,
                                                                  new_value);
    param->with_flags|= item_with_t::DATA_TYPE_WITH_SIDE_EFFECT;
    return false;
  }
  ...
  /*
    Don't override Item_char_typecast_fix_length_and_dec().
    Use inherited CAST(.. AS CHAR).
  */
