...
public:
  ...
  {
    return &thd->m_statement_cursors;
  }
};
...
public:
  ...
  */
  ...
  bool store_item(Item *item)
  {
    ...
    item->expr_event_handler(thd, expr_event_t::DESTRUCT_SPVAR_RIGHT_HAND);
    return rc;
  }
...
public:
  ...
  */
  ...
  /*
    This method destructs a SYS_REFCURSOR variable by detaching it
    from the referenced cursor. The referenced cursor is not destructed.
    See comments near the declaration of expr_event_handler() in
    sql/sql_type.h
  */
  void expr_event_handler(THD *thd, expr_event_t event,
                          ulonglong offset) const override
  {
    cursor_array(thd)->ref_count_dec(thd, offset);
  }
  ...
  void Item_update_null_value(Item *item) const override
  {
    ...
    item->expr_event_handler(current_thd,
                     ...
                     Item *a, Item *b) const override
  {
    /*
      ...
      - Also Item_sp_variable::const_item() returns false for
        data types with side effects.
    */
    DBUG_ASSERT(0);
    return false;
  }
  ...
  Type_ref_null Item_func_hybrid_field_type_val_ref(THD *thd,
                                             Item_func_hybrid_field_type *item)
                                                                 const override
  {
    const Type_ref_null ref= item->val_ref_from_ref_op(thd);
    item->expr_event_handler_args(thd,
                                  expr_event_t::DESTRUCT_BUILT_IN_ROUTINE_ARG);
    return ref;
  }
  ...
  public:
    ...
    String *val_str(Item_handled_func *item, String *to) const override
    {
      ...
      if ((item->null_value= ref.is_null()))
        return 0;
      if (arg->with_complex_data_types())
        arg->expr_event_handler(thd,
                                expr_event_t::DESTRUCT_BUILT_IN_ROUTINE_ARG);
