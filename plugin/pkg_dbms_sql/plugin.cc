...
class Item_func_dbms_sql_command: public Item_long_func
{
public:
  ...
  longlong val_int() override
  {
    ...
    if ((null_value= ps_name == nullptr))
      return 0;
    ULonglong_null cmd= mysql_sql_stmt_command(current_thd,
                          Lex_ident_sys(ps_name->ptr(), ps_name->length()));
    null_value= cmd.is_null();
    return (longlong) cmd.value();
  }
  LEX_CSTRING func_name_cstring() const override
  {
    static LEX_CSTRING name= "dbms_sql_command"_LEX_CSTRING;
    return name;
  }
  Item *shallow_copy(THD *thd) const override
  {
    return get_item_copy<Item_func_dbms_sql_command>(thd, this);
  }

  class Create_func : public Create_func_arg1
  {
  public:
    using Create_func_arg1::Create_func_arg1;
    Item *create_1_arg(THD *thd, Item *arg1) override
    {
      return new (thd->mem_root) Item_func_dbms_sql_command(thd, arg1);
    }
  };
...
maria_declare_plugin(pkg_dbms_sql)
{
  ...
  Item_func_dbms_sql_command::plugin_descriptor(),
  "dbms_sql_command",           // plugin name
  "MariaDB Corporation",        // plugin author
  "Function DBMS_SQL_COMMAND()",// the plugin description
