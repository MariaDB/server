This is a plugin that implements interaction with Hashicorp Vault KMS:
...
The plugin currently does not unseal Hashicorp Vault itself,
you must do this in advance and on your own.

To use Hashicorp Vault KMS, the plugin must be preloaded and
activated on the server. Its parameters should not change during
operation and therefore should be pre-specified as part of the
server configuration, for example, through command line options:
...
--[loose-]hashicorp-key-management-use-cache-on-timeout="on"|"off"
  ...
  values or version numbers taken from the cache. y default this
  option is disabled.
  ...
  --[loose-]hashicorp-key-management-retries parameter has been
  exhausted.
