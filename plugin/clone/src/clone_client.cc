...
bool Client_Stat::tune_has_improved(uint32_t num_threads)
{
  ...
  if (m_tune.m_cur_number != num_threads)
  {
    snprintf(info_mesg, MESG_SZ, "Tune stop, current: %u expected: %u",
             num_threads, m_tune.m_cur_number);
    return false;
  }
  ...
  auto last_index= (m_current_history_index - 1) % STAT_HISTORY_SIZE;
  double data_speed= static_cast<double>(m_data_speed_history[last_index]);
  double target_speed= static_cast<double>(m_tune.m_prev_speed);
  ...
  else
  {
    ...
    target_speed= static_cast<double>(m_tune.m_last_step_speed);
    target_speed *= 0.95;
  }
...
int Exec_State::begin_worker(Sub_Command &state)
{
  ...
  m_wait_state.wait(lock, cond_fn);
  state= m_cur_state;
...
int Client::clone()
{
  ...
  do
  {
    ...
    err= remote_command(rpc_com, SUBCOM_NONE, false);
    ...
    if (is_master())
    {
      ...
      auto err2= remote_command(COM_EXIT, SUBCOM_NONE, true);
      ...
      {
        err2= clone_kill(m_conn_aux.m_conn, m_conn);
      ...
      clone_disconnect(nullptr, m_conn_aux.m_conn, abort_net_aux, false);
      m_conn_aux.m_conn= nullptr;

      snprintf(info_mesg, MESG_SZ, "Source ACK Disconnect : abort: %s",
    ...
    if (!abort_net)
    {
      auto err2= remote_command(COM_EXIT, SUBCOM_NONE, false);
      /* if COM_EXIT is unsuccessful, abort the connection */
      if (err2 != 0)
        abort_net= true;
    ...
    clone_disconnect(get_thd(), m_conn, abort_net, clear_err);

    snprintf(info_mesg, MESG_SZ, "Task Disconnect : abort: %s",
  ...
  clone_get_error(get_thd(), &err_number, &err_mesg);
...
int Client::remote_command(Command_RPC com, Sub_Command sub, bool use_aux)
{
  size_t cmd_buff_len;

  /* Prepare command buffer */
  auto err= prepare_command_buffer(com, sub, cmd_buff_len);
  ...
  /* Send remote command */
  err= clone_send_command(get_thd(), conn, !use_aux, command,
                           m_cmd_buff.m_buffer, cmd_buff_len);
  if (err != 0)
    return err;
...
int Client::prepare_command_buffer(Command_RPC com, Sub_Command sub,
                                   size_t &buf_len)
{
  int err= 0;
  buf_len= 0;
  ...
  switch (com)
  {
    ...
    case COM_EXECUTE:
      err= serialize_exec_cmd(sub, buf_len);
...
int Client::serialize_exec_cmd(Sub_Command sub, size_t &buf_len)
{
  /* Add length for sub-command. */
  buf_len= 1;

  /* Allocate for command buffer. This is a sanity check as the buffer must
  have already been allocated in previous RPC. */
  auto err= m_cmd_buff.allocate(buf_len);
  auto buf_ptr= m_cmd_buff.m_buffer;
...
int Client::receive_response(Command_RPC com, bool use_aux)
{
  ...
  while (!last_packet)
  {
    ...
    err= clone_get_response(get_thd(), conn, !use_aux, timeout_sec, &packet,
...
int Client::set_descriptor(const uchar *buffer, size_t length)
{
  ...
  remote_command(COM_ACK, SUBCOM_NONE, true);
...
int Client_Cbk::buffer_cbk(uchar *from_buffer [[maybe_unused]], uint buf_len)
{
  ...
  auto err= client->remote_command(COM_ACK, SUBCOM_NONE, true);
                          ...
                          uchar *&to_buffer, uint &to_len)
{
  ...
  auto err= clone_get_response(client->get_thd(), conn, true, 0, &packet,
                               &length, &network_length);
  if (err != 0)
    return err;
