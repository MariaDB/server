...
public:
  /*
    ...
    record[0] images memcpy'd straight from scan_table (the worker and the
  ...
  */
  uchar           *rows;
  uint            count;   // rows the worker placed in batch_rows
  /*
...
class pwt_worker_execution
{
public:
  /*
    Per-worker copy of the manager's first non-const source table.
    Opened in init_parallel_workers, closed in the worker thread before
    its THD is destroyed.
  */
  TABLE                 *scan_table;
  JOIN                  *worker_join; // The JOIN for the worker's JOIN_TABs
  JOIN_TAB              *join_tabs;   // n_tables x non-const JOIN_TABs in order
