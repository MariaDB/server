...
/*
  ...
  over the non-const, parallel-scannable driving table 'scan_tab' that the
  workers can run themselves and ship result rows for. Called from
  make_join_readinfo(); see JOIN::worker_side_parallel.
*/
extern bool can_run_query_in_workers(JOIN *join, JOIN_TAB *scan_tab);

/*
  Part of the same gate: whether the workers can partition what this table's
  access method reads -- the whole index for a full scan, or the key intervals
  of a primary-key range scan. See sql_parallel_execution.cc.
*/
