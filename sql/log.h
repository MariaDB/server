...
public:
  ...
  */
  ...
  void prepare_for_later_open()
  {
    log_state= LOG_TO_BE_OPENED;
  }
  /*
...
enum enum_binlog_state
{
  ...
  */
  ...
  BINLOG_STATE_UNION_DISABLED=  (1<<10),  // Used for start_union_events()
  BINLOG_STATE_SLAVE_DISABLED=  (1<<11),  // !opt_log_slave_updates
  BINLOG_STATE_READONLY=        (1<<12)   // Nothing to log (no writes)
};
...
#define BINLOG_STATE_DISABLED_FLAGS (BINLOG_STATE_TMP_DISABLED | BINLOG_STATE_USER_DISABLED | BINLOG_STATE_WSREP_DISABLED | BINLOG_STATE_SUBSTMT_DISABLED | BINLOG_STATE_UNION_DISABLED | BINLOG_STATE_SLAVE_DISABLED)

/* Disabled stats honored by Galera */
#define BINLOG_STATE_DISABLED_WSREP (BINLOG_STATE_READONLY | BINLOG_STATE_WSREP_DISABLED)
/* All other disabled states for binary log with or without Galera */
#define BINLOG_STATE_DISABLED (BINLOG_STATE_BYPASS | BINLOG_STATE_DISABLED_FLAGS)
