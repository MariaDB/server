         ...
         uint32 flags) const
{
  return new (root) Field_vector(rec.ptr(), rec.null_ptr(), rec.null_bit(),
            attr->unireg_check, name, share, attr->length);
}

uint32 Type_handler_vector::max_display_length_for_field(
...
int Field_vector::store(const char *from, size_t length, CHARSET_INFO *cs)
{
  if (cs != &my_charset_bin) // XXX todo
    return report_wrong_value(ErrConvStringQ(from, length, cs));
