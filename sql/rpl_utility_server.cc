                                ...
                                TABLE **conv_table_var) const
{
  ...
  /*
    ...
    TODO Is cols_to_check still correct? We now can put in default values on
      the slave if the master doesn't have them
  */
  ulong master_table_n_cols= size();
  uint const cols_to_check= MY_MIN(table->s->fields, master_table_n_cols);
  Relay_log_info *rli= rgi->rli;
  TABLE *tmp_table= NULL;


  /*
    TODO Write MTR tests which use both type conversions & mis-indexed col
  */
  for (uint col= 0 ; col < cols_to_check ; ++col)
  {
    ...
    /*
      TODO Use table_list->master_unmatched_cols
    */
                                          ...
                                          RPL_TABLE_LIST *target_table_list_el) const
{
  ...
  */
  ...
  /*
    TODO Enhance for loop to consider field name (when using field names,
    always go through all master cols)
  */
...
bool RPL_TABLE_LIST::create_column_mappings(unsigned char *optional_metadata_raw,
                                            unsigned int optional_metadata_len)
{
  ...
  */
  ...
  DBUG_ASSERT(master_to_slave_structs_inited);
  DBUG_ASSERT(lookup_slave_column_func == &RPL_TABLE_LIST::lookup_by_col_mapping);

  bool indices_in_sync= true;
  uint slave_start_idx= 0;
  for (uint master_tbl_col_idx= 0 ; master_tbl_col_idx < master_table_n_cols ; master_tbl_col_idx++)
  {
      std::string master_col_name_cppstr= optional_metadata.m_column_name[master_tbl_col_idx];
    ...
    */
    ...
    for (uint slave_tbl_col_idx= slave_start_idx; slave_tbl_col_idx < table->s->fields; slave_tbl_col_idx++)
    {
