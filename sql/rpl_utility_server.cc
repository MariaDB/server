...
bool RPL_TABLE_LIST::give_compatibility_error(rpl_group_info *rgi, uint col)
{
  ...
  case SLAVE_FIELD_NAME_MISSING:
    if (slave_type_conversions_options &
        SLAVE_TYPE_CONVERSIONS_ALLOW_MISSING_FIELDS)
      return 0;
  ...
  case SLAVE_FIELD_NR_MISSING:
  {
    ...
    if (slave_type_conversions_options &
      ...
      return 0;
    /*'
      We only give an warning here as traditionally we have not given
      error when slave has less columns than master in default mode.
    */
    if (table->in_use->variables.log_warnings >= 1)
      rgi->rli->report(WARNING_LEVEL, ER_SLAVE_CORRUPT_EVENT, rgi->gtid_info(),
                       "Column %s missing from table '%s.%s",
                       llstr(col+1, number),
                       table->s->db.str, table->s->table_name.str);
    return 0;
