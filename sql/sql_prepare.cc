...
extern PSI_memory_key key_memory_Protocol_local;

MYSQL_DATA *Protocol_local::alloc_new_dataset()
{
  MYSQL_DATA *data;
  struct embedded_query_result *emb_data;
  if (!my_multi_malloc(key_memory_Protocol_local, MYF(MY_WME | MY_ZEROFILL),
                       &data, sizeof(*data),
...
bool Protocol_local::begin_dataset()
{
  ...
  init_alloc_root(key_memory_Protocol_local, alloc, 8192, 0, MYF(0));
...
extern "C" MYSQL *mysql_real_connect_local(MYSQL *mysql)
{
  ...
  mysql->info_buffer= (char *) my_malloc(key_memory_Protocol_local,
