...
sp_head::execute(THD *thd, bool merge_da_on_success)
{
  ...
  */
  ...
  if (!thd->sql_condition_handled)
  {
    ...
    // Copy the qualified name to avoid use-after-free
    strcpy(instr_and_lineno.qname, (m_qname.str && m_qname.length > 0) ?
        m_qname.str : "anonymous block");
    instr_and_lineno.qname_length= strlen(instr_and_lineno.qname);
    ...
    Diagnostics_area *da, const int line_no) const
{
  ...
  // Copy the qualified name to avoid use-after-free
  strcpy(instr_and_linno.qname, (sphead && sphead->m_qname.str &&
      sphead->m_qname.length > 0) ? sphead->m_qname.str : "anonymous block");
  instr_and_linno.qname_length= strlen(instr_and_linno.qname);

  thd->erroring_bt_list.push(instr_and_linno);

  struct Error_info msg_and_errno;
  msg_and_errno.err_no= da->get_sql_errno();
  msg_and_errno.msg[0]= '\0';
  if (da->is_set())
    ...
    CHARSET_INFO *cs_info) const
{
  ...
  if (check_errstack_str_length_and_append(thd, {STRING_WITH_LEN(".")},
      cs_info))
    return true;
  // Use copied qname instead of dereferencing potentially freed sphead
  if (check_errstack_str_length_and_append(thd,
      frames_list[loop_ctr].qname,
      frames_list[loop_ctr].qname_length,
      cs_info))
    return true;
...
{
  // Use copied qname instead of dereferencing potentially freed sphead
  if (frames_list[loop_ctr].qname_length > 0)
  {
    char err[10]= "";
    char line_no_str[20]= "";
    ...
    if (frames_list[loop_ctr].qname[0] != '\0' &&
        frames_list[loop_ctr].qname_length &&
        frames_list[loop_ctr].qname_length < UINT_MAX32 &&
        frames_list[loop_ctr].qname_length ==
        strlen(frames_list[loop_ctr].qname))
      thd->backtrace_std_str.append({frames_list[loop_ctr].qname,
          frames_list[loop_ctr].qname_length});
