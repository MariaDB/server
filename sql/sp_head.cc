...
{
  if (is_frame_for_members(pcontext))
...
bool sp_head::add_instr_copenX(THD *thd,
                               ...
                               const sp_rcontext_addr &addr)
{
  ...
  if (addr.rcontext_handler() != &sp_rcontext_handler_local)
    i= new (thd->mem_root) sp_instr_copen2(instructions(), spcont, addr,
                                           pcursor->lex());
  else
    i= new (thd->mem_root) sp_instr_copen(instructions(), spcont, addr);
  return i == NULL || add_instr(i);
}


bool sp_head::add_open_cursor(THD *thd,
                              const sp_pcursor *pcursor,
                              sp_pcontext *spcont,
                              const sp_rcontext_addr &addr,
                              ...
                              List<sp_assignment_lex> *parameters)
{
  ...
  */
  ...
  return add_instr_copenX(thd, pcursor, spcont, addr);
}
                                       ...
                                       Item_args *parameters)
{
  ...
  if (add_instr_copenX(thd, pcursor, cursor_pctx, cursor_addr))
    return true;
