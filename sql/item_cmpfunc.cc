                                 ...
                                 const DTCollation &collation)
{
  ...
  /* 
    The MySQL 8.0 behavior is that match_type fully overrides
    the default flags for the flags it can control.
  */
...
Item_func_regexp_instr::fix_length_and_dec(THD *thd)
{
  ...
  if (re.fix_owner(this, args[0], args[1]))
    return true;
  
  // Account for nullability of the new optional arguments
...
longlong Item_func_regexp_instr::val_int()
{
  ...
  while (byte_offset <= subject_len)
  {
    ...
    found_occurrence++;
    size_t match_start= re.subpattern_start(0);
    size_t match_end  = re.subpattern_end(0);

    if (found_occurrence == occurrence)
    {
      if (return_option == 0)
        return (longlong) lib_cs->numchars(subject_ptr,
                                           subject_ptr + match_start) + 1;
      else
        return (longlong) lib_cs->numchars(subject_ptr,
                                           subject_ptr + match_end) + 1;
    }

    /* 
      This ensures that after a zero-length match at position 2
      (found while searching from position 0), you advance to position 3
      instead of searching again from position 2.
    */
