...
extern "C" const uchar *dboptions_get_key(const uchar *opt, size_t *length,
                                          my_bool);

const uchar *dboptions_get_key(const uchar *opt_, size_t *length, my_bool)
{
  auto opt= static_cast<const my_dbopt_t *>(static_cast<const void *>(opt_));
  *length= opt->name_length;
  return static_cast<const uchar *>(static_cast<const void *>(opt->name));
}
