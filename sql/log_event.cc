   ...
   common_header_len(0), post_header_len(NULL), event_type_permutation(0)
{
  ...
  if (unlikely(
      (common_header_len=buf[ST_COMMON_HEADER_LEN_OFFSET]) < LOG_EVENT_MINIMAL_HEADER_LEN))
    DBUG_VOID_RETURN; /* sanity check */
...
bool Format_description_log_event::to_packet(String *packet)
{
  ...
  /*
    ...
    Master is supposed to trigger checksum computing by
    binlog_checksum_options, slave does it via marking the event according to
  ...
  */
  ...
  /*
     ...
     A combination of (A) != 0, (V) != 0 denotes FD of the checksum-aware
     server heading the checksummed binlog. (A), (V) presence in FD of the
...
bool Gtid_list_log_event::to_packet(String *packet)
{
  uint32 i;
  uchar *p;
  uint32 needed_length;
...
error:
  allocation_error= 1;
  /*
    The column names may be partially filled if parsing failed midway. Clear
    all of them so callers checking the first column's name fall back to
    positional column mapping instead of using a half-parsed name list.
  */
  for (size_t i= 0; i < m_column_metadata.size(); i++)
    m_column_metadata.at(i).column_name= LEX_CSTRING{nullptr, 0};
  return;
}
