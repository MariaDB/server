...
public:
  ...
  */
  ...
  /*
    Check that the constant `item` has a value which lies STRICTLY inside
    the domain of this field's data type:
    - the value is stored into the field without any data loss, and
    - it is not an extreme value of the domain.
    Field::store() maps out-of-domain values onto the extreme values of the
    domain: numbers are clamped, strings are truncated. So a comparison
    against the constant survives such a conversion only if no other value
    can be mapped onto the constant, that is, only if the constant lies
    strictly inside the domain.

    Note, the method stores into the field's record buffer, so it can only
    be called at the optimization stage, before any row has been read.
...
protected:
  /*
    Check that the value currently stored in the field is not an extreme
    value of the domain of the field's data type.
    Only called from is_const_strictly_inside_domain(), after the constant
    has been stored into the field without any data loss.
    The default implementation makes is_const_strictly_inside_domain()
    always return false, for the data types whose domain has not been
    examined yet.
  */
...
protected:
  ...
  */
  ...
  virtual uint32 value_length_in_capacity_unit(const String &value) const
  {
    return (uint32) value.numchars();
  }
...
class Field_string final :public Field_longstr {
  ...
  bool stored_value_is_strictly_inside_domain() override
  {
    /*
      ...
      field. Under a PAD SPACE collation they also compare equal, so the
      padding does not change comparison results. Under a NO PAD collation
      it does, no matter how short the value is.
    */
...
protected:
  ...
  */
  ...
  uint32 value_length_in_capacity_unit(const String &value) const override
  {
    return value.length();
  }
public:
