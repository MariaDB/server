...
public:
  ...
  */
  ...
  /*
    Is this column's data stored outside the record?

    The record then holds only a length and a pointer to the data.  A
    record copied as a whole copies that pointer, so both records share
    the data instead of owning a copy of it, and anything that keeps a
    row after the handler has moved on has to copy the data for itself
    first.

    Ask this, not `flags & BLOB_FLAG', when the question is where the
    data is kept.  BLOB_FLAG answers a different question -- whether the
    column was declared as a BLOB -- and `sql_select.cc' asserts that it
    agrees with type(), so it cannot be set on a column that reports
    VARCHAR.
  */
...
/*
  ...
  Two questions, and they are asked in this order because only the first
  says whether the second is meaningful: may the column's payload be
  kept outside the record at all, and is there enough declared width
  here to be worth reclaiming.  Field answers the first no by default,
  so a column that has not said otherwise is left where it is, and
  Field_vector answers it no as well, because its store() refuses
  anything but the declared width and a row is therefore that wide
  whatever is in it.
*/
