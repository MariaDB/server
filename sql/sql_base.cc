                  ...
                  bool allow_sum_func)
{
  ...
  */
  ...
  else if(!ref_pointer_array.is_null())
  {
    ...
    while ((item= li++))
    {
      if (ref_pointer_array[i])
        li.replace(ref_pointer_array[i]);
      i++;
    }
    uint n= thd->lex->current_select->fields_added_by_fix_inner_refs;
    JOIN *join= thd->lex->current_select->join;
    while (n)
    {
      join->all_fields.push_front(ref_pointer_array[i++], thd->mem_root);
      n--;
    }
  }
