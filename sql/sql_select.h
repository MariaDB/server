...
/*
  ...
  Then hp_make_key(record[0]) builds the HEAP key at index/write time.
  BLOB data is already in record[0]; non-BLOB parts are copied from
  the key buffer to record[0] as before.
