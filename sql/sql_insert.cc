...
/*
  Count number of non transactional tables used by the statement
  This code uses same logic as THD::binlog_write_table_maps()
*/

uint THD::count_non_transactional_tables()
{
  MYSQL_LOCK *locks[2], **locks_end= locks;
  uint found= 0;
  DBUG_ENTER("THD::count_non_transactional_tables");
...
void select_create::abort_result_set()
{
  ...
  */
  ...
  if (table)
  {
    ...
    */
    if (thd->count_non_transactional_tables() ==
        MY_TEST(!table->s->transactional))
