...
public:
  ...
  */
  bool protect_against_backup()
  {
    DBUG_ASSERT(!backup_commit_lock.load(std::memory_order_relaxed));
    if (unlikely(backup_commit_lock_enabled.load(std::memory_order_relaxed)))
    {
      /*
        We call direct protect_against_backup() here to avoid setting
        backup_commit_lock temporary to IN_BLOCK_COMMIT as this
        could cause starvation of enable_backup_commit_locks().
      */
      return protect_against_backup_slow();
    }
...
private:
  bool protect_against_backup_slow();
  void unprotect_against_backup_slow();
public:
  bool enable_backup_commit_locks();
  static void disable_backup_commit_locks();
