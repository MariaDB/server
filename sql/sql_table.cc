...
/**
  ...
  an ordered forward read of that leading part, so its first and last entries
  yield the range of historical row-end values. To qualify, an index must:
...
*/
KEY *TABLE::vers_end_best_idx(uint &best_idx)
{
...
/**
  ...
  reads its first and last entries to obtain the range. If no suitable
  index exists, falls back to a full table scan with a warning. Open-ended
  "current" rows (@c MY_TIMESPEC_MAX) are skipped when scanning.
  ...
  @retval false  Success.
  @retval true   Handler or conversion error.
*/
                                   ...
                                   my_timespec_t &max_ts)
{
  ...
  // Find best key
  uint best_idx;
  KEY *best_key= vers_end_best_idx(best_idx);
