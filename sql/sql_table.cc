  ...
  */
  for (FK_info &fk: table->s->foreign_keys)
  {
    ...
    if (drop)
    {
      ...
      drop_it.remove();
      /*
        Do not silently swallow duplicate DROP FOREIGN KEY of the same name.
        The first one was consumed above; any remaining same-name (non-IF
        EXISTS) entry stays in drop_list and is reported as
        ER_CANT_DROP_FIELD_OR_KEY below (MDEV-19194). IF EXISTS duplicates
        were already removed with a note in handle_if_exists_options().
      */
      continue;
    }
    ...
    for (const Create_field &def: nullable_foreign)
    {
      ...
      for (const Lex_ident_column &fcol: fk.foreign_fields)
      {
        ...
        if (fk.update_method == FK_OPTION_SET_NULL ||
            fk.delete_method == FK_OPTION_SET_NULL)
        {
          my_error(ER_FK_COLUMN_NOT_NULL, MYF(0), def.change.str, fk.name.str);
          goto err;
        }
  ...
  if (table_renamed || !nullable_referenced.is_empty())
  {
    for (FK_info &rk: table->s->referenced_keys)
    {
      ...
      {
        DBUG_ASSERT(thd->is_strict_mode());
        DBUG_ASSERT(rk.foreign_fields.elements == rk.referenced_fields.elements);
  ...
  for (FK_ddl_backup &ref: fk_rename_backup)
  {
    ...
    for (FK_info &fk: ref_share->foreign_keys)
    {
      if (cmp_table(fk.ref_db(), old_table->db) ||
        ...
        continue;
      if (0 != cmp_table(old_table->db, *new_db) &&
          fk.referenced_db.strdup(&ref_share->mem_root, *new_db))
        goto mem_error;
