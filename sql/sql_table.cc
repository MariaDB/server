...
static bool
fk_check_column_changes(THD *thd, List<Create_field> &new_create_list,
                        FK_info *fk, bool referenced,
                        Alter_table_ctx::FK_nullchg_list *null_changes,
                          ...
                          Alter_table_ctx *alter_ctx)
{
  ...
  */
  ...
  new_create_list.append(&new_create_tail); // new_create_list is full now!
  ...
  */
  for (FK_info &rk: table->s->referenced_keys)
  {
    ...
    if (fk_check_column_changes(thd, new_create_list, &rk, true,
                                &alter_ctx->rk_null_changes,
                                &fk_tables_to_lock))
      goto err;
  }
  ...
  */
  for (FK_info &fk: table->s->foreign_keys)
  {
    ...
    if (fk_check_column_changes(thd, new_create_list, &fk, false,
                                &alter_ctx->fk_null_changes,
                                &fk_tables_to_lock))
      goto err;
  ...
  */
  ...
  if (!fk_tables_to_lock.empty())
  {
    for (const FK_table_to_lock &t: fk_tables_to_lock)
    {
      ...
      enum_mdl_type req_type= (!alter_ctx->fk_null_changes.empty() ||
                               !alter_ctx->rk_null_changes.empty()) ?
                              MDL_SHARED_NO_WRITE : MDL_INTENTION_EXCLUSIVE;
      MDL_REQUEST_INIT(req, MDL_key::TABLE, t.table.db.str, t.table.name.str,
                       req_type, MDL_STATEMENT);
...
static Create_field *get_field_by_new_name(List<Create_field> &create_list,
                                           const LEX_CSTRING &old_name)
{
  ...
  while ((new_field= new_field_it++))
  {
    if (new_field->field_name.streq(old_name))
      break;
  }
  return new_field;
}
...
/**
  ...
  @retval FK_COLUMN_NULL_CHANGE  Check later when other share acquired if
                                 corresponding column in other table has
                                 acceptable nullable status.
...
fk_check_column_changes(THD *thd, List<Create_field> &new_create_list,
                        ...
                        mbd::set<FK_table_to_lock> *fk_tables_to_lock)
{
  List_iterator_fast<Lex_ident_column> column_it;
  List_iterator_fast<Lex_ident_column> ref_column_it;

  if (referenced)
  {
    column_it.init(fk->referenced_fields);
    ref_column_it.init(fk->foreign_fields);
  }
  else
  {
    ref_column_it.init(fk->referenced_fields);
    column_it.init(fk->foreign_fields);
  }
  Lex_ident_column *column;
  Lex_ident_column *ref_column;

  Alter_table_ctx::FK_null_changes::column_list columns_to_check;
  const bool prepare_copy_stage= !null_changes;
  DBUG_ASSERT(!prepare_copy_stage || !fk_tables_to_lock);

  while ((column= column_it++))
  {
    ref_column= ref_column_it++;
    /* column is already renamed by fk_prepare_rename() */
    Create_field *new_field= get_field_by_new_name(new_create_list, *column);

    if (new_field)
    {
#if 0
        // Assigned by mysql_prepare_alter_table():
        /* Check if field is changed */
        ...
        if (def && field->invisible < INVISIBLE_SYSTEM)
        {						// Field is changed
      ...
      if (prepare_copy_stage)
      {
        ...
        */
        ...
        /* Note: pack_length is only available after mysql_prepare_create_table_stage1() */
        ...
        if ((equal_result == IS_EQUAL_NO))
        {
          ...
          */
          if (referenced)
          {
            char buff[NAME_LEN * 2 + 6];
            strxnmov(buff, sizeof(buff)-1, "`", fk->foreign_db.str, "`", ".",
                    "`", fk->foreign_table.str, "`", NullS);
            my_error(ER_FK_COLUMN_CANNOT_CHANGE_CHILD, MYF(0), column->str,
                    fk->foreign_id.str, buff);
          }
          else
          {
            my_error(ER_FK_COLUMN_CANNOT_CHANGE, MYF(0), column->str,
                    fk->foreign_id.str);
          }
          return true;
        }
      ...
      else if (old_field_not_null != new_field_not_null)
      {
        if (referenced && !new_field_not_null)
        {
          ...
          */
          ...
          {
            columns_to_check.push_back(make_pair(column, ref_column));
          }
        }
	  ...
	  */
          ...
          {
            my_error(ER_FK_COLUMN_NOT_NULL, MYF(0), column->str,
                     fk->foreign_id.str);
            return true;
          }
          if (fk->update_method == FK_OPTION_CASCADE)
    ...
    else
    {
      /*
        ...
        TODO: Generally this is protected by lower level error codes like
        ER_WRONG_FK_DEF, ER_FK_NO_INDEX_PARENT.

        Should ER_FK_COLUMN_CANNOT_DROP_* be deprecated?
      */
      if (referenced)
      {
        char buff[NAME_LEN * 2 + 6];
        strxnmov(buff, sizeof(buff)-1, "`", fk->foreign_db.str, "`", ".",
                "`", fk->foreign_table.str, "`", NullS);
        my_error(ER_FK_COLUMN_CANNOT_DROP_CHILD, MYF(0), column->str,
                 fk->foreign_id.str, buff);
      }
      else
      {
        my_error(ER_FK_COLUMN_CANNOT_DROP, MYF(0), column->str,
                 fk->foreign_id.str);
      }
      return true;
    }
  ...
  if (!prepare_copy_stage && !columns_to_check.empty())
  {
    Table_name t(referenced ? fk->for_table(thd->mem_root) : fk->ref_table(thd->mem_root));
    null_changes->push_back({fk, t, columns_to_check});
    if (!fk->self_ref() &&
        !fk_tables_to_lock->insert(t))
      return true;
  }
                                        ...
                                        Alter_table_ctx *alter_ctx)
{
  ...
  if (!table->s->referenced_keys.is_empty())
  {
      ...
      while ((drop= drop_it++))
      {
        ...
        */
        if ((drop->type == Alter_drop::FOREIGN_KEY) &&
          ...
          fk_parent_key_it.remove(); // FIXME: is it needed? Test
      }
    }
    ...
    */
    ...
    while ((f_key= fk_parent_key_it++))
    {
      if (fk_check_column_changes(thd, alter_info->create_list,
                                  f_key, true, NULL, NULL))
        DBUG_RETURN(true);
    }
  }
  ...
  if (!table->s->foreign_keys.is_empty())
  {
    /*
      ...
      FIXME: is it needed? Test without it.
    */
    List_iterator<FOREIGN_KEY_INFO> fk_key_it(table->s->foreign_keys);
...
TABLE_SHARE *Alter_table_ctx::fk_get_share(const FK_null_changes &null_changes,
                                           bool &not_found)
{
  not_found= false;
  if (null_changes.fk->self_ref())
    return table->s;
  else
  {
    auto i= fk_shares.find(null_changes.table);
    if (i == fk_shares.end())
    {
      not_found= true;
      return NULL;
...
bool Alter_table_ctx::fk_handle_alter(THD *thd)
{
  ...
  DBUG_ASSERT(thd->mdl_context.is_lock_owner(MDL_key::TABLE, db.str,
                                             table_name.str, MDL_EXCLUSIVE));
  bool not_found;
  for (const FK_null_changes &null_changes: fk_null_changes)
  {
    TABLE_SHARE *ref_share= fk_get_share(null_changes, not_found);
    if (not_found)
    {
      DBUG_ASSERT(!thd->variables.check_foreign());
      continue;
    }
    DBUG_ASSERT(!null_changes.columns.empty());
    for (auto &col: null_changes.columns)
    {
      auto column= col.first;
      auto ref_column= col.second;
      for (Field **f= ref_share->field; *f; f++)
      {
        if ((*f)->field_name.streq(*ref_column) &&
            !((*f)->flags & NOT_NULL_FLAG))
        {
          my_error(ER_FK_COLUMN_NOT_NULL, MYF(0), column->str,
                   null_changes.fk->foreign_id.str);
          return true;
        }
      } // for (f)
    } // for (col)
  ...
  for (const FK_null_changes &null_changes: rk_null_changes)
  {
    TABLE_SHARE *for_share= fk_get_share(null_changes, not_found);
    if (not_found)
    {
      DBUG_ASSERT(0);
      continue;
    ...
    for (auto &col: null_changes.columns)
    {
      auto column= col.first;
      auto ref_column= col.second;
      for (Field **f= for_share->field; *f; f++)
      {
        if ((*f)->field_name.streq(*ref_column) &&
            ((*f)->flags & NOT_NULL_FLAG))
        {
          char buff[NAME_LEN*2+2];
          strxnmov(buff, sizeof(buff)-1, null_changes.fk->foreign_db.str, ".",
                   null_changes.fk->foreign_table.str, NullS);
          my_error(ER_FK_COLUMN_CANNOT_CHANGE_CHILD, MYF(0), column->str,
                   null_changes.fk->foreign_id.str, buff);
          return true;
        }
      } // for (f)
    } // for (col)
