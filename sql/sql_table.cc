  ...
  for (table= tables; table; table= table->next_local)
  {
    ...
    */
    ...
    else
    {
      ...
      case ER_NOT_FORM_FILE: /* backward compatibility, see main.partition_not_blackhole */
...
static Lex_ident make_internal_field_name(THD *thd, const char *prefix,
                                          List<Create_field> *create_list)
{
  char buf[INTERNAL_FIELD_NAME_LENGTH]= {0};
  Lex_ident name = {buf, 0};
  bool dup_found= true;
  for (uint num= 1; dup_found; num++)
  {
    name.length= my_snprintf(buf, sizeof(buf), "%s%u", prefix, num);
    for (auto &dup_field : *create_list)
      if ((dup_found= name.streq(dup_field.field_name)))
        break;
  }
  return thd->strmake_lex_cstring(name);
}
