...
static bool has_binlog_hton(Ha_trx_info *ha_info, THD* thd= NULL)
{
      ...
      unlikely(rw_count > 0))
  {
    /*
      ...
      transaction (thd is NULL) set out binlogging of an empty xa-prepare.
    */
  ...
  if (ha_info)
  {
         ...
         ha_info; ha_info= ha_info_next)
    {
      ...
      /*
        ...
        via `binlog_rollback`. This is reflected by `xa_already_done`.
        Oth the normal transaction engine's rollback is done in the loop.
      */
