    ...
    while ((got= hton->recover(hton, info->list, info->len)) > 0 )
    {
      ...
      */
      ...
      for (int i=0; i < got; i ++)
      {
        ...
        {
          /*
            If external XA is binlogged, ensure that after crash, the binlog
            and the engines have consistent state.

            If the XID is not in the binlog, we crashed between engine prepare
            and binlogging -> roll it back.

            If the binlog has a "prepared" event for the XID, leave it
            prepared in the engines.

            If the binlog has a "committed" event for the XID, we crashed
            between binlogging the commit and committing in the engines ->
            commit it in the engines.
          */
          if (info->ext_xa_list)
          {
            xid_with_status *x= (xid_with_status *)
              my_hash_search(info->ext_xa_list, info->list[i].key(),
                             info->list[i].key_length());
            if (!x)
            {
            ...
            /* Found as prepared in binlog, leave prepared in engines also. */
          }
          DBUG_EXECUTE("info",{
