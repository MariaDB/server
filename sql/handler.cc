...
int handler::update_auto_increment()
{
  ...
  */
  ...
  if (append)
  {
    ...
    if (thd->binlog_ready_later() && thd->is_current_stmt_binlog_format_stmt())
...
#ifdef WITH_WSREP
  /*
    wsrep patch sets WSREP_DISABLED to silence binlogging from high
    priority threads
  */
  if (thd->binlog_state & BINLOG_STATE_WSREP_DISABLED)
  {
    DBUG_ASSERT(wsrep_thd_is_applying(thd));
    return 0;
  }
#endif
  return (!(thd->binlog_state & BINLOG_STATE_TMP_DISABLED) &&
          table->s->can_do_row_logging &&
          ...
          */
          ((IF_WSREP((WSREP_EMULATE_BINLOG_NNULL(thd) &&
                     ...
                     (WSREP_NNULL(thd) &&
                      (thd->binlog_state & BINLOG_STATE_OPEN)),0)) ||
