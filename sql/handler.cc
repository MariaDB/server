...
const uchar *
my_xid_get_key(const void *p, size_t *out_len, my_bool)
{
  /*
    Rely on the layout of the struct to have 8 byte conn_id followed by
    4 byte commit_id.
  */
  *out_len= 12;
                               ...
                               XID *full_xid_arg)
{
  ...
  size_t key_len= 0;
  const uchar *key_ptr= my_xid_get_key(&xid_arg, &key_len, 1);
  if ((member= (xid_recovery_member *)
       my_hash_search(hash_arg, key_ptr, key_len)))
  ...
  if (!rc)
  {
    ...
    */
    ...
    if (global_system_variables.log_warnings > 2)
      sql_print_information("%s transaction with xid %llu:%u",
                            member->decided_to_commit ? "Committed" :
                            "Rolled back",
                            member->xid.conn_id, member->xid.commit_id);
  }
}
                                            ...
                                            void *param_arg)
{
  ...
  if (member->in_engine_prepare)
  {
    ...
    if (global_system_variables.log_warnings > 2)
      sql_print_warning("Found prepared transaction with xid %llu:%u",
                        member->xid.conn_id, member->xid.commit_id);
  }
  ...
  if (hton->recover)
  {
    while ((got= hton->recover(hton, info->list, info->len)) > 0 )
    {
      ...
      /* If wsrep_on=ON, XIDs are first ordered and then the range of
         ...
         the prepare.
      */
      my_xid wsrep_limit __attribute__((unused))= { 0, 0};
      ...
      */
      ...
      for (int i=0; i < got; i ++)
      {
        ...
        bool is_server_xid= x.commit_id > 0;

#ifdef WITH_WSREP
        if (!is_server_xid && wsrep_is_wsrep_xid(&info->list[i]))
          x= wsrep_xid_seqno(&info->list[i]);
#endif
        if (!x.commit_id) // not "mine" - that is generated by external TM
        {
          DBUG_EXECUTE("info",{
