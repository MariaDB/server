...
Item_field::fix_outer_field(THD *thd, Field **from_field, Item **reference)
{
  ...
  */
       ...
       outer_context= outer_context->get_outer_context())
  {
    ...
    bool at_top= (outer_context->get_select_lex() == select);
      ...
      */
      ...
      if (from_field != not_found_field)
      {
        ...
        if (thd->stmt_arena->is_conventional() ||
            thd->stmt_arena->is_stmt_prepare_or_first_stmt_execute())
        {
          // should be on statement memory?  Rex
          Query_arena *arena, backup;
          arena= thd->activate_stmt_arena_if_needed(&backup);
  ...
  */
  if (ret)
  {
    while (ret->merged_into &&
           (!ret->merged_into->master_unit()->derived ||
            !ret->merged_into->master_unit()->derived->is_derived()))
      ret= ret->merged_into;
  }
