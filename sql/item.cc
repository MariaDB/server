                                   ...
                                   bool merge)
{
  ...
  if (context)
  {
    ...
    */
    ...
    /*
      We have already fixed this Item_field, so hardwire the resolution table
      to prevent later problems -- but fall back to the context's table when
      the field is not backed by a base-table entry (e.g. a temporary table),
      so we never store a NULL first_name_resolution_table.
    */
    ctx->first_name_resolution_table=
      (field && field->table && field->table->pos_in_table_list) ?
      field->table->pos_in_table_list : context->first_name_resolution_table;
...
/**
  @description
  This item represents a reference to a column in having clause of some outer
  select. Wrap this item into an Item_ref object if we are at the
  first execution of the query or at processing the prepare command for it.
  Make this wrapping permanent for the first query execution so that it could
  be used for next executions of the query.
                                  ...
                                  Item **reference)
{
  ...
  if (thd->stmt_arena->state != Query_arena::STMT_EXECUTED)
  {
    ...
    if (is_first_execution && !thd->stmt_arena->is_conventional())
      arena= thd->activate_stmt_arena_if_needed(&backup);
    if ((rf= new (thd->mem_root) Item_ref(thd, context, found->table->s->db,
                                          Lex_cstring_strlen(
                                                  found->table->alias.c_ptr()),
                                          item->field_name)))
    {
      if (is_first_execution)
        *reference= rf;
  ...
  */
  ...
  else
  {
    ...
    */
    ...
    if (last_checked_context->select_lex->having_fix_field)
    {
      if (fix_field_reference_to_having(thd, context, *from_field, this,
                                        reference))
        return -1;
      return 0;
    }
...
bool Item_field::fix_fields(THD *thd, Item **reference)
{
  ...
  if (depended_from)         // this is outer reference and we have fixed before
  {
    ...
    if (depended_from->add_outer_reference_resolved_here(thd, this))
    {
        ...
        else
        {
          ...
          if (thd->stmt_arena->state == Query_arena::STMT_EXECUTED)
          {
            DBUG_ASSERT(reference && *reference &&
                        (*reference)->type() == Item::FIELD_ITEM);
