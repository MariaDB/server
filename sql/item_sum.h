...
private:
  ...
  */
  ...
  Item *filter_expr;
  /* filter_expr before the optimizer may rewrite it in place, for print() */
  Item *orig_filter;
...
public:
  ...
  Item *get_orig_filter() const { return orig_filter; }
  bool filter_passed();
                   ...
                   public Item_sp
{
 private:
  bool execute();
  /* Lets execute() tell "group had zero passing rows" apart from "routine
     already finished", so it returns NULL only for the former. */
  bool m_started;
