                                                 ...
                                                 Item_literal *cmp)
{
  ...
  */
  uint exceeding_group_size= (uint)floor(val*group_size/average);
...
double Expected_distribution::get_selectivity_ge(uint group_size,
                                                 Item_literal *cmp)
{
  double r= get_selectivity_gt(group_size, cmp) +
            get_selectivity_eq(group_size, cmp);
  return (r > 0 ? r : 0);
}
                                                 ...
                                                 Item_literal *cmp)
{
  double val= cmp->val_real();
  double average= (min_value+max_value)/2;
  uint this_group_size= (uint)ceil(val*group_size/average);
...
/*
  Return something representing the expected distribution of
  the sum of this field, it should probably look something
  like this
...
/*
  Return something representing the expected distribution of
  the minimum of this field, it should probably look something
  like this
                                    ...
                                    uint group_size)
{
  if (args[0]->type() == Item::FIELD_ITEM)
  {
    ...
    if (item->field->read_stats && check_read_stats(item->field->read_stats))
    {
      dist->min_value= item->field->read_stats->min_value->val_real();
      dist->max_value= item->field->read_stats->min_value->val_real();
      return TRUE;
    }
  }
  return FALSE;
...
/*
  Return something representing the expected distribution of
  the maximum of this field, it should probably look something
  like this
                                    ...
                                    uint group_size)
{
  if (args[0]->type() == Item::FIELD_ITEM)
  {
    ...
    if (item->field->read_stats && check_read_stats(item->field->read_stats))
    {
      dist->min_value= item->field->read_stats->max_value->val_real();
      dist->max_value= item->field->read_stats->max_value->val_real();
      return TRUE;
    }
  }
  return FALSE;
...
/*
  Return something representing the expected distribution of a count
  of this item.  TODO, not sure about this one.
*/
