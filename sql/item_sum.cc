...
int Aggregator_distinct::key_cmp(void *arg, const void *key1_,
                                 const void *key2_)
{
...
int json_arrayagg_key_cmp_with_order(void *arg, const void *key1,
                                     const void *key2)
{
  auto grp_item= static_cast<const Item_func_json_arrayagg *>(arg);

  return group_concat_order_list_comparison(grp_item->order,
                                            grp_item->arg_count_order,
                                            static_cast<const uchar *>(key1),
                                            static_cast<const uchar *>(key2), true);
}
