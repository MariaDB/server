...
bool setup_mvi_for_join(JOIN *join)
{
  ...
  if (collect_mvi_vcols_for_join(join, &ctx.vcol_fields))
    return true;
  if (join->conds)
    return join->conds->walk(&Item::mvi_ft_processor, &ctx, 0);
  return false;
}

bool Item_func_json_contains::mvi_ft_processor(void *arg)
{
  Mvi_context *ctx= (Mvi_context *) arg;
  List<Field> *vcol_fields= &ctx->vcol_fields;
  THD* thd= ctx->thd;
  ...
  if (arg_count > 2 || !a2_constant)
    return false;
  while ((vcol_field= it++) && !vcol_field->vcol_info->expr->eq(args[0], true))
    ;
  if (!vcol_field)
    return false;
  ...
  DBUG_ASSERT(((Item_func *) vcol_field->vcol_info->expr)->functype() ==
              MVI_ENCODE_FUNC);
  Item_func_mvi_encode *mvitem= (Item_func_mvi_encode *) vcol_field->vcol_info->expr;
  Lex_cast_type_st cast_type= mvitem->cast_type();
  CHARSET_INFO *cs= val->charset();
  const Type_handler *cast_th= cast_type.type_handler();
  StringBuffer<42> sorted;
  StringBuffer<256> buf;
  const uchar *start= reinterpret_cast<const uchar *>(val->ptr());
  const uchar *end= start + val->length();
  Item_func_match *ift;
  List<Item> ifm_args;
  Item_field *ivcol;
...
ok:
  ...
  ift= new (ctx->thd->mem_root) Item_func_match(thd, ifm_args, FT_BOOL);
  /* TODO: AND the ift */
  return false;
}
