      ...
      else
      {
        if (TEST_NEW_MODE_FLAG(join->thd, NEW_MODE_FIX_DUPLICATE_WEEDOUT) )
        {
          /*
            ...
            Estimate its "fanout", ie. how many duplicate rows would be
            produced on average for a record combination of outer tables'
            records.
          */
          uint n_tables= my_count_bits(sj_nest->sj_inner_tables & ~join->const_table_map);
                                         ...
                                         )
{
                              ...
                              dupsweedout_tables))
  {
    ...
    */
    ...
    // Fanout of all tables in the join prefix.
    ...
    for (uint j= first_dupsweedout_table; j <= idx; j++)
    {
      ...
      if (p->table->emb_sj_nest)
      {
        if (TEST_NEW_MODE_FLAG(join->thd, NEW_MODE_FIX_DUPLICATE_WEEDOUT) )
        {
          if (!(dups_removed_fanout & p->table->table->map))
          {
