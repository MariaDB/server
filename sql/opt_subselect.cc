...
/*
  ...
  SYNOPSIS
    ...
    @param   where
               IN: If not NULL and the predicate is eligible for removal, then
               this function removes it.
               OUT: The remaining WHERE condition, or NULL
    @return  true if the predicate was removed.  If where was passed as NULL,
             then the return value indicates whether the predicate would have
             been removed.
...
static bool remove_subq_pushed_predicates_impl(Item *item_in, Item **where)
{
  Item_func *item= dynamic_cast<Item_func*>(item_in);
  if (!item)
    return false;

  return item->type() == Item::FUNC_ITEM &&
         item->functype() == Item_func::EQ_FUNC &&
         item->arguments()[0]->type() == Item::REF_ITEM &&
         item->arguments()[1]->type() == Item::FIELD_ITEM &&
         test_if_ref (item,
                      (Item_field *)item->arguments()[1],
                      item->arguments()[0]);
}
...
static void remove_subq_pushed_predicates(JOIN *join, Item **where)
{
  DBUG_ASSERT(!where || !*where || join->conds == *where);
  if (remove_subq_pushed_predicates_impl(join->conds, where) && where)
    *where= nullptr;
}
