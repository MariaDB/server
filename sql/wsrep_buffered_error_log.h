...
class Buffered_error_logger
{
 ...
 public:
  void init(std::size_t buffer_size, std::size_t file_size);
  void resize_buffer(std::size_t buffer_size);
  void resize_file_size(std::size_t file_size);
  void rotate(uint n_rotations);
  ...
  void write_to_disk();
  void write_to_disk_safe(); // For signal handler
