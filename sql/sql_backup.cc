...
int append(handle src, backup_fd stream, uint64_t start, uint64_t end) noexcept
{
  ...
  assert(end >= start);
#if 0 /* On Linux, this turned out to be slower in our tests! */
  /*
    It is unsafe to zero-copy send from src unless we can guarantee
    that the receiving end of the pipe has consumed everything before
    our caller re-enables writes to this src region.
...
# endif
  ...
  {
    /*
      It should be safe to invoke the zero-copy path if we can
      guarantee that the recipient will drain the buffer before the
      final nonzero-copy path completes. We assume that this is
      guaranteed by writing at least pipe_size bytes via the
      nonzero-copy path after this shortcut.
    */
...
struct backup_target_phase
{
  ...
  int ret;
  /*
    Engine-specific backup context, shared by all threads.  Only
...
static my_bool backup_step(THD *thd, plugin_ref plugin, void *arg) noexcept
{
  ...
  if (hton->backup_step)
  {
    const auto it= t.ha_data.find(hton);
...
static void backup_thread_init() noexcept
{
  ...
  psi= PSI_CALL_new_thread(key_thread_backup_worker, NULL, 0);
                           ...
                           int threads)
{
  ...
  if (threads > 1)
  {
    if (!(tp= tpool::create_thread_pool_generic()))
      goto oor;
  ...
  if (command)
  {
    ...
    for (int t{threads}; t; )
    {
      ...
      */
      ...
      int sink= fileno(f);
# ifdef __linux__
      /* Request a larger pipe buffer; the default appears to be 64 KiB. */
