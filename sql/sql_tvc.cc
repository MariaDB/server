				 ...
				 uint first_list_el_count)
{
  ...
  /*
    ...
    every call.  Nullability is OR-accumulated by aggregate_attributes()
    below, so it has to start over as well, else a column that one
    execution saw a NULL value for stays nullable for the rest of the
    statement's lifetime.
  */
  for (uint pos= 0; pos < first_list_el_count; pos++)
  {
    holders[pos].remove_arguments();
    holders[pos].set_type_maybe_null(false);
  }
				 ...
				 st_select_lex_unit *unit_arg)
{
  ...
  */
  if (!holders)
  {
    ...
    /*
      ...
      every holder's args[] buffer has been allocated.
    */
  ...
  */
  if (join_type_handlers_for_tvc(thd, li, holders, cnt) ||
      get_type_attributes_for_tvc(thd, li, holders, cnt))
     DBUG_RETURN(true);
  ...
  /*
    ...
    The list says whether they are there, not the holders array.  The
    type collection above runs after populating the holders and before
    populating the list, so if it reports an error, it will leave the
    holders array allocated but the list still empty.
  */
  if (sl->item_list.is_empty())
  {
