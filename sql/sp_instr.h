...
class sp_instr_stmt : public sp_lex_instr
{
  ...
  */
  ...
  /**
    In case an instance of this class is instantiated on handling assignment
    operator, this data memeber contains a string value of right hand side
    part of the SET statement, that is the string "value" for the following
    statement
      SET var= value
  */
  LEX_CSTRING m_rhs_value;
...
public:
  sp_instr_stmt(uint ip, sp_pcontext *ctx, LEX *lex, const LEX_STRING& query,
    ...
    : sp_lex_instr(ip, ctx, lex, true),
      m_valid(true), m_query(query), m_rhs_value(rhs_value)
  {}
...
public:
  ...
  sp_instr_cpush(uint ip, sp_pcontext *ctx, LEX *lex, uint offset,
                 const LEX_CSTRING &cursor_query)
    : sp_lex_instr(ip, ctx, lex, true), m_cursor(offset),
      m_metadata_changed(false), m_cursor_query(cursor_query)
  {}
