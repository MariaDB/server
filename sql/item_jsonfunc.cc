...
bool Item_func_json_value::fix_length_and_dec(THD *thd)
{
  ...
  mem_root_dynamic_array_init(thd->active_stmt_arena_to_use()->mem_root, PSI_NOT_INSTRUMENTED,
                 ...
                 sizeof(int), NULL,
                              32, 32, MYF(0));
  Json_path_extractor::init_json_engine_stack(thd->active_stmt_arena_to_use()->mem_root);
...
String *Item_func_json_value::val_str(String *to)
{
  ...
  null_value= Json_path_extractor::extract(_current_thd()->active_stmt_arena_to_use()->mem_root, to, args[0],
                                       args[1],
                                       collation.collation,
                           &json_depth_array,
                            &func_name_str);
...
bool Item_json_str_multipath::fix_fields(THD *thd, Item **ref)
{
  if (!tmp_paths)
  {
    ...
    */
    ...
    if (alloc_tmp_paths(n_paths, &paths, &tmp_paths, thd->active_stmt_arena_to_use()->mem_root))
      return true;
  }
...
bool Item_func_json_contains_path::fix_fields(THD *thd, Item **ref)
{
  ...
  */
  if (!tmp_paths)
  {
    if (alloc_tmp_paths(arg_count-2, &paths, &tmp_paths, thd->active_stmt_arena_to_use()->mem_root) ||
...
String* Item_func_json_key_value::val_str(String *str)
{
  ...
  null_value= Json_path_extractor::extract(_current_thd()->active_stmt_arena_to_use()->mem_root, &tmp_str,
                                           args[0], args[1],
                                        collation.collation,
                            &json_depth_array,
                             &func_name_str);
...
void Item_func_json_array_intersect::prepare_json_and_create_hash(json_engine_t *je1, String *js)
{
  ...
  */
  ...
  if (json_read_value(je1) || je1->value_type != JSON_VALUE_ARRAY ||
      create_hash(je1, &items, hash_inited, &hash_root, _current_thd()->active_stmt_arena_to_use()->mem_root))
    {
      if (je1->s.error)
