...
{
  kill_callback_arg(THD *thd_arg, uint32 id): thd(thd_arg), slave_server_id(id) {}
  THD *thd;
...
bool kill_zombie_dump_threads(THD *thd, uint32 slave_server_id)
{
  ...
  List_iterator_fast<THD> it(arg.threads_to_kill);
  ...
  while (THD *ptr= it++)
  {
    ...
    */
    ack_receiver.remove_slave(ptr);

    ptr->awake_no_mutex(KILL_SLAVE_SAME_ID);
