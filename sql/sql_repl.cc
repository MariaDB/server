...
load_ignore_domain_ids(const char *str, size_t len, DYNAMIC_ARRAY *ids)
{
  ...
  while (p < end)
  {
    ...
    {
      char *endptr;
      uint32 domain_id= (uint32) strtoul(p, &endptr, 10);
      if (endptr == p)
                       ...
                       const DYNAMIC_ARRAY *do_ids)
{
  ...
  *out_errormsg= nullptr;
  /*
    ...
    However, when the slave has configured IGNORE_DOMAIN_IDS or DO_DOMAIN_IDS,
    we must skip the GTID index and use the Gtid_list_log_event fallback path.
    The GTID index search (is_before_pos()) does not understand domain
    filtering: it considers domains present in the binlog state but absent
    from the slave's position as "the slave needs events from an earlier file",
    causing a false mismatch. The contains_all_slave_gtid() path handles
    ignored domains correctly.

    Making the GTID index domain-aware would require threading ignore/do arrays
    through search_gtid_pos() -> search_cmp_gtid_pos() -> is_before_pos(),
  ...
  */
  ...
  if (likely(reader && !reader->open_index_file(buf)) &&
      ignore_ids->elements == 0 && do_ids->elements == 0)
  {
  ...
  if (info->using_gtid_state)
  {
    ...
    */
    if (get_slave_ignore_domain_ids(thd, &slave_ignore_domain_ids_str))
    {
      ...
      sort_dynamic(&info->slave_ignore_domain_ids, uint32_cmp);
    }
    if (get_slave_do_domain_ids(thd, &slave_do_domain_ids_str))
    {
      ...
      sort_dynamic(&info->slave_do_domain_ids, uint32_cmp);
    }
  }
...
static int send_engine_events(binlog_send_info *info, LOG_INFO* linfo)
{
  ...
  while (!should_stop(info))
  {
    ...
    Log_event_type event_type=
        (Log_event_type)((uchar)(*packet)[EVENT_TYPE_OFFSET+ev_offset]);
