...
public:
  ...
  */
  ...
  LEX_CSTRING lex_cstring(THD *thd) const;

  LEX_CSTRING lex_cstring() const { return m_schemas_str; }
private:
  /*
...
class Sql_path_stack
{
  ...
  THD *m_thd;
public:
  Sql_path_stack(THD *thd) : m_thd(thd) {}
...
{
public:
  virtual ~Sroutine_key_ref() = default;
  /*
    Called when a Sroutine_hash_entry's key is resolved.
  */
  virtual void on_resolve_key(const Sp_handler *handler,
                              const MDL_key *key) = 0;
};
