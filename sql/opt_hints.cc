...
struct st_opt_hint_info opt_hint_info[]=
{
  ...
  {{STRING_WITH_LEN("JOIN_PREFIX")}, false, true/*, true*/}, // OLEGS: MySQL has irregular_hint flag here
                                         ...
                                         String *str)
{
  ...
  case JOIN_PREFIX_HINT_ENUM:
    join_prefix->append_args(thd, str);
    break;
  case JOIN_SUFFIX_HINT_ENUM:
    join_suffix->append_args(thd, str);
    break;
  case JOIN_ORDER_HINT_ENUM:
    // OLEGS: that for JOIN_ORDER()s
    // join_order_hints.begin()->append_args(thd, str);
    break;
