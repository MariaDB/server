                                         ...
                                         DDL_LOG_STATE *ddl_log_state_tmp_file)
{
  ...
  */
  ...
  if (iterate_trigger_fields_and_run_func(
    ...
    if (iterate_trigger_row_and_run_func(
        lex->sphead->m_trg_table_row,
        [thd, table] (Item_trigger_row* trg_row)
        {
          /*
            We can fix fields at the time of execution, which will also
            populate fields.
          */
          return trg_row->setup_field(thd, table, nullptr);
        }
     ))
    DBUG_RETURN(true);
         ...
         trigger= trigger->next[event])
    {
      ...
      */
      ...
      (void)iterate_trigger_row_and_run_func(
        ...
        [this, fields_already_marked] (Item_trigger_row* trg_row) mutable
        {
          if (trg_row->check_if_settable())
          {
            if (!fields_already_marked)
            {
              ...
              for (; !field_iterator.end_of_fields(); field_iterator.next(),i++)
              {
