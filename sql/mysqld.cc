...
void handle_accepted_socket(MYSQL_SOCKET new_sock, MYSQL_SOCKET sock)
{
  ...
  {
    is_unix_sock= sock.is_unix_domain_socket;
...
void handle_connections_sockets()
{
  ...
  for (size_t i = 0; i < listen_sockets.size(); i++)
  {
    ...
    fds[i].events= POLLIN;
    listen_sockets[i].fd_flags= fcntl(fds[i].fd, F_GETFL, 0);
  }
#else
  FD_ZERO(&clientFDs);
  for (size_t i= 0; i < listen_sockets.size(); i++)
  {
    ...
    FD_SET(fd, &clientFDs);
    listen_sockets[i].fd_flags= fcntl(fd, F_GETFL, 0);
  }
#endif
  ...
  while (!abort_loop)
  {
    ...
    for (size_t i=0; i < listen_sockets.size(); i++)
    {
      ...
      {
        sock= listen_sockets[i];
        flags= listen_sockets[i].fd_flags;
        break;
      }
    }
#endif // HAVE_POLL
