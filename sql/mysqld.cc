...
/*
  ...
  tmp_file_prefix_length in table.h.
    ...
    Deliberately not 'const': on Windows, plugins import this symbol
    from server.dll, whose export list is generated by CMake's
    bindexplib from the object files. bindexplib only exports data
    symbols that live in a writable section, so a 'const' array would
    land in .rdata and never be exported (LNK2001 when linking
    ha_rocksdb.dll). This is a CMake limitation, not a Windows one:
    the linker itself exports read-only data without problems.
*/
char tmp_file_prefix[]= "#sql";
static_assert(sizeof(tmp_file_prefix) == tmp_file_prefix_length + 1, "");
