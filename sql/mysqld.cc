  ...
  if (signal_handler_using_server_resources)
  {
    ...
    if (signal_handler_using_server_resources)
    {
      ...
      set_timespec(abstime,
                   DBUG_EVALUATE("force_sighup_processing_timeout", 0.5, 10));
      if (mysql_cond_timedwait(&COND_signal_handler_thd_sighup,
                               &LOCK_signal_handler_thd_sighup, &abstime))
      {
        sql_print_warning("Signal handler thread did not finish processing in "
                          "timely manner, continuing to kill connection "
                          "threads with active signal handler.");
      }
    }
