...
{
  IO_AND_CPU_COST row_cost, index_cost;
...
public:
  ...
  */
  ...
  {
    DBUG_ASSERT(avg_io_cost || index_cost.io + row_cost.io == 0);
...
public:
  ...
  */
  ...
  ulonglong index_blocks(uint index, uint ranges, ha_rows rows);
...
protected:
  virtual IO_AND_CPU_COST scan_time()
  {
    ...
    cost.cpu= (!stats.block_size ? 0.0 :
               (double) (length / stats.block_size) * INDEX_BLOCK_COPY_COST);
    return cost;
  }
public:
