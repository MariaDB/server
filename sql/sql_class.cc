...
bool THD::init_collecting_unit_results()
{
  if (!unit_results)
  {
    ...
    if (!(my_multi_malloc(PSI_INSTRUMENT_ME, MYF(MY_WME), &unit_results,
                          sizeof(DYNAMIC_ARRAY),
                          &buff,
                          sizeof(unit_results_desc) * UNIT_RESULTS_ALLOC_INIT,
                          NullS)) ||
        my_init_dynamic_array2(PSI_INSTRUMENT_ME, unit_results,
                               sizeof(unit_results_desc),
                               buff, UNIT_RESULTS_ALLOC_INIT,
                               UNIT_RESULTS_ALLOC_INCREMENT, MYF(MY_WME)))
    {
      if (unit_results)
