...
void THD::disconnect()
{
  ...
  /*
    Consume active_vio so later KILLs cannot reuse this borrowed pointer.

    If active_vio belongs to another connection, e.g. replication's
    MYSQL connection, we do not own it and must not close it here.
    Just wake it up in case it is stuck in network I/O; the owner
    will close it later.

    For THD's own connection, active_vio can point to the transport
    under net.vio, while net.vio itself can be an SSL/filter layer,
    so compare transports rather than VIO pointers.
  */
  if (active_vio &&
      (!net.vio || vio_get_transport(active_vio) != vio_get_transport(net.vio)))
    vio_shutdown(active_vio, SHUT_RDWR);
  active_vio= 0;
