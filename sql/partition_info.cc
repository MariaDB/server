                                      ...
                                      MY_BITMAP *used_partitions)
{
  ...
  */
  if (table->triggers &&
      ...
      table->triggers->is_fields_updated_in_trigger(&full_part_field_set,
                         TRG_EVENT_INSERT, TRG_ACTION_BEFORE))
    DBUG_RETURN(false);
  ...
  */
  if (table->vfield)
  {
    for (Field **fld = table->vfield; *fld; fld++)
    {
  ...
  if (duplic == DUP_UPDATE)
  {
    ...
    */
    if (table->triggers &&
        ...
        table->triggers->is_fields_updated_in_trigger(&full_part_field_set,
                           TRG_EVENT_UPDATE, TRG_ACTION_BEFORE))
      DBUG_RETURN(false);
  }
  ...
  */
  if (fields.elements && !is_field_in_part_expr(fields))
                                        ...
                                        MY_BITMAP *used_partitions)
{
  ...
  {
    if (fill_record(thd, table, fields, values, false, false, &full_part_field_set))
      goto err;
  }
  else
  {
    if (fill_record(thd, table, table->field, values, false, false, &full_part_field_set))
      goto err;
  }
...
bool partition_info::is_full_part_expr_in_fields(List<Item> &fields)
{
  ...
  /*
    ...
    to compare with.
    TODO: should it be implemented with full_part_field_set?
  */
  do
  {
    List_iterator<Item> it(fields);
