...
multi_upddel_handler *find_multi_upddel_handler(THD *thd,
                                                SELECT_LEX *select_lex)
{
  // Pushdown is not supported for non-top-level statements
  if (select_lex->master_unit()->outer_select())
    return 0;

  return scan_tables_for_pushdown(select_lex->join->tables_list,
    [&](handlerton *ht) -> multi_upddel_handler *
  {
    return ht->create_multi_upddel ?
           ht->create_multi_upddel(thd, select_lex) : nullptr;
  });
}
...
int multi_upddel_handler::execute()
{
  ...
  if (result->direct_update_delete_done(found_rows, affected_rows))
  {
    my_error(ER_INTERNAL_ERROR, MYF(0),
             "a pushed down UPDATE/DELETE cannot be reported by this result");
    DBUG_RETURN(-1);
  }
