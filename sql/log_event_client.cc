                                               ...
                                               uchar *rows_buff, Log_event_type ev_type)
{
  ...
  for (uchar *value= m_rows_buf; value < m_rows_end; )
  {
    ...
    size_t length1= 0;
    if (!(length1= calc_row_event_length(td, bi_fields, &m_cols, value)))
    {
      fprintf(stderr, "\nError row length: %zu\n", length1);
      exit(1);
    }
    ...
    {
      if (!(length2= calc_row_event_length(td, ai_fields, &m_cols_ai, value)))
      {
        fprintf(stderr, "\nError row length: %zu\n", length2);
        exit(1);
      }
...
void Rows_log_event::count_row_events(PRINT_EVENT_INFO *print_event_info)
{
  ...
  for (const uchar *value= m_rows_buf; value < m_rows_end; )
  {
    ...
    if (!(length= calc_row_event_length(td, NULL, &m_cols, value)))
      break;
    value+= length;
    ...
    if (row_events == 2)
    {
      if (!(length= calc_row_event_length(td, NULL, &m_cols_ai, value)))
        break;
      value+= length;
      DBUG_ASSERT(value <= m_rows_end);
    }
