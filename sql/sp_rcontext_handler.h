...
public:
  ...
  */
  ...
  /**
    At parse time thd->lex->spcont points to the parse context (sp_pcontext)
    of the currently parsed routine.
    PACKAGE BODY variables and cursors store their data in a separate
    sp_pcontext which belongs to sp_package.
    This methods provides access to the proper sp_pcontext structure,
    depending on the variable/cursor location.
  */
  virtual sp_pcontext *get_pcontext(THD *thd) const= 0;
...
public:
  ...
  sp_pcontext *get_pcontext(THD *thd) const override;
