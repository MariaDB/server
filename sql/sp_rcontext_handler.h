...
public:
  /*
    Evaluate the offset of the referenced variable or cursor
    using Field::val_int().

    @retval  true  If Field::is_null() returned true,
                   or if Field::val_int() greater than max_deref_offset.
    @retval false  otherwise.
  */
  static bool dereference(Field *ref, uint *deref_pos, uint max_deref_offset);
...
class Sp_rcontext_handler_session final :public Sp_rcontext_handler
{
public:
  const LEX_CSTRING *get_name_prefix() const override;
  sp_rcontext *get_rcontext(sp_rcontext *ctx) const override
