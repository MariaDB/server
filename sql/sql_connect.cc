...
static int check_connection(THD *thd)
{
  ...
  if (auth_rc == 0)
  {
    ...
    */
    ...
    if (net->using_proxy_protocol & NET_PROXY_PROTOCOL_CONNECT_ERRORS)
...
#endif
  if (vio_type != VIO_TYPE_INVALID)
    mysql_socket_close(sock);
  vio_type= VIO_TYPE_INVALID;
...
THD *CONNECT::create_thd(THD *thd)
{
  ...
  vio_type= VIO_TYPE_INVALID;          // Vio now handled by thd
