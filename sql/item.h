...
#define ITEM_FLAGS_MAYBE_NULL_SHIFT 0 // Must match MAYBE_NULL
  ...
  IS_IN_PS_SAFE_CONTEXT= (1<<7)    // Appears bare in PS safe context,
...
public:
  ...
  */
  ...
  /*
    Mark that this item is evaluated in a context where a stored function
    is allowed to execute prepared statements, e.g. an assignment right
    hand side. The flag is only ever consulted by Item_func_sp, see
    Item_sp::execute_impl(). A container item overrides this to opt its
    children in, see Item_row::set_in_ps_safe_context().
  */
  virtual void set_in_ps_safe_context()
  {
    base_flags|= item_base_t::IS_IN_PS_SAFE_CONTEXT;
  }
...
protected:
  ...
  void set_in_ps_safe_context_args()
  {
    for (uint i= 0; i < arg_count; i++)
      args[i]->set_in_ps_safe_context();
  }
public:
  Item_args(void)
...
public:
  ...
  */
  ...
  virtual bool is_in_ps_safe_context() const
  {
    return false;
  }
