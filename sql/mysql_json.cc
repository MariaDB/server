...
*/
constexpr uchar SMALL_OFFSET_SIZE= 2;
constexpr uchar LARGE_OFFSET_SIZE= 4;
...
*/
constexpr uchar KEY_ENTRY_SIZE_SMALL= (2 + SMALL_OFFSET_SIZE);
constexpr uchar KEY_ENTRY_SIZE_LARGE= (2 + LARGE_OFFSET_SIZE);
...
*/
constexpr uchar VALUE_ENTRY_SIZE_SMALL= (1 + SMALL_OFFSET_SIZE);
constexpr uchar VALUE_ENTRY_SIZE_LARGE= (1 + LARGE_OFFSET_SIZE);

/* The maximum number of nesting levels allowed in a JSON document. */
constexpr uchar JSON_DOCUMENT_MAX_DEPTH= 100;
                               ...
                               const char *data, size_t len)
{
  ...
  case JSONB_TYPE_OPAQUE:
  {
    ...
    default:
    {
      ...
      const int needed= my_base64_needed_encoded_length(blob_length);
