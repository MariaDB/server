...
void Item_func_last_value::evaluate_sideeffects()
{
  ...
  {
    args[i]->val_int();
    args[i]->side_effect_detach(thd, expr_event_t::BUILT_IN_FUNC_ARG_EVALUATED);
  }
}
...
longlong Item_func_last_value::val_int()
{
  ...
  null_value= last_value->null_value;
  // Attach "this" to the side effect of "last_value".
  m_side_effect_ref= last_value->side_effect_ref();
  if (!m_side_effect_ref.is_null())
    last_value->type_handler()->
      side_effect_join(thd, m_side_effect_ref.value());
  // Detouch the side effect from "last_value"
