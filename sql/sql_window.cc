  ...
  Returns true if:
    ...
    2. Every function is streamable:
       ...
       - it is not a DISTINCT aggregate (the streaming path uses the SIMPLE
         aggregator and cannot deduplicate).

    3. The frame is prohibited or streamable: for aggregates it must be either
       ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW or an order list that
       is provably unique and not NULL.
                                 ...
                                 table_map const_table_map)
{
  ...
  if (streaming_wf_order_is_longer)
  {
    for (ORDER *o= longest_wf_order; o; o= o->next)
    {
      ...
      // used & ~first_table_map includes outer table references and
      // non-deterministic functions (OUTER_REF_TABLE_BIT and RAND_TABLE_BIT).
      if ((used & ~first_table_map) || (*o->item)->is_expensive() ||
...
/*
  Sets Window_spec::order_is_unique to true for window orders that contain a
  unique index.
  ...
  Caller must verify the query has a single non-const table.
*/
void check_window_order_uniqueness(List<Item_window_func> &win_funcs,
                                   TABLE *table)
{
  DBUG_ASSERT(table);
  ...
  while ((wf= it++))
  {
        ...
        !spec->window_frame)
    {
      ...
      spec->order_is_unique=
          order_contains_unique_index(table, spec->partition_list->first);
      spec->disjoin_partition_and_order_lists();
    }
  }
}
...
bool Window_funcs_sort_streaming::setup(List<Item_window_func> &window_funcs)
{
  ...
  while ((win_func= it++))
  {
    ...
    // sets the internal peer tracker
    Item_sum *sum_func= win_func->window_func();
    sum_func->setup_window_func(thd, win_func->window_spec);

    // for handling aggregate functions
    win_func->window_func()->set_aggregator(thd,
                                            Aggregator::SIMPLE_AGGREGATOR);
  }
