...
#include "table_cache.h"                        // tdc_share_is_cached()
...
static bool ddl_log_increment_phase_no_lock(uint entry_pos)
{
  ...
  {
    /* Failed read/write already raised. Write an entry to error log */
...
HASH ddl_recovery_gtid_hash;
bool ddl_recovery_gtid_hash_inited= false;

static void
store_slave_gtid_during_ddl_recovery(uint32 domain_id,
                                     uint32 server_id,
                                     uint64 seq_no,
                                     uint64 sub_id)
{
  ...
  if (rpl_global_gtid_slave_state->loaded)
  {
    ...
    rpl_gtid gtid{domain_id, server_id, seq_no};
    if (rpl_global_gtid_slave_state->recover_gtid(current_thd, &gtid, sub_id))
    {
      sql_print_error("Fatal error during DDL recovery of slave position, "
                      "cannot continue");
      abort();
    }
    return;
  ...
  /*
    We got a recovered GTID for the mysql.gtid_slave_pos early during server
    startup, before the position has been loaded from the on-disk table.
    This can happen during MYSQL_BIN_LOG::recover().

    Save the to-be-recovered GTID for later, it will be applied when we
    load the position.
  */
  if (!ddl_recovery_gtid_hash_inited)
  {
    my_hash_init(key_memory_DDL_LOG_MEMORY_ENTRY, &ddl_recovery_gtid_hash,
                 &my_charset_bin, 4,
                 offsetof(ddl_recovery_gtid, domain_id),
                 sizeof(ddl_recovery_gtid::domain_id),
                 NULL, my_free, HASH_UNIQUE);
    ddl_recovery_gtid_hash_inited= true;
  }
  ddl_recovery_gtid *g= (ddl_recovery_gtid *)
    my_hash_search(&ddl_recovery_gtid_hash, (const uchar *)&domain_id,
                   sizeof(domain_id));
  if (!g)
  {
    if (!(g= (ddl_recovery_gtid *)my_malloc(key_memory_DDL_LOG_MEMORY_ENTRY,
                                            sizeof(*g), MYF(MY_WME))))
      abort();
    ...
    g->seq_no= seq_no;
    g->sub_id= sub_id;
    if (my_hash_insert(&ddl_recovery_gtid_hash, (uchar *)g))
    {
      sql_print_error("Out of memory during DDL recovery of slave position, "
                      "cannot continue");
      abort();
    }
  }
  else if (g->sub_id <= sub_id)
  {
    DBUG_ASSERT(g->sub_id != sub_id);
    g->domain_id= domain_id;
    g->server_id= server_id;
    g->seq_no= seq_no;
    g->sub_id= sub_id;
...
static void ddl_log_to_binary_log(THD *thd, String *query)
{
  ...
  */
  ...
  store_slave_gtid_during_ddl_recovery(recovery_state.domain_id,
                                ...
                                char *from_path, char *to_path)
{
  ...
  int error= 0, err2;
  MDL_request mdl_request_from, mdl_request_to;
  ...
  if (mdl_request_from.ticket)
    thd->mdl_context.release_lock(mdl_request_from.ticket);
  if (mdl_request_to.ticket)
    thd->mdl_context.release_lock(mdl_request_to.ticket);
  DBUG_RETURN(error);
}


static int execute_drop_table(THD *thd, handlerton *hton, const LEX_CSTRING *db,
                              const LEX_CSTRING *table, const char *path)
{
  uint keys, total_keys;
  ...
  /*
    ...
    still be an open table definition for the table when we come here.
    To make things safe, remove the table definition if it exists.
  */
  ...
  if (get_hlindex_keys_by_open(thd, db, table, path, &keys, &total_keys) == 0)
  {
    ...
    for (uint i= keys; i < total_keys; i++)
    {
      ...
      error= hton->drop_table(hton, idx_path);
      if (!non_existing_table_error(error) && !first_error)
        first_error= error;
    }
  }
  if (!first_error)
                                  ...
                                  DDL_LOG_ENTRY *ddl_log_entry)
{
    ...
    case DDL_RENAME_PHASE_STAT:
     if (fn_flags & FN_TO_IS_TMP)
     {
       /*
         ...
         the original table to a temporary name.
  ...
  */
    ...
    */
    ...
    case DDL_ALTER_TABLE_PHASE_COPIED:
    {
      ...
      */
      ...
      if (likely(org_hton))
      {
        error= execute_drop_table(thd, org_hton, &ddl_log_entry->from_db,
        ...
        if (likely(org_hton))
        {
          if (ddl_log_entry->flags & DDL_LOG_FLAG_ALTER_ENGINE_CHANGED)
          {
            ...
            build_table_filename(to_path, sizeof(from_path) - 1,
                                 ...
                                 "", 0);
            error= execute_drop_table(thd, org_hton, &ddl_log_entry->from_db,
                                      &ddl_log_entry->from_name, to_path);
          }
          else
            error= execute_drop_table(thd, org_hton, &ddl_log_entry->from_db,
                                      &ddl_log_entry->from_name, from_path);
  ...
  /*
    ...
    The old value of error is only needed for debugging and can be
    ignored.
    no_such_table_handler has recorded all relevant errors
   */
...
bool ddl_log_close_binlogged_events(HASH *xids)
{
  ...
  for (i= 1; i <= global_ddl_log.num_entries; i++)
  {
                       ...
                       sizeof(ddl_log_entry.xid)))
    {
      ...
      store_slave_gtid_during_ddl_recovery(ddl_log_entry.domain_id,
...
{
  DDL_LOG_ENTRY ddl_log_entry;
  DBUG_ENTER("ddl_log_rename_table");
