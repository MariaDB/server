        ...
        */
        ...
        if (can_eliminate)
        {
          ...
          */
          ...
          /*
            Remove from JOIN::group_list
          */
          for (ORDER *prev= NULL, *order= subq_select_lex->join->group_list;
                order;
                order= order->next)
          {
            if (*order->item == *ord->item)               // to be removed
            {
              if (prev)
                prev->next= order->next;
              else
                subq_select_lex->join->group_list= order->next;
              break;
            }
            prev= order;
          }
