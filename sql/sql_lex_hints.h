...
public:
  Hint_scanner(THD *thd, uint lineno_arg, const char *buf, uint len,
               sql_digest_state *digest_state_arg);
  uint get_lineno() const { return lineno; }
...
protected:
  ...
  bool skip_mb()
  {
    uint len = my_ismbchar(cs, ptr, input_buf_end);
    if (len == 0)
    {
      ptr++;
      yyleng++;
  ...
  void compact(LEX_STRING *to, const char *from, uint len, uint doubles)
  {
    assert(doubles > 0);
