...
static inline LEX_CSTRING to_lex(const string &s)
{
...
struct priv_node
{
  ...
  priv_node *parent;
  priv_node *first_child;
  priv_node *next_sibling;
};

static inline bool node_less_key(const priv_node *a,
                                 ACL_PRIV_TYPE type,
                                 const string &db,
                                 const string &table,
                                 const string &column)
...
static inline bool node_equal_key(const priv_node *a,
                                  ACL_PRIV_TYPE type,
                                  const string &db,
                                  const string &table,
                                  const string &column)
...
public:
  ...
  void add_all(const vector<deny_entry_t> &v)
  {
    for (const deny_entry_t &d : v)
      add(d);
  }

  vector<deny_entry_t> finish()
  {
    build_tree_if_needed();
    compute_subtree_denies(m_root);

    vector<deny_entry_t> out;
...
private:
  priv_node *intern_node(ACL_PRIV_TYPE type,
                         const string &db,
                         const string &name,
                         const string &column)
  {
    vector<priv_node *>::iterator it= std::lower_bound(
      m_nodes.begin(), m_nodes.end(), type,
      [&](const priv_node *n, ACL_PRIV_TYPE)
      {
        return node_less_key(n, type, db, name, column);
      });

    if (it != m_nodes.end() && node_equal_key(*it, type, db, name, column))
      return *it;

    priv_node *n= new priv_node;
    n->data.type= type;
    n->data.db= db;
    n->data.table= name;
  ...
  priv_node *find_parent(priv_node *n)
  {
  ...
  void attach_child(priv_node *parent, priv_node *child)
  {
    child->parent= parent;
  ...
  void build_tree_if_needed()
  {
    ...
    {
      priv_node *n= m_nodes[i++];
      if (n == m_root)
        continue;

      priv_node *p= find_parent(n);
      if (p)
        attach_child(p, n);
    }
  ...
  privilege_t compute_subtree_denies(priv_node *n)
  {
    privilege_t acc= NO_ACL;

    for (priv_node *c= n->first_child; c; c= c->next_sibling)
  ...
  void flatten(priv_node *n, vector<deny_entry_t> &out)
  {
...
std::vector<deny_entry_t>
diff_deny_closures(const std::vector<deny_entry_t> &old_closure_in,
                   const std::vector<deny_entry_t> &new_closure_in)
{
  vector<deny_entry_t> old_cl= old_closure_in;
  vector<deny_entry_t> new_cl= new_closure_in;

  std::sort(old_cl.begin(), old_cl.end(), entry_less);
  std::sort(new_cl.begin(), new_cl.end(), entry_less);

  vector<deny_entry_t> delta;
...
{
  vector<deny_entry_t> old_cl= build_deny_closure(old_input);
  vector<deny_entry_t> new_cl= build_deny_closure(new_input);
