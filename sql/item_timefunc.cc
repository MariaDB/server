...
{
  constexpr ulong seconds_in_month31= 3600 * 31 * 24;
  THD *thd= current_thd;
  Datetime::Options opt(TIME_NO_ZEROS, thd);
  const Datetime dt0(thd, args[0], opt);
  if ((null_value= !dt0.is_valid_datetime()))
    return 0.0;
  const Datetime dt1(thd, args[1], opt);
  if ((null_value= !dt1.is_valid_datetime()))
    return 0.0;
  ...
  /*
    ...
    regarded as equal
  */
  const MYSQL_TIME *d0= dt0.get_mysql_time();
  const MYSQL_TIME *d1= dt1.get_mysql_time();
  if (d0->day == d1->day || (is_last_day(d0) && is_last_day(d1)))
    return ((int) d0->year - (int) d1->year) * 12 +
           ((int) d0->month - (int) d1->month);

  return (dt0.to_seconds_ff() - dt1.to_seconds_ff()) / seconds_in_month31;
}
