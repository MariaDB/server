...
static double fractional_day(const MYSQL_TIME *t)
{
  ...
  ulonglong milliseconds= (t->hour * 3600LL + t->minute * 60LL +
                           t->second)*1000 + t->second_part / 1000;
  /*
...
double Item_func_months_between::val_real()
{
  ...
  */
  ...
  if (d1->day > d2->day)
    frac= (d1->day - d2->day) / 31.0;
  else
  {
    months--;
    frac= (31 - d2->day + d1->day) / 31.0;
  }
  ...
  /* Adjust fractional difference based on time of day */
  if (frac_d1 < frac_d2)
    frac-= 1.0 / 31.0;
#endif /* EXTENDED_MONTHS_BETWEEN */
...
Item_func_trunc::enum_trunc Item_func_trunc::get_trunc_option(String *format_arg)
{
...
{
  Item_datefunc::fix_length_and_dec(thd);
