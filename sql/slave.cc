...
READ_ONLY_SYSVAR my_bool replicate_same_server_id;
...
/*
  Build a "SET @var_name='id1,id2,...'" query string from a DYNAMIC_ARRAY of
  ulong domain ids.
                       ...
                       const LEX_CSTRING *var_name)
{
  ...
  if (query_str->append(STRING_WITH_LEN("SET @"), system_charset_info) ||
      query_str->append(var_name->str, var_name->length,
                        system_charset_info) ||
  ...
  for (uint i= 0; i < ids->elements; i++)
  {
    ...
    if ((i > 0 && query_str->append(',')) ||
        query_str->append_ulonglong(domain_id))
      return true;
  }
  return query_str->append(STRING_WITH_LEN("'"), system_charset_info);
}
    ...
    */
    {
      ...
      if (ids)
      {
        if (build_domain_ids_query(&query_str, ids, var_name))
        {
          err_code= ER_OUTOFMEMORY;
          errmsg= "The slave I/O thread stops because a fatal out-of-memory "
