   ...
   total_ddl_groups(0), total_non_trans_groups(0), total_trans_groups(0)
{
  ...
  */
  ...
  mysql_mutex_init(key_master_info_start_stop_lock, &start_stop_lock,
                   MY_MUTEX_INIT_SLOW);
  mysql_mutex_init(0, &start_alter_struct_master.start_alter_list_lock,
                    MY_MUTEX_INIT_FAST);
  mysql_mutex_init(0, &start_alter_struct_master.start_alter_lock,
                    MY_MUTEX_INIT_FAST);
  ...
  mysql_cond_init(0, &start_alter_struct_master.start_alter_list_cond, NULL);
}
...
Master_info::~Master_info()
{
  ...
  mysql_mutex_destroy(&start_alter_struct_master.start_alter_list_lock);
  mysql_mutex_destroy(&start_alter_struct_master.start_alter_lock);
  mysql_cond_destroy(&data_cond);
  mysql_cond_destroy(&start_cond);
  mysql_cond_destroy(&stop_cond);
  mysql_cond_destroy(&sleep_cond);
  mysql_cond_destroy(&start_alter_struct_master.start_alter_list_cond);
}
