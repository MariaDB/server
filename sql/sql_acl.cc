...
/**
  ...
  in-memory caches (acl_users, acl_dbs, column_priv_hash, etc.).
  ...
  @param diff     deny entries calculated by diff_deny_closure_inputs()
                  or by build_deny_closure

  @return 0 on success, non-zero on error
*/
static int
apply_deny_closure_to_caches(const LEX_USER &combo,
                             const deny_set_t &diff)
{
...
static int collect_denies(const User_table &user_table, deny_set_t & out)
{
  auto collect_to_vector= [](const deny_callback_data_t *e, void *ctx) -> int {
    auto *deny_set= static_cast<deny_set_t *>(ctx);
                             ...
                             const deny_entry_t &e)
{
  ...
  if (!grant_table)
  {
    if (!has_denies)
      return 0;
    /* Parent table entry should exist: closure is sorted and applied top-down. */
    DBUG_ASSERT(0);
    return 1;
  }
