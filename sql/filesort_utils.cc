...
/*
 ...
 With PQ we could have a simple key (memcmp) or a complex key (double
 & varchar for example)

 PQ_SORT -> With / Without addon fields

   TODO(cvicentiu)
 * With fixed row length comparison
 * */
  ...
  */
  ...
  if (queue_size < num_available_keys)
  {
    costs[PQ_SORT_ORDER_BY_FIELDS]=
      ...
      param->sort_form->file->ha_rndpos_time(MY_MIN(queue_size, num_rows));
  }
