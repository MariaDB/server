...
bool ha_partition::create_handler_file(const char *name)
{
  ...
  for (i= 0; i < num_parts; i++)
  {
    ...
    if (part_elem->part_state != PART_NORMAL &&
        ...
        part_elem->part_state != PART_CHANGED)
      continue;
    if (error_if_mysql50_prefix(part_elem->partition_name, ER_WRONG_PARTITION_NAME))
      DBUG_RETURN(TRUE);
    if (!tablename_to_filename(part_elem->partition_name, part_name, FN_REFLEN))
      DBUG_RETURN(TRUE);
    ...
    else
    {
      ...
      {
	subpart_elem= sub_it++;
        if (error_if_mysql50_prefix(subpart_elem->partition_name, ER_WRONG_PARTITION_NAME))
          DBUG_RETURN(TRUE);
        if (!tablename_to_filename(subpart_elem->partition_name, subpart_name,
                                   FN_REFLEN))
          DBUG_RETURN(TRUE);
