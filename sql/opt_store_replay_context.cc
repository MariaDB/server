...
static void dump_double(double val, String &sql_script, bool add_comma= false)
{
  char buf[64];
...
static void dump_string(const char *val, size_t len, String &sql_script,
                        bool add_comma= false)
{
  sql_script.append(STRING_WITH_LEN("\'"));
  sql_script.append(val, len);
  sql_script.append(STRING_WITH_LEN("\'"));
  if (add_comma)
    sql_script.append(STRING_WITH_LEN(", "));
}

static void dump_ulonglong(ulonglong val, String &sql_script,
                           bool add_comma= false)
{
  sql_script.append_ulonglong(val);
  if (add_comma)
    sql_script.append(STRING_WITH_LEN(", "));
}

static void dump_start_eits_table_info(TABLE_LIST *tbl, enum_stat_tables pos,
                                       String &sql_script)
{
  sql_script.append(STRING_WITH_LEN("REPLACE INTO mysql."));
  sql_script.append(stat_table_name[pos]);
  sql_script.append(STRING_WITH_LEN(" VALUES("));
  dump_string(tbl->get_db_name().str, tbl->get_db_name().length, sql_script,
              true);
  dump_string(tbl->get_table_name().str, tbl->get_table_name().length,
              sql_script, true);
}

static void dump_end_eits_table_info(String &sql_script)
{
  sql_script.append(STRING_WITH_LEN(");\n\n"));
}

int dump_eits(THD *thd, List<TABLE_LIST> *tables_list, String &sql_script)
{
  DBUG_ENTER("Optimizer_context_replay::read_stats");
  ...
  // read stats value for the tables the query uses.
  for (TABLE_LIST *tbl= li++; tbl; tbl= li++)
  {
    ...
    */
    ...
    if (!(stats_cb=
              read_statistics_for_table(thd, table, stat_tables, true, true)))
    {
    ...
    if (Table_statistics *table_stats= stats_cb->table_stats)
    {
      dump_start_eits_table_info(tbl, TABLE_STAT, sql_script);
      sql_script.append_ulonglong(table_stats->cardinality);
      dump_end_eits_table_info(sql_script);
      if (Column_statistics *col_stats= table_stats->column_stats)
      {
        Field **field_ptr;
        for (field_ptr= table_share->field; *field_ptr;
             field_ptr++, col_stats++)
        {
          Field *table_field= *field_ptr;
          dump_start_eits_table_info(tbl, COLUMN_STAT, sql_script);
          dump_string(table_field->field_name.str,
                      table_field->field_name.length, sql_script, true);
          if (table_field->type() == MYSQL_TYPE_BIT)
          {
            dump_ulonglong(col_stats->min_value->val_int(), sql_script, true);
            dump_ulonglong(col_stats->max_value->val_int(), sql_script, true);
          }
          else
          {
            StringBuffer<MAX_FIELD_WIDTH> val(&my_charset_bin);
            col_stats->min_value->val_str(&val);
            dump_string(val.c_ptr_safe(), val.length(), sql_script, true);
            col_stats->max_value->val_str(&val);
            dump_string(val.c_ptr_safe(), val.length(), sql_script, true);
          }
          dump_double(col_stats->get_nulls_ratio(), sql_script, true);
          dump_double(col_stats->get_avg_length(), sql_script, true);
          dump_double(col_stats->get_avg_frequency(), sql_script, true);
          if (col_stats->histogram_exists)
          {
            String ser_hist;
            col_stats->histogram->serialize(&ser_hist);
            dump_ulonglong(ser_hist.length(), sql_script, true);
            dump_ulonglong(col_stats->histogram->get_type(), sql_script, true);
            dump_string(ser_hist.c_ptr_safe(), ser_hist.length(), sql_script,
                        false);
          }
          else
          {
            dump_ulonglong(0, sql_script, true);
            dump_ulonglong(JSON_HB, sql_script, true);
            dump_string(NULL, 0, sql_script, false);
          }
          dump_end_eits_table_info(sql_script);
        }
      }
      if (Index_statistics *idx_stats= table_stats->index_stats)
      {
        KEY *key_info, *key_info_end;
        for (key_info= table_share->key_info,
            key_info_end= key_info + table_share->keys;
             key_info < key_info_end; key_info++, idx_stats++)
        {
          uint k= key_info->user_defined_key_parts;
          for (uint i= 0; i < k; i++)
          {
            dump_start_eits_table_info(tbl, INDEX_STAT, sql_script);
            dump_string(key_info->name.str, key_info->name.length, sql_script,
                        true);
            dump_ulonglong(i, sql_script, true);
            dump_double(idx_stats->get_avg_frequency(i), sql_script, false);
            dump_end_eits_table_info(sql_script);
          }
        }
      }
...
bool store_optimizer_context(THD *thd)
{
  ...
  */
  ...
  if (!res)
    res= dump_eits(thd, &uniq_tables_list, sql_script);
