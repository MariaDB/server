                                            ...
                                            size_t tbl_name_len)
{
  ...
  while (Multi_range_read_const_call_record *irc= irc_li++)
  {
    ...
    Json_writer_array cost_wrapper(thd, "cost");
    {
      cost_wrapper.add(irc->cost.avg_io_cost);
      cost_wrapper.add(irc->cost.cpu_cost);
      cost_wrapper.add(irc->cost.comp_cost);
      cost_wrapper.add(irc->cost.copy_cost);
      cost_wrapper.add(irc->cost.limit_cost);
      cost_wrapper.add(irc->cost.setup_cost);
      cost_wrapper.add(irc->cost.index_cost.io);
      cost_wrapper.add(irc->cost.index_cost.cpu);
      cost_wrapper.add(irc->cost.row_cost.io);
      cost_wrapper.add(irc->cost.row_cost.cpu);
    }
    cost_wrapper.end();
    ...
    trace_table_context_read *table_ctx)
{
  ...
  while (!(rc= json_scan_next(je)) && je->state != JST_OBJ_END)
  {
    ...
    if (json_key_matches(je, name_key.get()))
    {
      ...
      StringBuffer<128> name_buf;
      bool res= json_unescape_to_string((const char *) je->value,
                                        je->value_len, &name_buf);
      if (!res)
      {
        table_ctx->name= 
            strdup_root(thd->mem_root, name_buf.c_ptr_safe());
        have_name= true;
        continue;
      }
      *err= "un-escaping error of name element";
      return 1;
    }
    save1.restore_to(je);
    ...
    if (json_key_matches(je, ddl_str.get()))
    {
      ...
      StringBuffer<128> ddl_buf;
      bool res= json_unescape_to_string((const char *) je->value,
                                        je->value_len, &ddl_buf);
      if (res)
      {
        *err= "un-escaping error of ddl name element";
        return 1;
      }
      table_ctx->ddl= strdup_root(thd->mem_root, ddl_buf.c_ptr_safe());
      have_ddl= true;
      continue;
    }
    save1.restore_to(je);
    ...
    if (json_key_matches(je, read_cost_io_key.get()))
    {
      if (json_read_value(je))
      {
        *err= "error reading read_cost_io value";
        return 1;
      }
      ...
      char *size_end= (char *) je->value_end;
      int conv_err;
      double read_cost_io= my_strtod(size, &size_end, &conv_err);
      if (conv_err)
      {
        *err= "read_cost_io member must be a floating point value";
        return 1;
      }
      table_ctx->read_cost_io= read_cost_io;
    ...
    if (json_key_matches(je, read_cost_cpu_key.get()))
    {
      if (json_read_value(je))
      {
        *err= "error reading read_cost_cpu value";
        return 1;
      }
      ...
      char *size_end= (char *) je->value_end;
      int conv_err;
      double read_cost_cpu= my_strtod(size, &size_end, &conv_err);
      if (conv_err)
      {
        *err= "read_cost_cpu member must be a floating point value";
        return 1;
      }
      table_ctx->read_cost_cpu= read_cost_cpu;
    ...
    trace_index_context_read *index_ctx)
{
  ...
  while (!(rc= json_scan_next(je)) && je->state != JST_OBJ_END)
  {
    ...
    if (json_key_matches(je, index_name_key.get()))
    {
      ...
      StringBuffer<128> idx_name_buf;
      bool res= json_unescape_to_string((const char *) je->value,
                                        je->value_len, &idx_name_buf);
      if (!res)
      {
        index_ctx->idx_name= strdup_root(thd->mem_root,
            idx_name_buf.c_ptr_safe());
  ...
  while (!(rc= json_scan_next(je)) && je->state != JST_OBJ_END)
  {
    ...
    if (json_key_matches(je, index_name_key.get()))
    {
      ...
      if (!res)
      {
        range_ctx->index_name=
            strdup_root(thd->mem_root, idx_name_buf.c_ptr_safe());
        have_index_name= true;
        continue;
      }
      *err= "un-escaping error of index_name element";
    ...
    if (json_key_matches(je, cost_key.get()))
    {
      ...
      while (je->state != JST_ARRAY_END)
      {
        if (json_read_value(je))
        {
          *err= "error reading cost value";
          return 1;
        }

        StringBuffer<128> size_buf;
        bool res= json_unescape_to_string((const char *) je->value,
                                          je->value_len, &size_buf);
        if (res)
        {
          *err= "un-escaping error of cost element";
          return 1;
        }
        char *size= size_buf.c_ptr_safe();
        int conv_err;
        char *endptr= size + strlen(size);
        double cost_elem= my_strtod(size, &endptr, &conv_err);
        if (conv_err)
        {
          *err= "element of cost array must be a floating point value";
          return 1;
        }
        switch (num_elements)
        {
        case 0:
          range_ctx->cost.avg_io_cost= cost_elem;
          break;
        case 1:
          range_ctx->cost.cpu_cost= cost_elem;
          break;
        case 2:
          range_ctx->cost.comp_cost= cost_elem;
          break;
        case 3:
          range_ctx->cost.copy_cost= cost_elem;
          break;
        case 4:
          range_ctx->cost.limit_cost= cost_elem;
          break;
        case 5:
          range_ctx->cost.setup_cost= cost_elem;
          break;
        case 6:
          range_ctx->cost.index_cost.io= cost_elem;
          break;
        case 7:
          range_ctx->cost.index_cost.cpu= cost_elem;
          break;
        case 8:
          range_ctx->cost.row_cost.io= cost_elem;
          break;
        case 9:
          range_ctx->cost.row_cost.cpu= cost_elem;
          break;
        default:
          break;
        }
    ...
    if (json_key_matches(je, max_index_blocks_key.get()))
    {
      if (json_read_value(je))
      {
        *err= "error reading max_index_blocks value";
        return 1;
      }
      ...
      char *size_end= (char *) je->value_end;
      int conv_err;
      ha_rows max_index_blocks= my_strtoll10(size, &size_end, &conv_err);
      if (conv_err)
      {
        *err= "max_index_blocks member must be a numeric value";
        return 1;
      }
    ...
    if (json_key_matches(je, max_row_blocks_key.get()))
    {
      if (json_read_value(je))
      {
        *err= "error reading max_row_blocks value";
        return 1;
      }
      ...
      char *size_end= (char *) je->value_end;
      int conv_err;
      ha_rows max_row_blocks= my_strtoll10(size, &size_end, &conv_err);
      if (conv_err)
      {
        *err= "max_row_blocks member must be a numeric value";
        return 1;
      }
