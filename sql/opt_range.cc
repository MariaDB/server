                     ...
                     Item_func::Functype type, Item *value)
{
  ...
  */
  if (type != Item_func::LIKE_FUNC)
  {
    ...
    tmp2.copy(*res);
    int ret;
    const char *p= tmp2.ptr(), *q= res->ptr(), *end= p + MAX_FIELD_WIDTH;
    my_wc_t wc;
    CHARSET_INFO *cs= field->charset();
    while (p < tmp2.end())
    {
      /* Advance to the next character */
      if ((ret= my_ci_mb_wc(cs, &wc, (uchar *) p, (uchar *) tmp2.end())) <= 0)
      {
      ...
      {
        if ((ret= my_ci_wc_mb(cs, (my_wc_t) '\\', (uchar *) q,
                              (uchar *) end)) <= 0)
          DBUG_RETURN(0); /* No space - no LIKE optimize */
        q+= ret;
      }
      if ((ret= my_ci_wc_mb(cs, wc, (uchar *) q, (uchar *) end)) <= 0)
        DBUG_RETURN(0); /* No space - no LIKE optimize */
      q+= ret;
    }
    res->length(q - res->ptr());
    res->append("%", 1);
  }
