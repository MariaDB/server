                        ...
                        double *read_cost, ha_rows *out_records)
{
  ...
  /*
    ...
    group.
  */
  *read_cost= file->ha_keyread_and_compare_time(index_info - table->key_info,
                                                num_groups, num_groups,
                                                io_cost);
  *out_records= num_groups;

  DBUG_PRINT("info",
             ("rows: %lu  keys/block: %u  keys/group: %lu  "
              "result rows: %lu  blocks: %lu",
              (ulong) records, keys_per_block, (ulong) keys_per_group,
