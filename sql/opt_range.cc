...
public:
    ...
    range_info(system_charset_info)
  {
    ...
    image_type= param->key[idx][keypart->part].image_type;
    seq.keyno= idx;
    seq.key_parts= param->key[idx];
...
/*
  ...
  @detail
    ...
    and an itMBR keypart always coincide -- get_mm_leaf() and
    get_mm_leaf_for_sargable() both refuse to build a range for a non-itRAW
    keypart unless it is a GEOM range -- but passing image_type keeps this
