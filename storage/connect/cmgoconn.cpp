...
bool CMgoConn::Connect(PGLOBAL g)
{
	if (!Pcg->Db_name || !Pcg->Coll_name) {
		...
		strlcpy(g->Message, "Missing DB or collection name", sizeof(g->Message));
		return true;
	}	// endif name
	...
	if (!IsInit)
		...
		__try {
		  ...
		  strlcpy(g->Message, "Cannot load MongoDB C driver", sizeof(g->Message));
		  return true;
	  }	// end try/except
#else   // !_WIN32
		mongo_init(true);
...
bool CMgoConn::MakeCursor(PGLOBAL g)
{
	...
	if (Pcg->Pipe) {
		...
		if (!p) {
			strlcpy(g->Message, "Missing ] in pipeline", sizeof(g->Message));
			return true;
		} else
			*(char*)p = 0;
		...
		if (filp) {
			...
			if (MakeSelector(g, filp, s)) {
				strlcpy(g->Message, "Failed making selector", sizeof(g->Message));
				return true;
			} else
				s->Append('}');
...
int CMgoConn::Write(PGLOBAL g)
{
	...
	if (tp->GetMode() == MODE_INSERT) {
		...
		if (b) {
			...
			strlcpy(g->Message, "Mongo update: cannot find _id", sizeof(g->Message));
			rc = RC_FX;
		}	// endif b
