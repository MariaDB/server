...
the SST files instead of copying them.

Its base name is deliberately ROCKSDB_BACKUP_DIR, so that the name
of a checkpoint file in the backup is a suffix of its path in the
file system: <rocksdb_datadir>/#rocksdb/<name> is stored as
#rocksdb/<name>. That is what lets backup::copy_or_stream() derive
the one from the other by omitting a prefix, on Windows as well,
where the source path and the name in the backup are one string. */
#define ROCKSDB_CHECKPOINT_SUBDIR ROCKSDB_BACKUP_DIR
...
/** Back up one file of the checkpoint that was frozen in
rocksdb_backup_start().
@param thd     current session
@param target  backup target
