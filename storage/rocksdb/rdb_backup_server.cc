...
public:
  ...
  /** Create the "#rocksdb" subdirectory in a directory target, exactly once.
...
private:
  ...
  /** next index to hand to a step thread */
  std::atomic<size_t> next{0};
  /** whether the "#rocksdb" subdir has been created in a directory target */
  std::atomic<bool> subdir_made{false};
                             ...
                             const std::string &name) noexcept
{
  ...
  else
  {
    ...
    else if (size_t pad= size_t(end) & 511)
      if (backup_stream_write(sink.stream, zerobuf, 512 - pad))
      {
        my_error(ER_ERROR_ON_WRITE, MYF(0), rel.c_str(), errno);
        ret_val= 1;
      }
  ...
  else if (size_t pad= size_t(li.LowPart) & 511)
