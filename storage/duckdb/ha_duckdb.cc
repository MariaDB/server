    ...
    const char *table_name)
{
    ...
    auto cols = [](auto &parts) {
        ...
        while (const Key_part_spec *part = it++) {
            ...
            out += '`';
            out.append(part->field_name.str, part->field_name.length);
            out += '`';
        }
        return out;
    };
    ...
    while ((key = it++)) {
        ...
        if (fk->constraint_name.str != nullptr && fk->constraint_name.length > 0) {
            clause = "CONSTRAINT `"
                     + std::string(fk->constraint_name.str,
                                   fk->constraint_name.length)
                     + "` ";
        }
        clause += "FOREIGN KEY (" + cols(fk->columns) + ") REFERENCES `"
            + std::string(fk->ref_table.str, fk->ref_table.length) + "`";
