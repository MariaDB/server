...
static ulong hp_hashnr(HP_KEYDEF *keydef, const uchar *key)
{
  ...
  for (seg=keydef->seg,endseg=seg+keydef->keysegs ; seg < endseg ; seg++)
  {
    ...
    else
    {
      ...
      {
	hasher.m_nr1^=(ulong) ((((uint) hasher.m_nr1 & 63)+hasher.m_nr2)*((uint) *pos)) + (hasher.m_nr1 << 8);
	hasher.m_nr2+=3;
      }
    }
  }
...
ulong hp_rec_hashnr(register HP_KEYDEF *keydef, register const uchar *rec)
{
  ...
  for (seg=keydef->seg,endseg=seg+keydef->keysegs ; seg < endseg ; seg++)
  {
	...
	hasher.m_nr1^=(ulong) ((((uint) hasher.m_nr1 & 63)+hasher.m_nr2)*((uint) bits))+ (hasher.m_nr1 << 8);
	hasher.m_nr2+=3;
        end--;
      }
