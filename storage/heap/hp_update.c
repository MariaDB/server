...
int heap_update(HP_INFO *info, const uchar *old, const uchar *heap_new)
{
  ...
  if (lastinx_changed && (info->update & HA_STATE_NEXT_FOUND))
  {
    /*
      ...
      heap_rnext() hits the "!current_ptr && HA_STATE_NEXT_FOUND" guard, reports
      a false end-of-file and leaves a row behind. We only get here on an index
      scan; rnd scans (heap_scan()/heap_rrnd()) leave lastinx == -1, so despite
      also setting HA_STATE_NEXT_FOUND they never reach this. A plain single-row
      UPDATE has no preceding heap_rnext(), so the bit is unset and this is a
      no-op there.
    */
