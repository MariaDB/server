...
static void test_failed_recovery_marks_crashed(void)
{
  ...
  ok(heap_is_crashed(share),
     "the failed check leaves the still damaged table marked crashed");
...
/*
  Test 5: a check of an intact table clears a leftover crashed mark.

  The crashed mark makes every read and write refuse the table, but it is
  only a mark: when a check walks the whole structure and finds it intact,
  the mark has nothing left to protect and is dropped, so the check itself
  is the way back for a table whose damage has been repaired.
*/

static void test_check_clears_stale_crashed_mark(void)
{
  HP_SHARE *share;
  HP_INFO *info;
  uchar rec[REC_LENGTH];

  if (create_and_open("test_upd_stale", 2, HA_KEY_ALG_HASH,
                      HA_KEY_ALG_BTREE, 0, &share, &info) ||
      fill_and_position(info, rec))
  {
    ok(0, "setup failed: %d", my_errno);
  ...
  /* Set the mark without damaging the table */
  heap_mark_crashed(share);

  build_rec(rec, 3, 30);
  ok(heap_write(info, rec) == HA_ERR_CRASHED,
     "the mark alone refuses writes");
  ok(heap_check_heap(info, 0) == 0,
     "the check walks the intact structure and passes");
  ok(!heap_is_crashed(share), "the passed check cleared the mark");
  build_rec(rec, 3, 30);
  ok(heap_write(info, rec) == 0 && row_found(info, 0, 3),
     "the table accepts and finds rows again");
         ...
         char **argv __attribute__((unused)))
{
  ...
  diag("Test 5: a check of an intact table clears a leftover crashed mark");
  test_check_clears_stale_crashed_mark();
#endif
