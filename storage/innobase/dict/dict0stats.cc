...
struct IndexLevelStats
{
  ...
  void sample_leaf_pages(trx_t *trx, uint16_t n_prefix,
...
struct PageStats
{
  ...
  void scan_below(trx_t *trx, const btr_cur_t *cur) noexcept;
...
uint32_t IndexLevelStats::analyze_level() noexcept
{
	...
	/* The mini-transaction must only be holding index->lock,
	which was acquired at savepoint 0. */
	ut_ad(mtr->get_savepoint() == 1);
	...
	while (btr_pcur_is_on_user_rec(&pcur)) {
		...
		ut_ad(prev_savepoint < mtr->get_savepoint());
		ut_ad(prev_savepoint
		      ? page_align(prev_rec)
			== mtr->at_savepoint(prev_savepoint)->page.frame
		      : !prev_rec || page_align(prev_rec) == page);
...
next_rec:
		...
		for (;;) {
			...
			the page that it resides on, so that prev_rec can be
			compared with the next non-delete-marked record that
			we may find. The latch will be released once that
			comparison has been made. If prev_rec does not reside
			on the current page of pcur, that page will simply be
			released, and the latch on the page of prev_rec will
			be kept */
			const bool retain_latch = prev_rec && !prev_savepoint;
			ut_ad(retain_latch
			      == (prev_rec
				  && page_align(prev_rec)
				     == btr_pcur_get_page(&pcur)));
			if (retain_latch) {
				prev_savepoint = mtr->get_savepoint() - 1;
			}
...
void PageStats::scan_below(trx_t *trx, const btr_cur_t *cur) noexcept
{
	...
	NDiffData*		n_diff_data)const noexcept
{
	...
	for (i = 0; i < n_diff_data->n_analyze_leaf_pages; i++) {
		...
		leaf_stats.scan_below(trx, btr_pcur_get_btr_cur(&pcur));
...
@return index stats */
static IndexStats dict_stats_analyze_index(trx_t *trx,
                                           dict_index_t *index,
...
found_level:
		...
		level_stats.sample_leaf_pages(trx, n_prefix, data);
	}
...
dberr_t dict_stats_update_persistent(trx_t *trx, dict_table_t *table) noexcept
{
	...
	IndexStats stats = dict_stats_analyze_index(trx, index, stats_method);
	...
	for (index = dict_table_get_next_index(index);
		...
		table->stats_mutex_unlock();
		stats = dict_stats_analyze_index(trx, index, stats_method);
		table->stats_mutex_lock();
  ...
  noexcept
{
  if (table->stats_is_persistent() &&
    ...
    return dict_stats_update_persistent(trx, table);
  return DB_SUCCESS;
}
