...
public:
  dberr_t insert_table_stats(const TableStat *entry) noexcept
  {
    ...
    uint32_t n_fields= static_cast<uint32_t>(index->n_fields);
    dfield_t* fields=
      static_cast<dfield_t*>(alloca(n_fields * sizeof(dfield_t)));
    fill_stats_table_fields(entry, n_fields, fields); 
    dtuple_t tuple{0, n_fields, index->n_uniq, fields, 0, nullptr
#ifdef UNIV_DEBUG 
                   , DATA_TUPLE_MAGIC_N
#endif
		   };
                             ...
                             uint32_t n_fields= 0) noexcept
  {
    ...
    dfield_t* fields=
      static_cast<dfield_t*>(alloca(n_fields * sizeof(dfield_t)));
    fill_stats_table_fields(entry, n_fields, fields);
    dtuple_t tuple{0, n_fields, n_fields, fields, 0, nullptr
#ifdef UNIV_DEBUG 
                  , DATA_TUPLE_MAGIC_N
#endif
		  };
...
error_handling:
  ...
  mtr.start();
  mtr.set_named_space(table->space);
  if (m_pcur->restore_position(latch_mode, &mtr) !=
        btr_pcur_t::SAME_ALL)
  {
    err= DB_CORRUPTION;
    goto func_exit;
...
static void dict_index_stats_print_err(dict_index_t *index, dberr_t ret,
...
unlocked_free_and_exit:
  ...
  TableStat table_entry(db_utf8, table_utf8, &now, table->stat_n_rows,
                        table->stat_clustered_index_size,
                        table->stat_sum_of_other_index_sizes);
  ret= runner.delete_table_stats(&table_entry);
  if (ret == DB_SUCCESS)
    ret= runner.insert_table_stats(&table_entry);
...
free_and_exit:
  ...
  auto process_index_stats = [&](dict_index_t* index) -> dberr_t {
    ...
    if (ret)
      dict_index_stats_print_err(index, ret, index_entry.stat_name);
    return ret;
  };
  ...
  /* Below we do all the modifications in innodb_index_stats
  in a single transaction for performance reasons. Modifying
  more than one row in a single transaction may deadlock
  with other transactions if they lock the rows in different
  order. Other transaction could be for example when we
  DROP a table and do
