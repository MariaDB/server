    ...
    if (rec_offs_nth_extern(offsets, i))
    {
      uint32_t len=
        mach_read_from_4(btr_rec_get_field_ref(rec, offsets, i) +
                         BTR_EXTERN_LEN + 4);
      n_blobs+= len < table->space->physical_size();
    }
  }
  return n_blobs;
}
	...
	for (ulint i = 0; i < n_sample_pages; i++) {
		...
		while (rec != page_get_supremum_rec(page)) {
			...
			if (!next_rec
				...
				total_external_size +=
					rec_get_n_blob_pages(rec, offsets_rec, index->table);
				break;
			}
...
struct IndexLevelStats
{
  ...
  IndexLevelStats(unsigned stats_method, dict_index_t *index) :
    ...
    bounds(nullptr), n_recs(0), n_pages(0)
  {
  ...
  IndexLevelStats(uint16_t level_, unsigned stats_method,
    ...
    n_diff(static_cast<uint64_t*>(ut_zalloc(index->n_uniq * sizeof(*n_diff),
                                   mem_key_dict_stats_n_diff_on_level))),
    n_non_null(static_cast<uint64_t*>(
                 ut_zalloc(index->n_uniq * sizeof(*n_non_null),
                           mem_key_dict_stats_n_diff_on_level))),
...
struct LeafPageStats
{
  ...
  const bool nulls_unequal;
  /** Number of externally stored blob pages encountered */
  uint32_t n_blob= 0;
  /** Number of distinct key values found for the prefix */
  uint64_t n_diff= 0;
  /** Number of non-null key values found for the prefix */
  uint64_t n_non_null= 0;

  LeafPageStats(uint8_t n_pfx, bool nulls) :
	n_prefix(n_pfx), nulls_unequal(nulls) {}
};
...
@param[out]	leaf_stats		statistics to collect leaf
					...
					fields to compare
For NON-LEAF pages (template parameter <false>):
 n_diff: Quick scan to detect "boring" pages
 Returns 0 (empty), 1 (all equal), or 2 (found distinct record)
 Stops after first non-boring record for efficiency

For LEAF pages (template parameter <true>):
 n_diff: Full scan counting all distinct key values for n_prefix columns
 Compares consecutive records using cmp_rec_rec() with nulls_unequal setting
 n_blob: Counts externally stored blob pages via rec_get_n_blob_pages()
 n_non_null: Counts records where last prefix column is not NULL
	...
	LeafPageStats*		leaf_stats)
{
	...
	if (leaf) {
		leaf_stats->n_blob += rec_get_n_blob_pages(rec, offsets_rec,
							   index->table);
	}
	...
	mtr_t*			mtr)
{
	...
	for (i = 0; i < n_diff_data->n_leaf_pages_to_analyze; i++) {
		...
		LeafPageStats leaf_stats((uint8_t)n_prefix,
					 level_stats->nulls_unequal);
