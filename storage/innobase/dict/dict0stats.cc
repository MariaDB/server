...
Symbols: A sampled pages/prefix; L index leaf pages (L_ord = non-BLOB leaves);
LA the level chosen to sample from; see dict_stats_index_set_n_diff() [REF01]
...
typedef small_vector<uint64_t, 8>	boundaries_t;
					  ...
					  trx_id_t bulk_trx_id)
{
	...
	uint64_t	n_sample_pages = 1; /* number of pages to sample */
	ulint		not_empty_flag	= 0;
	uint64_t	n_blob_pages = 0;
	uintmax_t	add_on;
	mtr_t		mtr;
...
compute n_diff[] (= N_DIFF_LA, distinct keys on the level) and n_recs
(= TOTAL_LA, records on the level), and record in bounds[] the index of the
last record of each group of equal keys (0-based, counted continuously across
pages). These boundaries let sample_leaf_pages() split the level into groups
and pick one representative record per group.
*/
...
split level LA into n_leaf_pages_to_analyze (<= A) groups of equal keys, pick
the last record of each group and dive below it (scan_below()). Accumulate over
the samples: n_diff_all_analyzed_pages (= sum of Pi), n_non_null_all_analyzed_pages
and n_external_pages_sum (= E, external/BLOB pages).
...
Per n-prefix, on the sampled level LA (see dict_stats_analyze_index()):
  ...
  D        = ordinary leaves analyzed  (n_leaf_pages_to_analyze)
  E        = external/BLOB pages linked from those D leaves (n_external_pages_sum)
  ...
  avg(Pi)  = n_diff_all_analyzed_pages / D
...
removes them, giving an estimate over data-bearing leaves only. n_non_null uses
...
empty_index:
	...
	NDiffData*	n_diff_data =
	  static_cast<NDiffData*>(
		ut_zalloc_nokey(n_uniq * sizeof(NDiffData)));
