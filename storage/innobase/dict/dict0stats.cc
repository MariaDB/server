...
struct IndexLevelStats
{
  ...
  IndexLevelStats(ulint level_, unsigned stats_method,
                  dict_index_t* index) : level(level_)
  {
    nulls_unequal= (stats_method > SRV_STATS_NULLS_EQUAL);
    bool calc_non_null= (stats_method == SRV_STATS_NULLS_IGNORED);
    if (level == 0)
    {
      n_diff= index->stat_n_diff_key_vals;
      memset(n_diff, 0x0, index->n_uniq * sizeof(uint64_t));
      if (calc_non_null)
      {
        n_non_null= index->stat_n_non_null_key_vals;
	memset(n_non_null, 0x0, index->n_uniq * sizeof(uint64_t));
      }
    ...
    {
      n_diff= static_cast<uint64_t*>(
        ut_zalloc(index->n_uniq * sizeof(uint64_t),
...
struct LeafPageStats
{
  ...
  LeafPageStats(uint32_t n_pfx, bool nulls, bool non_null)
  : n_prefix(n_pfx), n_diff(0), n_blob(0), n_non_null(0),
    nulls_unequal(nulls), calc_non_null(non_null) {}
};
...
@param[out]	offsets1		rec_get_offsets() working
					space (must be big enough)
@param[out]	offsets2		rec_get_offsets() working
					space (must be big enough)
	...
	mtr_t*			mtr)
{
	...
	for (i = 0; i < n_diff_data->n_leaf_pages_to_analyze; i++) {
		...
		LeafPageStats leaf_stats(
			n_prefix, level_stats->nulls_unequal,
...
empty_index:
	...
	uint32_t n_uniq = dict_index_get_n_unique(index);
	...
	if (root_level == 0
		...
		IndexLevelStats leaf_stats(0, stats_method, index);
