...
public:
	...
	{
		return m_zip_ssize
			? page_zip_des_t::zip_size(m_zip_ssize) : srv_page_size;
	}
		...
		const page_t*	page) UNIV_NOTHROW
	{
		...
		if (mach_read_from_4(XDES_ARR_OFFSET + XDES_STATE + page)
			...
			const ulint physical_size = m_zip_ssize
				? page_zip_des_t::zip_size(m_zip_ssize)
				: srv_page_size;
...
FetchIndexRootPages::build_row_import(row_import* cfg) const UNIV_NOTHROW
{
	...
	cfg->m_zip_size = page_zip_des_t::zip_size(m_zip_ssize);
	cfg->m_n_indexes = 1;
	...
	const char *dir_path)
{
	...
	/* This block is a raw zeroed descriptor built outside the normal
	buf_pool allocation path, not a real buf_pool slot; it still has to
	satisfy buf_page_t::invalidate()'s assertion that the caller holds a
	page latch, which callback.run() below relies on via
	block->invalidate() (see page_cur_delete_rec()). Initialize and
	acquire a dummy exclusive latch here, debug-only like the assertion
	itself, and free it once this block is no longer used as one. */
	ut_d(block->page.lock.init());
	...
	if (err == DB_SUCCESS) {
		...
		iter.crypt_data = fil_space_read_crypt_data(
			page_zip_des_t::zip_size(callback.get_zip_ssize()),
			page);
