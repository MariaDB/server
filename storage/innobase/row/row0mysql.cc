...
struct row_mysql_truncate_t{
	...
	UT_LIST_NODE_T(row_mysql_truncate_t)row_mysql_truncate_list; /*!< list chain node */
};

/** @brief List of files we should truncate in background. Protected by row_truncate_list_mutex. */
...
/** Flag: has row_mysql_truncate_list been initialized? */
static ibool row_mysql_truncate_list_inited = FALSE;
...
{
	if (row_mysql_truncate_list_inited == FALSE)
		return false;
	...
	DBUG_EXECUTE_IF("ib_before_row_truncate_file", os_thread_sleep(3000000););
	struct stat st;
	int ret;
	...
	if (ret == -1) {
		if (errno == ENOENT)
			...
			if (trun->fail_count > MAX_TRUNCATE_FAIL_COUNT)
				...
				return true; /* master thread would retry */
		}
	}
	...
	const char*	name)	/*!< in: file name */
{
	...
	ut_a(row_mysql_truncate_list_inited);
...
init_mysql_truncate_list(char* dir_path)
{
	...
	ut_a(row_mysql_truncate_list_inited == FALSE);
...
/*================*/
{
	...
	if (srv_async_drop_tmp_dir != NULL && strcmp(srv_async_drop_tmp_dir, "") != 0) {
		...
		row_mysql_truncate_list_inited = FALSE;
	}
