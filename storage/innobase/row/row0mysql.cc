...
next_column:
	...
	if (!DICT_TF2_FLAG_IS_SET(prebuilt->table, DICT_TF2_FTS_HAS_DOC_ID)) {
		if (prebuilt->table->fts->cache->first_doc_id
		    == FTS_NULL_DOC_ID) {
			return fts_get_next_doc_id(prebuilt->table, &doc_id,
	...
	if (dberr_t err = fts_get_next_doc_id(prebuilt->table, &doc_id,
					      prebuilt->trx->mysql_thd)) {
		return err;
	}
	...
	ins_mode_t	ins_mode)
{
	...
	err = row_mysql_convert_row_to_innobase(node->row, prebuilt, mysql_rec,
						&blob_heap);
	if (err != DB_SUCCESS) {
		trx->op_info = "";
		if (blob_heap != NULL) {
