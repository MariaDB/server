	...
	*out_rec = NULL;
	...
	if (!page_rec_is_user_rec(clust_rec)
		...
		if  (dict_index_is_spatial(sec_index)
			...
			same as btr_pcur_get_block(clust_pcur),
			and is it not unsafe to use RW_NO_LATCH here? */
			buf_block_t*	block = buf_page_get_gen(
				btr_pcur_get_block(clust_pcur)->page.id(),
				btr_pcur_get_block(clust_pcur)->zip_size(),
...
requires_clust_rec:
		...
		err = row_sel_get_clust_rec_for_mysql(prebuilt->clust_ref,
						      prebuilt->clust_pcur,
						      prebuilt->select_lock_type,
						      prebuilt->trx,
						      prebuilt->old_vers_heap,
						      index, rec, thr, &clust_rec,
						      &offsets, &heap,
						      need_vrow ? &vrow : NULL,
						      &mtr);
...
rec_loop:
  ...
  else if (index->is_clust())
  {
    ...
    if (!trx->read_view.changes_visible(rec_trx_id))
    {
      ...
      err= row_sel_build_prev_vers_for_mysql(prebuilt->trx, index, rec, &offsets,
                                             &heap, prebuilt->old_vers_heap,
                                             &old_vers, nullptr, &mtr);
           ...
           page_get_max_trx_id(btr_pcur_get_page(prebuilt->pcur)))
  {
             ...
             !view.sees(page_trx_id))
    {
      ...
      check_old_vers:
        ...
        row_sel_reset_old_vers_heap(&prebuilt->old_vers_heap);
