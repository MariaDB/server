...
namespace flst
{
  ...
  @return error code */
  dberr_t remove_prepare(const buf_block_t &descr_block,
  ...
  @param mtr        mini-transaction */
  void remove_complete(buf_block_t *base, uint16_t boffset,
  ...
  @return error code */
  dberr_t append_prepare(const buf_block_t &base, uint16_t boffset,
  ...
  @param mtr        mini-transaction */
  void append_complete(buf_block_t *base, uint16_t boffset,
...
func_exit:
  ...
  inode= fseg_inode_try_get(seg_header, 0, 0, &mtr,
                            &iblock, &err);
  if (!inode)
    goto func_exit;
...
class spaceDefragmenter final
{
  ...
  dberr_t extract_extent_state() noexcept
  {
    ...
    buf_block_t *last_descr= buf_page_get_gen(page_id_t{m_space->id, 0}, 0,
                                              RW_S_LATCH, nullptr,
...
/** Validate the system tablespace list */
__attribute__((warn_unused_result))
static dberr_t fsp_tablespace_validate(fil_space_t *space) noexcept
{
  /* Validate all FSP list in system tablespace */
  mtr_t mtr;
  dberr_t err= DB_SUCCESS;
