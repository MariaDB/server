...
@param latch_type how the caller of the function latched lock_sys,
                  GLOBAL or CELL
@return true if the cell was latched successfully or if latch_type is GLOBAL,
        false otherwise */
static bool lock_rec_unlock_unmodified(buf_block_t *block, hash_cell_t *cell,
                                       lock_t *lock, rec_offs *&offsets,
                                       mem_heap_t *&heap,
                                       lock_sys_latch_type latch_type)
{
      ...
      if (latch->try_acquire())
      {
        ...
        else if (unlock_unmodified)
        {
          ...
          buf_block_t *block= btr_block_get(
              *lock->index, lock->un_member.rec_lock.page_id.page_no(),
              RW_S_LATCH, true, &mtr, nullptr, nullptr
#if defined(UNIV_DEBUG) || !defined(DBUG_OFF)
          ...
          if (block)
          {
            /* concurent lock_sys_t::resize() call could make a previously
            computed cell address invalid */
            cell= lock_sys.rec_hash.cell_get(lock->un_member.rec_lock.
                                             page_id.fold());
            latch= lock_sys_t::hash_table::latch(cell);
            if (latch->try_acquire())
            {
              if (!lock_rec_unlock_unmodified(block, cell, lock, offsets, heap,
                                              CELL))
                all_released= false;
              else
                latch->release();
      ...
      else
      {
        ...
        else if (unlock_unmodified)
        {
          ...
          if (block)
          {
            ...
            cell= lock_sys.rec_hash.cell_get(lock->un_member.rec_lock.
                                             page_id.fold());
            (void) lock_rec_unlock_unmodified(block, cell, lock, offsets, heap,
                                              GLOBAL);
          }
          else
            /* The block could be freed during merging pages when lock_sys
