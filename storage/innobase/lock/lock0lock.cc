...
extern const conflicting_lock_info null_c_lock_info{nullptr, nullptr, nullptr};
...
func_exit:
  ...
  else
  {
    ...
    if (lock_t *lock= lock_sys_t::get_first(cell, id))
    {
      ...
      do
      {
        /* Conflicting locks can be only before the waiting lock */
        if (lock == wait_lock)
          break;
...
/** Checks if the lock suits for bypassing.
@param lock lock to check
@param trx transaction for which the lock is checked
@return true if lock suits, false otherwise */
inline bool lock_rec_lock_suits_for_bypassing(const lock_t *lock, const trx_t *trx)
{
  return lock->is_waiting() && lock->trx->lock.wait_trx == trx &&
         lock_rec_mode_suits_for_bypassing(lock->type_mode);
}
...
@return conflicting lock and the lock after which new lock should be inserted
in lock queue in the case when some conflicting lock must be bypassed */
                               ...
                               const trx_t *trx)
{
       ...
       lock= lock_rec_get_next(heap_no, lock))
  {
    ...
    if (lock_rec_has_to_wait(trx, mode, lock, is_supremum))
    {
      ...
      is held, lock->trx->lock.wait_trx can't be changed. There also can't be
      lock loops for one record, because all waiting locks of the record  will
      always wait for the same lock of the record in a cell array, and check
                                           ...
                                           ulint heap_no)
{
  ...
  reorganizing, what causes validation failure. Skip the validation for such
  case. */
	...
	bool		holds_trx_mutex)
{
	...
	ut_ad(!c_lock_info.bypassed || c_lock_info.insert_after);
	...
	que_thr_t*		thr)	/*!< in: query thread */
{
  ...
  if (lock_t *lock= lock_sys_t::get_first(g.cell(), id))
  {
        ...
        lock_rec_get_n_bits(lock) <= heap_no)
    {
      ...
      if (!held_lock)
      {
        auto c_lock_info = lock_rec_other_has_conflicting(mode, g.cell(), id,
                                                           heap_no, trx);
...
struct lock_storage_t {
  ...
  lock_t *m_wait_lock;
  byte m_bitmap[128];
  lock_storage_t(const lock_t *lock)
      : m_lock_p(lock), m_lock_copy(*lock),
        m_wait_trx(lock->trx ? lock->trx->lock.wait_trx : nullptr),
        m_wait_trx_id(lock->trx && lock->trx->lock.wait_trx
                          ? lock->trx->lock.wait_trx->id
                          : 0),
        m_wait_lock(lock->trx ? lock->trx->lock.wait_lock : nullptr)
  {
...

  {
    ...
    *inherit= lock_sys_t::get_first(g.cell(), id, heap_no);
    ...
    if (*inherit)
    {
      ...
      auto c_lock_info = lock_rec_other_has_conflicting(type_mode,
                                                         g.cell(), id,
                                                         heap_no, trx);
      if (c_lock_info.conflicting)
      {
        // TODO: check there was not bypassed lock
        trx->mutex_lock();
        err= lock_rec_enqueue_waiting(c_lock_info, type_mode, id, block->page.frame,
