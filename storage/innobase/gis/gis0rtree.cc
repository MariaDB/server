	...
	double**	buf_pos)/*!< in/out: current buffer position */
{
	...
	*offsets = rec_get_offsets(rec, cursor->index, *offsets, format,
				   n_uniq, &heap);
	...
	mtr_t*		mtr)		/*!< in: mtr */
{
	...
	if (rec_info & REC_INFO_MIN_REC_FLAG) {
		...
		offsets2 = rec_get_offsets(old_rec, index, NULL, format,
					   ULINT_UNDEFINED, &heap);
	...
	mtr_t*			mtr)		/*!< in: mtr */
{
	...
	for MVCC. */
	if (format != REC_FMT_NODE_PTR && !index->table->is_temporary()) {
	...
	for (cur_split_node = node_array;
		...
		if (cur_split_node->n_node != first_rec_group) {
			...
			offsets = rec_get_offsets(
				...
				offsets, format, ULINT_UNDEFINED,
				&heap);
	...
	mtr_t*		mtr)		/*!< in: mtr */
{
	...
	ulint		moved = 0;
	const rec_fmt_t	format = page_is_leaf(new_page)
		? REC_FMT_LEAF : REC_FMT_NODE_PTR;
	...
	while (!page_cur_is_after_last(&cur1)) {
		...
		offsets1 = rec_get_offsets(cur1_rec, index, offsets1, format,
					   ULINT_UNDEFINED, &heap);
		while (!page_rec_is_supremum(cur_rec)) {
			...
			} else if (format != REC_FMT_NODE_PTR) {
...

{
	...
	while (!page_rec_is_supremum(rec)) {
		offsets = rec_get_offsets(
			rec, index, NULL, REC_FMT_NODE_PTR,
			ULINT_UNDEFINED, &heap);
