...
void log_t::close_file(bool really_close) noexcept
{
  ...
  else
  {
    ut_ad(!buf == !flush_buf);
#ifdef HAVE_PMEM
    ut_ad(!buf == !checkpoint_buf || disabled);
    if (checkpoint_buf)
#else
...
lsn_t log_t::skip_write_buf() noexcept
{
  ut_ad(log_sys.latch_have_wr());
...
lsn_t log_t::write_buf() noexcept
{
  ...
  ut_ad(resizing == RESIZING || !disabled);
...
bool log_t::flush(lsn_t lsn) noexcept
{
  ...
  const bool success{srv_file_flush_method == SRV_O_DSYNC ||
                     disabled || log.flush()};
