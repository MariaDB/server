...
@tparam resizing -1=skip latch.wr_unlock(); else, !!resize_in_progress()
@return the current log sequence number */
template<int resizing> inline __attribute__((always_inline))
lsn_t log_t::write_buf() noexcept
{
  ut_ad(latch_have_wr());
  ut_ad(!is_mmap());
  ut_ad(!srv_read_only_mode);
  ut_ad(resizing < 0 || bool(resizing) == (resize_in_progress() > 1));
  ...
  else
  {
    ...
    byte *const re_write_buf{resizing ? resize_buf : nullptr};
    ut_ad(resizing < 0 || bool(resizing) == !!re_write_buf);
...
#endif
      if (resizing < 0 ? UNIV_UNLIKELY(!!re_write_buf) : bool(resizing))
      {
...
template<bool resizing> static lsn_t log_writer() noexcept
{
  return log_sys.write_buf<resizing>();
}

void log_t::writer_update() noexcept
{
  ut_ad(latch_have_wr());
  writer= resize_in_progress() ? log_writer<true> : log_writer<false>;
...
#endif
  {
    const lsn_t lsn{log_sys.write_buf<-1>()};
