...
static size_t log_parse_length(byte*& l) noexcept
{
  if (size_t size= *l++ & 0xf)
    return size;

  const size_t lenlen{mlog_decode_varint_length(*l)};
  const size_t addlen{mlog_decode_varint(l)};
  ut_ad(addlen != MLOG_DECODE_ERROR);
  l+= lenlen;
  return addlen + 15 - lenlen;
}
...
class log_encrypt
{
  ...
  // FIXME: Add members to keep track of the last incomplete block
  // and the write-back locations.

  /** Finish the encryption */
  void finish() noexcept
  {
#if 0 // TODO: Encrypt any last block of data and write it back.
    ut_a(MY_AES_OK == encryption_ctx_update(ctx, tmp, len, dst, &tlen));
    ut_a(MY_AES_OK == encryption_ctx_finish(ctx, tmp, &len));
    ut_a(len == tlen);
#endif
  }
...
public:
  ...
  {
    // TODO: append to tmp, encrypt to dst, write back to buf now or later
    return buf + size;
  }
};
...
ATTRIBUTE_NOINLINE size_t mtr_t::encrypt() noexcept
{
  ...
  size_t rlen;
  void *ctx= alloca(encryption_ctx_size(LOG_DEFAULT_ENCRYPTION_KEY,
                                        info.key_version));
  {
    ...
    rlen= log_parse_length(buf);
    ut_ad(buf < end);
    ::memcpy(iv + 8, start, buf - start);
    ut_a(MY_AES_OK ==
         encryption_ctx_init(ctx, info.crypt_key, MY_AES_BLOCK_SIZE,
                             iv, MY_AES_BLOCK_SIZE,
                             ENCRYPTION_FLAG_ENCRYPT | ENCRYPTION_FLAG_NOPAD,
                             LOG_DEFAULT_ENCRYPTION_KEY, info.key_version));
  }
