...
static void buf_flush_wait(lsn_t lsn) noexcept
{
          ...
          8 * log_sys.is_encrypted() < lsn))
  {
    ...
    os_aio_wait_until_no_pending_writes(false);
    mysql_mutex_lock(&buf_pool.flush_list_mutex);
    if (buf_flush_sync_lsn && srv_shutdown_state >= SRV_SHUTDOWN_CLEANUP)
      my_cond_wait(&buf_pool.done_flush_list,
                   &buf_pool.flush_list_mutex.m_mutex);
  }
...
ATTRIBUTE_COLD void log_make_checkpoint() noexcept
{
  log_sys.latch.wr_lock(SRW_LOCK_CALL);
  mysql_mutex_lock(&buf_pool.flush_list_mutex);
  const lsn_t sync_lsn{log_sys.get_lsn()};
  log_sys.set_check_for_checkpoint();
  log_sys.latch.wr_unlock();
  buf_flush_wait_flushed(sync_lsn);
  mysql_mutex_unlock(&buf_pool.flush_list_mutex);
}
