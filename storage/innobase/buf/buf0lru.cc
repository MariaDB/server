...
/** Increasse LRU size in bytes by the page size.
	...
	for (bpage = UT_LIST_GET_LAST(buf_pool->flush_list);
		...
		if (observer && observer->is_partial_flush()) {
			...
			different tablespace. */
				   ...
				   bpage, observer != NULL, &must_restart)) {
		...
		if (buf_flush_try_yield(prev, processed, &must_restart)) {
			ut_ad(!must_restart);
	...
	buf_block_t*	block)
{
	...
	if (buf_get_withdraw_depth() && buf_block_will_be_withdrawn(block)) {
		/* This should be withdrawn */
