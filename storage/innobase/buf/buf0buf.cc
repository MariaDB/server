...
inline void buf_pool_t::garbage_collect() noexcept
{
  ...
  const size_t old_size{size_in_bytes}, min_size{size_in_bytes_min};
  if (first_to_withdraw || old_size != size_in_bytes_requested ||
      old_size < min_size + innodb_buffer_pool_extent_size)
  {
  ...
  size_t size= old_size - innodb_buffer_pool_extent_size;
  ...
  do
  {
    ...
    if (s == SHRINK_DONE)
    {
      ...
      os_total_large_mem_allocated-= innodb_buffer_pool_extent_size;
      MEM_NOACCESS(memory + size, innodb_buffer_pool_extent_size);
      madvise(memory + size, innodb_buffer_pool_extent_size, MADV_FREE);
#ifdef UNIV_PFS_MEMORY
      PSI_MEMORY_CALL(memory_free)(mem_key_buf_buf_pool,
                                   innodb_buffer_pool_extent_size, owner);
#endif
      mysql_mutex_unlock(&mutex);
...
#endif
      mysql_mutex_lock(&mutex);
      const bool resized= size_in_bytes == old_size;
      if (resized)
      {
        size_in_bytes_requested= size;
        size_in_bytes= size;
