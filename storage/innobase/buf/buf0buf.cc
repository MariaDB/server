...
void buf_pool_t::scratch_buffer::create(ulint n_slots) noexcept
{
  mutex.init();
...
buf_pool_t::scratch_buffer::grow(chunk *c) noexcept
{
  ...
  if (!next)
  {
    ...
    next->io_buf_t::create(std::min<ulint>(c->n_slots * 2, 16 * 1024));
...
ATTRIBUTE_COLD void buf_pool_t::scratch_buffer::shrink(bool all) noexcept
{
  for (chunk *c= &first; c; c= c->next.load(std::memory_order_acquire))
  {
    ...
    for (buf_tmp_buffer_t *s= c->slots, *e= s + c->n_slots; s != e; s++)
      if (s->acquire())
      {
        const bool used= s->test_used();
        if (s->frame() && (all || !used))
        {
          s->free_frame();
          /* Both the increment and the decrement happen while the slot is
          reserved, so the count is exact. A relaxed load is enough for the
          assertion: the increment for this frame happens before this thread
          acquired this slot, and only the holder of a slot decrements, so
          the value that this thread reads still includes this frame. */
          ut_ad(c->n_frames.load(std::memory_order_relaxed));
