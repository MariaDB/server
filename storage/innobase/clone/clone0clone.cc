...
int Clone_Sys::find_free_index(Clone_Handle_Type hdl_type, uint &free_index) {
  ...
  auto err = Clone_Sys::wait(
      ...
      [&](bool alert, bool &result) {
        ...
        if (!result) {
          ib::info() << "Clone Master aborted idle task";
  ...
  if (err == 0 && is_timeout) {
    ib::info() << "Clone Master wait for abort timed out";
...
bool Clone_Sys::mark_abort(bool force) {
  ...
  if (active_clone) {
    ...
    if (is_timeout) {
      ib::warn() << "DDL wait for CLONE abort timed out"
                                       ", Continuing DDL.";
      ut_d(ut_error);
    }
  }
...
void Clone_Task_Manager::debug_wait(uint chunk_num, Clone_Task *task) {
  ...
  DBUG_EXECUTE_IF("local_release_clone_file_pin", {
    ...
    ib::info() << "Clone debug close donor master file";
  });
...
int Clone_Task_Manager::add_task(THD *thd, const byte *ref_loc, uint loc_len,
  ...
  if (wait_before_add(ref_loc, loc_len)) {
    ...
    err = Clone_Sys::wait_default(
        [&](bool alert, bool &result) {
          ...
          if (err == 0 && result && alert) {
            ...
            if (++alert_count == 12) {
              alert_count = 0;
              ib::info() << "Clone Add task waiting "
                                               "for state change";
            }
          }
          return (err);
    ...
    if (err != 0) {
      ...
      ib::info() << "Clone Add task timed out";
...
bool Clone_Task_Manager::drop_task(THD *thd, uint task_id, bool &is_master) {
  ...
  if (m_num_tasks > 0) {
    ...
    auto err = Clone_Sys::wait_default(
        [&](bool alert, bool &result) {
          ...
          if (alert && result) {
            ...
            if (++alert_count == 12) {
              alert_count = 0;
              ib::info() << "Clone Master drop task waiting "
                                               "for other tasks";
            }
          }
          return (0);
    ...
    if (err != 0) {
      ...
      ib::info() << "Clone Master drop task timed out";
...
void Clone_Task_Manager::reinit_apply_state(const byte *ref_loc, uint ref_len,
  ...
  switch (m_current_state) {
    case CLONE_SNAPSHOT_INIT:
      ib::info() << "Clone Apply Restarting State: INIT";
      break;
    ...
    case CLONE_SNAPSHOT_DONE:
      ib::info() << "Clone Apply Restarting State: DONE";
      break;
...
void Clone_Task_Manager::reinit_copy_state(const byte *loc, uint loc_len) {
  ...
  switch (m_current_state) {
    case CLONE_SNAPSHOT_INIT:
      ib::info() << "Clone Restarting State: INIT";
      break;

    case CLONE_SNAPSHOT_FILE_COPY:
      ib::info() << "Clone Restarting State: FILE COPY";
      break;

    case CLONE_SNAPSHOT_PAGE_COPY:
      ib::info() << "Clone Restarting State: PAGE COPY";
      break;

    case CLONE_SNAPSHOT_REDO_COPY:
      ib::info() << "Clone Restarting State: REDO COPY";
      break;

    case CLONE_SNAPSHOT_DONE:
      ib::info() << "Clone Restarting State: DONE";
      break;
...
int Clone_Task_Manager::wait_ack(Clone_Handle *clone, Clone_Task *task,
  ...
  if (m_current_state != m_ack_state) {
    ...
    /* Wait too long */
    if (err == 0 && is_timeout) {
      ib::info() << "Clone Master wait for state change ACK"
                                       " timed out";
  ...
  if (err == 0) {
    ib::info() << "Clone Master received state change ACK";
  }
...
int Clone_Task_Manager::finish_state(Clone_Task *task) {
  ...
  if (m_num_tasks_finished < m_num_tasks) {
    ...
    if (err == 0 && is_timeout) {
      ib::info() << "Clone Apply Master wait timed out";
    ...
    err = Clone_Sys::wait_default(
        [&](bool alert, bool &result) {
          ...
          if (err == 0 && result && alert) {
            ...
            if (++alert_count == 12) {
              alert_count = 0;
              ib::info() << "Clone: master state change "
                                               "waiting for workers";
            }
            if (is_copy_clone()) {
