  ...
  const rec_offs *offsets)
{
  ...
  ulint doc_col_pos= dict_col_get_clust_pos(
  ...
  doc_id_t rec_doc_id= fts_read_doc_id(doc_id_data);
...
cleanup:
  ...
  {
    sql_print_error("InnoDB: Could not find 'value' column in "
		    "stopword table %s", stopword_table);
    goto cleanup;
  }
                             ...
                             const rec_offs* offsets) -> dberr_t
  {
    ...
    if (offsets == nullptr)
      offsets= rec_get_offsets(rec, index, nullptr, index->n_core_fields,
    ...
    if (field_len == UNIV_SQL_NULL) return DB_SUCCESS;
...
func_exit:
    ...
    return DB_SUCCESS; /* Continue processing */
  };

  RecordCallback callback(process_stopword,
                          [](const dtuple_t*, const rec_t*,
  ...
  for (;;)
  {
    dberr_t error= executor->read_by_index(
      table, clust_index, nullptr, PAGE_CUR_G, callback);
    ...
    else
    {
      if (error == DB_LOCK_WAIT_TIMEOUT)
      {
        sql_print_warning("InnoDB: Lock wait timeout reading "
                          "user stopword table. Retrying!");
        trx->error_state= DB_SUCCESS;
      }
      else
      {
        sql_print_error("InnoDB: Error '%s' while reading user "
                        "stopword table.", ut_strerr(error));
        ret= false;
        break;
      }
    }
                         ...
                         bool skip_doc_id_index)
{
  ...
  for (ulint i= 0; fts_common_tables[i]; ++i)
  {
    ...
    if (i == 2)
    {
      ut_ad(!strcmp(fts_common_tables[2], "CONFIG"));
      executor.set_config_table(common_table);
    }
...
static
dberr_t fts_read_synced_doc_id(FTSQueryExecutor *executor,
                               const dict_table_t *table,
	...
	doc_id_t           *doc_id) noexcept
{
	...
	dberr_t 	error = DB_SUCCESS;
retry:
	error = fts_read_synced_doc_id(executor, table, doc_id);
	...
	if (cmp_doc_id && cmp_doc_id >= *doc_id) {
		error = fts_update_sync_doc_id(
			executor, table, cache->synced_doc_id);
	}
                       ...
                       doc_id_t doc_id) noexcept
{
  ...
  return executor->update_config_record("synced_doc_id", id);
}
...
/** Apply one FTS table's row changes (inserts/modifies/deletes)
against the shared internal FTS transaction supplied by the caller.
The caller owns the trx lifecycle
@param ftt FTS table to commit
@param trx shared internal trx
@return DB_SUCCESS or error code */
static MY_ATTRIBUTE((nonnull, warn_unused_result))
dberr_t
fts_commit_table(
...
/** The given user transaction is about to be committed;
do whatever is necessary from the FTS system's POV.

A single internal transaction is shared across every FTS table
touched by the user trx, instead of one per table/

Rationale: each fts_commit_table() used to trx_create()+commit,
producing N undo segments, N read views and N group-commit batches
for N FTS tables.  Sharing one trx collapses that into one commit.
Lock accumulation across aux tables is intentional - a user
statement that writes to multiple FTS aux tables is logically one
operation.
@return DB_SUCCESS or error code */
dberr_t
fts_commit(
/*=======*/
	trx_t*	trx)				/*!< in: user transaction */
{
	fts_savepoint_t* savepoint = static_cast<fts_savepoint_t*>(
	...
	fts_trx->op_info = "FTS commit";
	trx_start_internal(fts_trx);

	dberr_t error = DB_SUCCESS;
	for (const ib_rbt_node_t* node = rbt_first(tables);
	     node != NULL && error == DB_SUCCESS;
	     node = rbt_next(tables, node)) {

		fts_trx_table_t** ftt = rbt_value(fts_trx_table_t*, node);
		error = fts_commit_table(*ftt, fts_trx);
	}

	fts_sql_commit(fts_trx);
       ...
       const dtuple_t* tuple)
{
       ...
       if (!ftt->table->fts->added_synced) {
               fts_init_index(ftt->table, FALSE,
                              ftt->fts_trx->trx->mysql_thd);
       }

       mtr_start(&mtr);

       FTSQueryExecutor executor(ftt->fts_trx->trx, ftt->table);
       ...
       for (ulint i = 0; i < num_idx; ++i) {
               ...
               if (doc.found) {
                       ...
                       if (table->fts->cache->stopword_info.status
                           & STOPWORD_NOT_INIT) {
                               fts_load_stopword(&executor, table);
                       }
	...
	doc_id_t	doc_id)		/*!< in: doc id */
{
	...
	FTSQueryExecutor executor(ftt->fts_trx->trx, table);
                           ...
                           ib_vector_t *doc_ids) noexcept
{
  ...
  dberr_t error= DB_SUCCESS;
  for (ulint i= 0; i < n_elems && error == DB_SUCCESS; ++i)
  {
    doc_id_t *update=
...
/** Run SYNC on the table, i.e., write out data from the index
specific cache to the FTS aux INDEX table and FTS aux doc id
stats table.
@param	executor	query executor
@param	sync		sync state
@param	index_cache	index cache
@return DB_SUCCESS if all OK */
static MY_ATTRIBUTE((nonnull, warn_unused_result))
dberr_t
	       ...
	       fts_index_cache_t *index_cache) noexcept
{
  ...
  ut_ad(rbt_validate(index_cache->words));
  return fts_sync_write_words(executor, index_cache, sync->unlock_cache);
}
...
begin_sync:
	...
	for (i = 0; i < ib_vector_size(cache->indexes); ++i) {
		...
		error= executor.open_all_aux_tables(index_cache->index);

		if (error) goto end_sync;
		DBUG_EXECUTE_IF("fts_instrument_sync_before_syncing",
				std::this_thread::sleep_for(
					std::chrono::milliseconds(300)););
		error = fts_sync_index(&executor, sync, index_cache);
...
dberr_t fts_sync_table(dict_table_t* table, bool wait, THD* thd)
{
  ...
  return table->space && !table->corrupted && table->fts->cache
    ? fts_sync(table->fts->cache->sync, !wait, wait, thd)
    : DB_SUCCESS;
}
...
fts_load_stopword(
/*==============*/
	FTSQueryExecutor* executor,	  /*!< in: FTSQueryExecutor */
	const dict_table_t* table) noexcept /*!< in: Table with FTS */
{
	fts_cache_t*	cache = table->fts->cache;
	fts_string_t	str;
	dberr_t		error = DB_SUCCESS;
	...
	/* If stopword is already initialized, return success */
	if (!(cache->stopword_info.status & STOPWORD_NOT_INIT)) {
		return true;
	}

	/* First check if stopwords are enabled */
	ulint use_stopword= 0;
	fts_string_t value;
	value.f_len= FTS_MAX_CONFIG_VALUE_LEN;
	value.f_str= static_cast<byte*>(ut_malloc_nokey(value.f_len + 1));
	error= fts_config_get_value(executor, table, FTS_USE_STOPWORD, &value);
	if (error == DB_SUCCESS) {
		use_stopword= strtoul(reinterpret_cast<char*>(value.f_str),
		                      nullptr, 10);
	}
	...
	if (!use_stopword) {
		/* Stopwords are disabled, mark as initialized but off */
		cache->stopword_info.status = STOPWORD_OFF;
		return true;
	}

	/* Read FTS_STOPWORD_TABLE_NAME from CONFIG table */
	str.f_n_char = 0;
	str.f_str = str_buffer;
	str.f_len = sizeof(str_buffer) - 1;

	error = fts_config_get_value(executor, table, FTS_STOPWORD_TABLE_NAME, &str);

	if (error == DB_SUCCESS && *str.f_str) {
		stopword_to_use = (const char*) str.f_str;
	}

	/* Load user stopword table if specified, otherwise load default */
	if (stopword_to_use
	    && fts_load_user_stopword(executor, table->fts, stopword_to_use,
		      &cache->stopword_info)) {
		/* Successfully loaded user stopword table */
	} else {
		/* Load system default stopword list */
		fts_load_default_stopword(&cache->stopword_info);
	}

	/* Initialize cached_stopword RB-tree if not already created */
	if (!cache->stopword_info.cached_stopword) {
...
static void fts_init_recover_all_docs(FTSQueryExecutor *executor,
                                      fts_get_doc_t *get_doc,
                                      doc_id_t doc_id) noexcept
{
  ...
  executor->read_by_index(user_table, fts_doc_id_index, &search_tuple,
                          PAGE_CUR_G, reader);
}
...
@param index  fulltext index */
static void fts_init_get_doc_id(FTSQueryExecutor *executor,
                                doc_id_t doc_id, dict_index_t *index) noexcept
{
  ...
  executor->read_by_index(user_table, index, &search_tuple, PAGE_CUR_G,
                          reader);
}
	...
	THD*		thd)		/*!< in: caller's THD; may be NULL */
{
	...
	/* Declare variables before any goto to avoid initialization bypass */
	trx_t *trx = nullptr;
	...
	if (!start_doc) {
		dberr_t err= fts_read_synced_doc_id(
				executor, table, &start_doc);
		if (err != DB_SUCCESS) {
			fts_sql_commit(trx);
			trx->free();
			delete executor;
	...
	if (ib_vector_is_empty(cache->get_docs)) {
		...
		fts_init_get_doc_id(executor, start_doc, index);
	} else {
		if (table->fts->cache->stopword_info.status
		    & STOPWORD_NOT_INIT) {
			fts_load_stopword(executor, table);
		...
		for (ulint i = 0; i < ib_vector_size(cache->get_docs); ++i) {
			...
			fts_init_recover_all_docs(executor, get_doc, start_doc);
		}
	}
