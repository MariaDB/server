...
static
int fts_cmp_rec_dtuple_prefix(const dtuple_t *dtuple, const rec_t *rec,
                              const dict_index_t *index,
...
rec_loop:
  ...
  if (m_tuple)
  {
    ...
    /* Searching tuple field can be prefix of the record. So compare
    the tuple field data with record only till tuple field length */
    if (match_op == MATCH_PREFIX &&
        fts_cmp_rec_dtuple_prefix(m_tuple, rec, index, offsets))
    {
      err= DB_RECORD_NOT_FOUND;
      goto func_exit;
    }
