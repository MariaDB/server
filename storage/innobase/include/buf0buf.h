...
class buf_tmp_buffer_t
{
  ...
  /** Whether this slot was reserved since the last test_used(). */
...
public:
  ...
  /** Test if the slot was reserved since the previous call, resetting
  the used bit.
  @return Whether the slot was reserved since the previous call. */
  bool test_used() noexcept
...
# error "BUF_BUDDY_LOW > UNIV_ZIP_SIZE_MIN"
  ...
  /** Free the page frames of the scratch buffer slots that are not in use.
  @param all Whether to free also the frames that were used recently. */
  ATTRIBUTE_COLD void scratch_buf_shrink(bool all) noexcept
  { scratch_buf.shrink(all); }
