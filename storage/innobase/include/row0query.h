...
private:
  ...
  @param     sec_offsets	secondary index offsets
  @param     callback           callback to process the clustered record
  @param     match_count        counter for processed records
  @return DB_SUCCESS to continue, DB_SUCCESS_LOCKED_REC to stop, or error code */
  dberr_t lookup_clustered_record(dict_index_t *sec_index,
                                  const rec_t *sec_rec,
  ...
  version building
  @param     clust_index        clustered index of the record
  @param     rec                record to process
  @param     callback           callback to process the record
  @return DB_SUCCESS to continue, DB_SUCCESS_LOCKED_REC to stop, or error code */
  dberr_t process_record_with_mvcc(
