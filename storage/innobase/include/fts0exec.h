...
private:
  ...
  /** Create search tuple for a word
  @param table   tuple to be created for this table
  @param word    word to create tuple for
  @param  heap   memory heap
  @return search tuple or nullptr if word is empty */
  dtuple_t* create_word_search_tuple(
...
public:
  ...
  Each call acquires a reference, which the destructor releases.  A
  second set_config_table() would leak a reference (and leave the
  caller's pointer dangling at dtor time), so callers must ensure they
  set it at most once on a given executor.
  ...
  @return DB_SUCCESS or error code */
  dberr_t read_by_index(dict_table_t *table, dict_index_t *index,
                        const dtuple_t *search_tuple, page_cur_mode_t mode,
                        RecordCallback& callback) noexcept
  { 
    dberr_t err= m_executor.read_by_index(table, index, search_tuple,
                                          mode, callback, true);
  ...
  dberr_t read_aux(uint8_t aux_index,
...
{
private:
  /** Heap owning m_doc_ids when this reader allocates
  its own vector; */
  mem_heap_t *m_heap;
...
public:
  ...
  /** Push doc_ids directly into a caller-supplied ib_vector
  @param dest  destination vector to append doc_ids to */
  explicit CommonTableReader(ib_vector_t *dest);

  ~CommonTableReader();

  /** @return number of doc_ids collected so far */
  ulint size() const { return ib_vector_size(m_doc_ids); }

  /** @param n  element index
  @return the n'th collected doc_id */
  doc_id_t get(ulint n) const
  { return *static_cast<const doc_id_t*>(ib_vector_get_const(m_doc_ids, n)); }

  /** Discard collected doc_ids so the reader can be reused. */
  void clear() { ib_vector_reset(m_doc_ids); }
...
private:
  ...
  /* Optional shared heap. When it is allocated, every fts_word_t
  allocated via default_word_processor stores its text, heap_alloc,
  nodes vector in this words heap instead of private mem_heap_t
  per word. It is used for information schema read query */
  mem_heap_t* m_words_heap= nullptr;
...
private:
  ...
  RecordComparator make_comparator()
  {
    return [this](const dtuple_t* search_tuple, const rec_t* rec,
                  const dict_index_t* index, const rec_offs* offsets)
           -> RecordCompareAction
    {
...
public:
  ...
  void set_words_heap(mem_heap_t *words_heap) noexcept
  { m_words_heap= words_heap; }

  mem_heap_t *words_heap() const noexcept { return m_words_heap; }
};
