  ...
  public:
    struct trx_rseg_cmp
    {
      ...
      bool operator()(const byte lhs, const byte rhs)
      {
        ...
        return trx_sys.rseg_array[lhs].last_trx_no() >
               trx_sys.rseg_array[rhs].last_trx_no();
      }
    };
    ...
    {
      ut_ad(!empty());
      byte index= m_array[0];
      ut_ad(index < TRX_SYS_N_RSEGS);
      std::pop_heap(begin(), end(), trx_rseg_cmp());
      --m_end;
      return &trx_sys.rseg_array[index];
    }
  };
