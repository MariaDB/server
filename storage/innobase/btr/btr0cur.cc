				...
				successor record */
{
	...
	if (!(flags & BTR_NO_UNDO_LOG_FLAG)) {
		/* For the instant ALTER TABLE metadata record, write the
		undo log record in the same mini-transaction that is
		going to insert the record. */
		if (dberr_t err = trx_undo_report_row_operation(
			    thr, index, entry, NULL, 0, NULL, NULL,
			    &roll_ptr,
			    entry->is_metadata() ? mtr : nullptr)) {
			return err;
		}
	...
	roll_ptr_t*	roll_ptr)/*!< out: roll pointer */
{
	...
	For the instant ALTER TABLE metadata record, write the undo log
	record in the same mini-transaction that is going to modify the
	record. */
	...
	return((flags & BTR_NO_UNDO_LOG_FLAG)
	       ...
	       : trx_undo_report_row_operation(
		       ...
		       rec_is_metadata(rec, *index) ? mtr : nullptr));
}
				...
				committed before latching any further pages */
{
	...
	*big_rec = NULL;
	...
	if (UNIV_UNLIKELY(is_metadata)) {
		/* The undo log record was written into this
		mini-transaction, which has not been committed yet.
		Allow tests to kill the server here. */
#ifndef DBUG_OFF
		log_buffer_flush_to_disk();
#endif
		DEBUG_SYNC_C("btr_metadata_undo_logged");
	}
...
return_after_reservations:
	...
	if (UNIV_UNLIKELY(metadata_big != NULL)) {
		/* The metadata record was written above, and it holds
		the BLOB pointers of the off-page columns that
		btr_store_big_rec_metadata() wrote. Only free the
		vector: freeing the pages would leave the record
		pointing to freed pages.

		On the DB_CORRUPTION path where page_cur_move_to_prev()
		failed, the record was deleted and not re-inserted, and
		the pages are merely orphaned. */
		dtuple_big_rec_free(metadata_big);
	}
	...
	mtr_t*		mtr)	/*!< in/out: mini-transaction */
{
	...
	/* A metadata pseudo-record is never delete-marked by DML. */
