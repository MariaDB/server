...
dberr_t i_s_fts_index_table_fill_selected(
  ...
  uint8_t selected, fts_string_t *word) noexcept
{
  DBUG_EXECUTE_IF("fts_instrument_result_cache_limit",
                  fts_result_cache_limit = 8192;);

  dberr_t error= executor->read_aux(selected, word, reader);
	...
	ib_vector_t*		words)		/*!< in: words fetched */
{
	for (ulint i = 0; i < ib_vector_size(words); i++) {
		...
		/* caller releases every word via
		mem_heap_empty(words_heap). */
	}
      ...
      ib_vector_t *words) noexcept
{
  ...
  do
  {
    ...
    for (;;)
    {
      ...
      else
      {
        ...
        if (error == DB_LOCK_WAIT_TIMEOUT)
        {
          ...
          if (reader.words_heap()) mem_heap_empty(reader.words_heap());
        ...
        else
        {
          ...
          i_s_fts_index_table_free_one_fetch(words);
          if (reader.words_heap())
            mem_heap_empty(reader.words_heap());
          return 1;
        }
      } 
    }
    ...
    if (has_more)
    {
      ...
      fts_word_t *last_word= static_cast<fts_word_t*>(ib_vector_last(words));
	...
	TABLE_LIST*		tables)		/*!< in/out: tables to fill */
{
	...
	/* Dedicated words heap for fts_word_t internals
	(text, nodes vector, heap_alloc). Emptied per batch in
	i_s_fts_read_aux_index_words so peak memory stays bounded. */
	mem_heap_t *words_heap= mem_heap_create(4096);
	reader.set_words_heap(words_heap);
