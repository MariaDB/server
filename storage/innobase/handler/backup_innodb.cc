...
private:
  ...
  struct context
  {
    ...
    on error, set to 0 while holding InnoDB_backup::mutex */
    lsn_t last_lsn{};
      ...
      if (lsn == last)
      {
        if (!extend_log(ctx.log_dst, end))
        {
          ...
          if (!err)
          {
            ...
            if (!write_checkpoint(ctx.first_log_dst, cp_buf))
            {
              err= IF_WIN(!CloseHandle(ctx.first_log_dst),
...
void InnoDB_backup::context::destroy() noexcept
{
  ...
  if (innodb_backup.ctx.first_log_dst != OS_FILE_CLOSED)
  {
    IF_WIN(CloseHandle,close)(innodb_backup.ctx.first_log_dst);
    if (innodb_backup.ctx.log_dst != innodb_backup.ctx.first_log_dst)
