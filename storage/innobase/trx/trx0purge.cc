...
loop:
  ...
  err= flst_remove(rseg_hdr, TRX_RSEG + TRX_RSEG_HISTORY + rseg_hdr_page, b,
                   page + hdr_addr.boffset + TRX_UNDO_HISTORY_NODE,
                   last_page, &mtr);
...
bool purge_sys_t::choose_next_log()
{
  ...
  else
  {
    ...
    if (!b)
      goto purge_nothing;
    const byte *const hoffset= b->page.frame + hdr_offset;
    ...
    if (!undo_rec)
    {
      ...
      if (!b)
        goto purge_nothing;
      undo_rec=
        trx_undo_page_get_first_rec(b, hdr_page_no,
				    b->page.frame + hdr_offset);
      if (!undo_rec)
        goto purge_nothing;
    }

    ut_ad(b->page.frame == page_align(undo_rec));
...
inline trx_purge_rec_t purge_sys_t::get_next_rec(roll_ptr_t roll_ptr)
{
  ...
  ut_ad(rseg->latch.have_wr());
  ut_ad(uint16_t(roll_ptr) >= FIL_PAGE_DATA);
  ut_ad(uint16_t(roll_ptr) < srv_page_size - FIL_PAGE_DATA_END);
           ...
           !mach_read_from_2(frame + hdr_offset + TRX_UNDO_NEXT_LOG))
  {
    ...
    if (next != FIL_NULL)
    {
      ...
      if (buf_block_t *next_page= get_page(page_id))
      {
        ...
        {
          page_no= next;
          offset= uint16_t(rec2 - next_page->page.frame);
          goto got_rec;
        }
      }
    }
