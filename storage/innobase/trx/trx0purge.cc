...
void purge_sys_t::create()
{
  ...
  ut_ad(!enabled());
  ut_ad(!m_active);
  m_skipped_rseg= srv_undo_tablespaces > 0;
  m_paused= 0;
...
func_exit:
    ...
    {
      uint8_t skipped_rseg= purge_sys.m_skipped_rseg;
      ut_ad(skipped_rseg < TRX_SYS_N_RSEGS);
      if (&rseg == &trx_sys.rseg_array[skipped_rseg] &&
          purge_sys.truncate.current != rseg.space)
      {
        rseg.clear_skip_allocation();
        skipped_rseg++;
        if (!(skipped_rseg%= TRX_SYS_N_RSEGS) && srv_undo_tablespaces)
          skipped_rseg++;
        purge_sys.m_skipped_rseg= skipped_rseg;
      }
    }
    return err;
  }
...
/** Start truncating an undo tablespace.
@return undo tablespace that is chosen for truncation
@retval nullptr  if undo tablespace truncation is not enabled or possible */
fil_space_t *purge_sys_t::truncating_tablespace()
{
  ut_ad(this == &purge_sys);

  fil_space_t *space= truncate.current;
  if (space || srv_undo_tablespaces_active < 2 || !srv_undo_log_truncate)
    return space;

  const ulint threshold= ulint(srv_max_undo_log_size >> srv_page_size_shift);
  for (ulint i= truncate.last, j= i;; )
  {
    const ulint space_id= srv_undo_space_id_start + i;
...
not_free:
    ...
    purge_sys.truncate.last= space->id - srv_undo_space_id_start;
  }
}
