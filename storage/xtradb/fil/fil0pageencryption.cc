...
ulint* out_len /*!< out: actual length of encrypted page */
	...
	flush_lsn = ut_ull_create(mach_read_from_3(buf + FIL_PAGE_FILE_FLUSH_LSN),
			mach_read_from_2(buf + FIL_PAGE_FILE_FLUSH_LSN + 3));
	...
	checksum = fil_page_encryption_calc_checksum(buf + header_len,
			page_size - (FIL_PAGE_DATA_END + header_len));
	...
	err = my_aes_encrypt_cbc((char*) buf + header_len, data_size - 1,
			(char *) out_buf + header_len, &write_size, stringkey, key_len,
			stringiv, iv_len);
	;
	...
	err = my_aes_encrypt_cbc(
			(char*) out_buf + page_size - FIL_PAGE_DATA_END - 62, 63,
			(char*) tmp_buf, &write_size, stringkey, key_len, stringiv, iv_len);
	ut_ad(write_size == 64);
...
// If no buffer was given, we need to allocate temporal buffer
	...
	err = my_aes_decrypt_cbc((char*) tmp_page_buf + FIL_PAGE_DATA, data_size,
			(char *) in_buf + FIL_PAGE_DATA, &tmp_write_size, stringkey,
			key_len, stringiv, iv_len);
	ut_ad(tmp_write_size = data_size-1);
	memcpy(in_buf + FIL_PAGE_DATA + data_size -1,
			tmp_page_buf + page_size - FIL_PAGE_DATA_END - 2, remainder);

	/* calculate a checksum to verify decryption*/
	checksum = fil_page_encryption_calc_checksum(in_buf + header_len,
			page_size - (FIL_PAGE_DATA_END + header_len));
	/* compare with stored checksum */
	stored_checksum = mach_read_from_4(buf);
	...
	if (err != AES_OK) {
		fprintf(stderr, "InnoDB: Corruption: Page is marked as encrypted\n"
				"InnoDB: but decrypt failed with error %d.\n"
				"InnoDB: space %lu size %lu len %lu, key %d\n", err, space_id, data_size, len,
				page_encryption_key);
		fflush(stderr);
