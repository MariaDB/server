                               ...
                               uchar **rowdata, ulonglong *rowdata_length)
{
  ...
  *rowdata = (uchar *)my_malloc(PSI_NOT_INSTRUMENTED, *rowdata_length, 
                                MYF(MY_WME));
...
private:
  int cache_row(uchar *rowdata, ulong rowdata_length)
  {
    ...
    rows[last_row].data = (uchar *)my_malloc(PSI_NOT_INSTRUMENTED, 
                                             rowdata_length, MYF(MY_WME));
...
public:
  ...
  int initialize(bool *stmt_completed)
  {
    ...
    rows = (struct rowdata *)my_malloc(PSI_NOT_INSTRUMENTED,
                                       buffer_size * sizeof(struct rowdata),
                                       MYF(MY_WME));
    if (!rows)
      DBUG_RETURN(HA_ERR_OUT_OF_MEM);
...
int xpand_connection::expand_command_buffer(size_t add_length)
{
  ...
  {
    command_buffer = (uchar *) my_malloc(PSI_NOT_INSTRUMENTED,
                                         expanded_length, MYF(MY_WME));
  }
  else
  {
    command_buffer = (uchar *) my_realloc(PSI_NOT_INSTRUMENTED, 
                                          command_buffer, expanded_length,
                                          MYF(MY_WME));
  }
  if (!command_buffer)
...
{
  strtok_buf = my_strdup(PSI_NOT_INSTRUMENTED, hosts, MYF(MY_WME));
  if (!strtok_buf) {
    return HA_ERR_OUT_OF_MEM;
  }
