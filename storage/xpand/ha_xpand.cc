                         ...
                         void *var_ptr, const void *save)
{
  ...
  xpand_host_list *list = static_cast<xpand_host_list*>(
                          my_malloc(PSI_NOT_INSTRUMENTED, sizeof(xpand_host_list), 
  ...
  my_free(*display_var);
  *display_var = my_strdup(PSI_NOT_INSTRUMENTED, from_save, MYF(MY_WME));
...
int ha_xpand::external_lock(THD *thd, int lock_type)
{
  ...
  if (lock_type != F_UNLCK) {
    ...
    trans_register_ha(thd, FALSE, xpand_hton, 0);
    if (thd_test_options(thd, OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN))
      trans_register_ha(thd, TRUE, xpand_hton, 0);
  }
                                         ...
                                         TABLE *table)
{
  ...
  *_rgi = rgi;
  ...
  rgi->tables_to_lock = (RPL_TABLE_LIST *)my_malloc(PSI_INSTRUMENT_ME,
