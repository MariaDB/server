...
enum Aria_backup_status {
  ...
  BACKUP_FAIL,
  /** translog_lock() has been called */
  TRANSLOG_LOCKED,
  /** translog_lock() and translog_disable_purge() have been called */
                           ...
                           const struct backup_sink *sink)
{
  ...
  int left;
  const char *filename= NULL;
  char path[IF_WIN(FN_REFLEN, 0) + sizeof "aria_log.00000001"];
  pthread_mutex_lock(&ab->mutex);
#ifndef _WIN32
  assert(!ab->d);
  assert(ab->dir);
  ...
  else
  {
    ...
    while ((d= readdir(ab->dir)) != NULL)
    {
      ...
      if ((strncmp(d->d_name, C_STRING_WITH_LEN("aria_log.")) ||
           strlen(d->d_name) != (sizeof path) - 1) &&
          strcmp(d->d_name, "aria_log_control"))
        continue;
      compile_time_assert(sizeof "aria_log_control" <= sizeof path);
      filename= strcpy(path, d->d_name);
      break;
    }
    left= d != NULL;
#else
