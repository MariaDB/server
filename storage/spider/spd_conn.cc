...
SPIDER_CONN *spider_create_conn(
  ...
  if (unlikely(!UTC))
  {
    ...
    if (conn->tgt_odbc_conn_str_length)
    {
      conn->tgt_odbc_conn_str = tmp_odbc_conn_str;
      memcpy(conn->tgt_odbc_conn_str, share->tgt_odbc_conn_str,
        share->tgt_odbc_conn_str_length);
    } else
      conn->tgt_odbc_conn_str = NULL;
