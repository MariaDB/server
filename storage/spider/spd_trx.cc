    ...
    if (
      ...
      if (spider_param_internal_xa_id_type(thd) == 0)
      {
        trx->xid.gtrid_length
          = my_snprintf(trx->xid.data, sizeof(trx->xid.data),
                        "%lx", thd_get_thread_id(thd));
      } else {
        trx->xid.gtrid_length
          = my_snprintf(trx->xid.data,
                        sizeof(trx->xid.data), "%lx%016llx",
                        thd_get_thread_id(thd),
                        thd->query_id);
      }
      trx->xid.bqual_length
        = my_snprintf(trx->xid.data + trx->xid.gtrid_length,
