...
int spider_create_conn_keys(
  ...
  for (roop_count = 0; roop_count < (int) share->all_link_count; roop_count++)
  {
    ...
    *tmp_name = '0';
    ...
    my_snprintf(port_str, sizeof(port_str), "%05ld", share->tgt_ports[roop_count]);
...
SPIDER_SHARE *spider_create_share(
  ...
  for (roop_count = 0; roop_count < (int) share->all_link_count;
    roop_count++)
  {
    my_snprintf(link_idx_str, sizeof(link_idx_str), "%010d", roop_count);
...
int spider_db_init(
  ...
  spider_unique_id.length = my_snprintf(spider_unique_id_buf,
