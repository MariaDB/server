...
void spider_db_append_xid_str(
  ...
  format_id_length =
    my_snprintf(format_id, sizeof(format_id), "%lu", xid->formatID);
...
int spider_db_append_key_columns(
  ...
  for (
    ...
    key_name_length = my_snprintf(tmp_buf, sizeof(tmp_buf), "c%u",
                                  key_count);
...
*/
  ...
  for (
    ...
    if ((error_num = dbton_hdl->set_sql_for_exec(sql_type, roop_count)))
    {
      ...
      spider_setup_before_query(conn, &spider->need_mons[roop_count]);
      if ((error_num = spider_db_set_names(spider, conn, roop_count)))
      {
        spider_unlock_after_query(conn, 0);
        if (
...
int spider_db_udf_ping_table_append_mon_next(
  ...
  limit_str_length = my_snprintf(limit_str, sizeof(limit_str), "%lld", limit);
  sid_str_length = my_snprintf(sid_str, sizeof(sid_str), "%lld", first_sid);
  if (str->reserve(
...
int spider_db_udf_ping_table_append_select(
  ...
  if (spider_dbton[dbton_id].db_util->limit_mode() == 1)
  {
    ...
    limit_str_length = my_snprintf(limit_str, sizeof(limit_str),
                                   "%lld", limit);
    if (str->reserve(
...
int spider_db_open_handler(
  ...
  if (!spider->handler_opened(link_idx))
    my_snprintf(spider->m_handler_cid[link_idx],
                SPIDER_SQL_HANDLER_CID_LEN + 1,
