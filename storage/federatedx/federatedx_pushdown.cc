...
create_federatedx_multi_upddel_handler(THD *thd, SELECT_LEX *sel_lex)
{
  ...
  */
  ...
  return new ha_federatedx_multi_upddel_handler(thd, sel_lex, tbl);
}
...
ha_federatedx_multi_upddel_handler::ha_federatedx_multi_upddel_handler(
    ...
    : multi_upddel_handler(thd, federatedx_hton, select_lex),
      federatedx_handler_base(thd, tbl)
{
  query.length(0);
  /*
  ...
  */
  select_lex->master_unit()->print(&query,
                                   FEDERATEDX_PRINT_UPD_DEL_QUERY_TYPE);
}
