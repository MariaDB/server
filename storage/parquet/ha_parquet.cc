...
int ha_parquet::create(const char *name, TABLE *table_arg, HA_CREATE_INFO *create_info)
{
  ...
  try {
    ...
    if (!create_result || create_result->HasError()) {
      std::cerr << "DuckDB CREATE error: " << (create_result ? create_result->GetError() : "null result") << std::endl;
      //delete con;
    ...
    if (!copy_result || copy_result->HasError()) {
      std::cerr << "DuckDB COPY error: " << (copy_result ? copy_result->GetError() : "null result") << std::endl;
      //delete con;
      return HA_ERR_INTERNAL_ERROR;
    }
...
void ha_parquet::flush_remaining_rows_to_s3(parquet_trx_data *trx)
{
  ...
  try {
    ...
    if (!copy_result || copy_result->HasError()) {
      std::cerr << "flush COPY error: " << (copy_result ? copy_result->GetError() : "null result") << std::endl;
      //delete con;
      return;
    }
...
int ha_parquet::rnd_init(bool scan)
{
  ...
  std::string current_snapshot_file = s3_files.back();
...
static int ha_parquet_commit(THD *thd, bool all)
{
 ...
 for (size_t i = 0; i < trx->s3_file_paths.size(); i++) {
   ...
   snprintf(lakekeeper_url, sizeof(lakekeeper_url),
     "http://localhost:8181/catalog/v1/"
     "fe89d40e-3472-11f1-8805-6fc69e665327/transactions/commit");
   ...
   snprintf(json_body, sizeof(json_body),
     "{"
       "\"table-changes\": [{"
         "\"identifier\": {\"namespace\": [\"default\"], \"name\": \"%s\"},"
         "\"requirements\": [],"
         "\"updates\": ["
...
static int ha_parquet_rollback(THD *thd, bool all)
{
 ...
 for (const std::string &s3_path : trx->s3_file_paths)
   std::cerr << "rollback: orphaned file will be GC'd: " << s3_path << std::endl;
