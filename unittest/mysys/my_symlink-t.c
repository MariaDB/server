...
static void test_win_realpath_and_nosymlinks()
{
  ...
  {
    char link_file[FN_REFLEN];
    my_snprintf(link_file, sizeof(link_file), "%s\\link.txt", realdir);
    if (!CreateSymbolicLink(link_file, file_via_real,
                             SYMBOLIC_LINK_FLAG_ALLOW_UNPRIVILEGED_CREATE) &&
        !CreateSymbolicLink(link_file, file_via_real, 0))
    {
      diag("CreateSymbolicLink() failed: %lu (needs Developer Mode or "
           "admin)", GetLastError());
      skip(2, "CreateSymbolicLink() failed");
    }
    else
    {
      err= my_delete(link_file, MYF(MY_NOSYMLINKS));
      ok(err != 0 && my_errno == ENOTDIR,
         "my_delete(MY_NOSYMLINKS) rejects a name whose leaf is itself a "
         "symlink: err=%d my_errno=%d", err, my_errno);
      ok(GetFileAttributes(file_via_real) != INVALID_FILE_ATTRIBUTES,
         "...and doesn't delete the symlink's target");
      my_delete(link_file, MYF(0));
    }
  }
