...
useradd -M -r --home / --shell /sbin/nologin --comment "MariaDB server" --gid %{mysqld_group} %{mysqld_user} 2> /dev/null || true
# The user may already exist, make sure it has the proper group nevertheless (BUG#12823)
usermod --gid %{mysqld_group} %{mysqld_user} 2> /dev/null || true
# Make sure the home dir is correct too (existing but not writable by mysql user)
usermod -d / %{mysqld_user} 2> /dev/null || true
