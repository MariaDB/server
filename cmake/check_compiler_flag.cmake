...
FUNCTION(MY_CHECK_AND_SET_COMPILER_FLAG_C flag_to_set)
  ...
  IF (have_C_${result})
    IF(ARGN)
      FOREACH(type ${ARGN})
        SET(CMAKE_C_FLAGS_${type} "${CMAKE_C_FLAGS_${type}} ${flag_to_set}" PARENT_SCOPE)
      ENDFOREACH()
    ELSE()
      SET(CMAKE_C_FLAGS "${CMAKE_C_FLAGS} ${flag_to_set}" PARENT_SCOPE)
    ENDIF()
  ENDIF()
ENDFUNCTION()
...
FUNCTION(MY_CHECK_AND_SET_COMPILER_FLAG_CXX flag_to_set)
  ...
  IF (have_CXX_${result})
    IF(ARGN)
      FOREACH(type ${ARGN})
        SET(CMAKE_CXX_FLAGS_${type} "${CMAKE_CXX_FLAGS_${type}} ${flag_to_set}" PARENT_SCOPE)
      ENDFOREACH()
    ELSE()
      SET(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} ${flag_to_set}" PARENT_SCOPE)
    ENDIF()
  ENDIF()
ENDFUNCTION()
