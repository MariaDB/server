...
FUNCTION(INSTALL_RUNTIME_DEPS)
  ...
  FOREACH(tgt ${installed_targets})
    SET(exclude_libs)
    SET(libs_seen)
    GET_TARGET_PROPERTY(libs_to_check ${tgt} LINK_LIBRARIES)
    IF(NOT libs_to_check)
      SET(libs_to_check)
    ENDIF()
    WHILE(libs_to_check)
      LIST(POP_FRONT libs_to_check lib)
      IF(NOT TARGET ${lib} OR lib IN_LIST libs_seen)
        CONTINUE()
      ENDIF()
      LIST(APPEND libs_seen ${lib})
      GET_TARGET_PROPERTY(type ${lib} TYPE)
      IF(type MATCHES "SHARED")
        LIST(APPEND exclude_libs "$<TARGET_FILE_BASE_NAME:${lib}>\\.dll")
      ENDIF()
      # an INTERFACE library (e.g. mariadb_private) may itself link a
      # shared target - follow it too, or its dependency gets missed
      GET_TARGET_PROPERTY(iface_libs ${lib} INTERFACE_LINK_LIBRARIES)
      IF(iface_libs)
        LIST(APPEND libs_to_check ${iface_libs})
      ENDIF()
    ENDWHILE()
