...
/*
  Give a file handle from the operating system a mysys file
  descriptor. my_detach_file() frees the descriptor without closing
  the handle, which is what one has to use for a handle that mysys
  does not own. On Posix a file handle is already a file descriptor,
  so nothing has to be done.
*/
extern File my_convert_handle_to_file(my_native_file handle, int oflag);
extern void my_detach_file(File fd);
#else
#define my_native_file_handle(fd) (fd)
#define my_convert_handle_to_file(handle, oflag) (handle)
#define my_detach_file(fd) do { } while (0)
#endif
