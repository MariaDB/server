...
/**
   @defgroup ftparser_plugin_data Full-text Parser Plugin
   @ingroup plugin_types
   API for Full-text parser plugin. (@ref MYSQL_FTPARSER_PLUGIN)
   @{
*/
#ifdef __cplusplus
extern "C" {
...
{
/**
  Fast and simple mode.
  
  This mode is used for indexing, and natural
  language queries.
...
/**
  Token types for boolean mode searching
  
  Used for the type member of MYSQL_FTPARSER_BOOLEAN_INFO struct.
*/

enum enum_ft_token_type
{
  FT_TOKEN_EOF= 0, /**< End of data. */
  FT_TOKEN_WORD= 1, /**< Regular word. */
  FT_TOKEN_LEFT_PAREN= 2, /**< Left parenthesis (start of group/sub-expression). */
  FT_TOKEN_RIGHT_PAREN= 3, /**< Right parenthesis (end of group/sub-expression). */
  FT_TOKEN_STOPWORD= 4 /**< Stopword. */
};

/**
  Structure for boolean mode search.
...
{
  /** The token type.  Should be one of the enum_ft_token_type values. */
  enum enum_ft_token_type type;
  /**
    Whether the word must be present for a match to occur
  ...
  */
  ...
  /**
    The sign of the word's weight in the query.
  ...
  */
  char wasign;
  /**
    Corresponds to the `*` operator in the default setting of the
    `ft_boolean_syntax` system variable.
  */
  char trunc;
  /** Parser state. Must be removed. */
  char prev;
  /** Parser state. Must be removed. */
  char *quot;
...
/**
  An argument of the full-text parser plugin. 
  
  This structure is filled in by MySQL server and passed to the parsing function of the
  plugin as an in/out parameter.
*/
typedef struct st_mysql_ftparser_param
{
  /**
    A pointer to the built-in parser implementation of the
    server.
    
    It's set by the server and can be used by the parser plugin
  ...
  */
  ...
  /**
    ...
    When parsing a document, the server sets this to point at a function that adds
  ...
  */
  ...
  /**
    ...
    The plugin can set it to point
  ...
  */
  ...
  /**
    ...
    It is used by MySQL functions
    called via mysql_parse() and mysql_add_word() callback. The plugin
    should not modify it.
  */
  void *mysql_ftparam;
  /**
    Information about the character set of the document or query string.
  */
  const struct charset_info_st *cs;
  /**
    A pointer to the document or query string to be parsed.
  */
  const char *doc;
  /**
    Length of the document or query string, in bytes.
  */
  int length;
  /**
    Flags for the parser. @sa MYSQL_FTFLAGS_NEED_COPY
  */
  unsigned int flags;
  /**
    The parsing mode. With boolean operators, with stopwords, or
    nothing.  @sa enum_ftparser_mode.
   */
...
/*
  ...
  interface_version is, e.g., @ref MYSQL_FTPARSER_INTERFACE_VERSION.
