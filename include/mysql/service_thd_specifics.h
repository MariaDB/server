...
/**
  @defgroup plugin_api_service_thd_specifics THD Specifics service
  @ingroup plugin_api_services
  THD specific for plugin(s)

  This API provides pthread_getspecific() like functionality to plugin authors.
  This is a functional alternative to the declarative MYSQL_THDVAR_* API.

  A plugin should at init call @ref thd_key_create() that create a key that
  will have storage in each THD. The key should be used by all threads
  and can be used concurrently from all threads.

  A plugin should at deinit call @ref thd_key_delete().

  Alternatively, a plugin can use @ref thd_key_create_from_var(K,V) to create
  a key that corresponds to a named MYSQL_THDVAR_* variable.
  ...
  Normally one should prefer MYSQL_THDVAR_* declarative API.

  The benefits are:

  - It supports typed variables (int, char*, enum, etc), not only void*.
  - The memory allocated for MYSQL_THDVAR_* is free'd automatically
    (if @ref PLUGIN_VAR_MEMALLOC is specified).
  - Continuous loading and unloading of the same plugin does not allocate
    memory for same variables over and over again.

  An example of using MYSQL_THDVAR_* for a thd local storage:
  @code
 ...
 */
#define thd_key_create(key) (thd_specifics_service->thd_key_create_func(key))
/**
 * delete THD specific storage
 */
#define thd_key_delete(key) (thd_specifics_service->thd_key_delete_func(key))
/**
 * get thd specific storage
 ...
 */
#define thd_getspecific(thd, key) (thd_specifics_service->thd_getspecific_func(thd, key))
/**
 * set thd specific storage
 *  - first time this is called from a thread it will return 0
 *  - this call is thread-safe in that different threads may call this
 *    simultaneously if operating on different THDs.
 *  - this call acquires no mutexes and is implemented as an array lookup
 */
#define thd_setspecific(thd, key, value) (thd_specifics_service->thd_setspecific_func(thd, key, value))
