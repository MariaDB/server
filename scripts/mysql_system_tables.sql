...
CREATE DEFINER='mariadb.sys'@'localhost' SQL SECURITY DEFINER VIEW IF NOT EXISTS user AS SELECT
  ...
  -- These bitfield values must remain in sync with those in sql/privilege.h; if not,
  -- the permissions referenced by these columns will be incorrect or incomplete.
  IF(JSON_VALUE(Priv, '$.access') & (1 <<  0), 'Y', 'N') AS Select_priv,
  IF(JSON_VALUE(Priv, '$.access') & (1 <<  1), 'Y', 'N') AS Insert_priv,
  IF(JSON_VALUE(Priv, '$.access') & (1 <<  2), 'Y', 'N') AS Update_priv,
  IF(JSON_VALUE(Priv, '$.access') & (1 <<  3), 'Y', 'N') AS Delete_priv,
  IF(JSON_VALUE(Priv, '$.access') & (1 <<  4), 'Y', 'N') AS Create_priv,
  IF(JSON_VALUE(Priv, '$.access') & (1 <<  5), 'Y', 'N') AS Drop_priv,
  IF(JSON_VALUE(Priv, '$.access') & (1 <<  6), 'Y', 'N') AS Reload_priv,
  IF(JSON_VALUE(Priv, '$.access') & (1 <<  7), 'Y', 'N') AS Shutdown_priv,
  IF(JSON_VALUE(Priv, '$.access') & (1 <<  8), 'Y', 'N') AS Process_priv,
  IF(JSON_VALUE(Priv, '$.access') & (1 <<  9), 'Y', 'N') AS File_priv,
  IF(JSON_VALUE(Priv, '$.access') & (1 << 10), 'Y', 'N') AS Grant_priv,
  IF(JSON_VALUE(Priv, '$.access') & (1 << 11), 'Y', 'N') AS References_priv,
  IF(JSON_VALUE(Priv, '$.access') & (1 << 12), 'Y', 'N') AS Index_priv,
  IF(JSON_VALUE(Priv, '$.access') & (1 << 13), 'Y', 'N') AS Alter_priv,
  IF(JSON_VALUE(Priv, '$.access') & (1 << 14), 'Y', 'N') AS Show_db_priv,
  IF(JSON_VALUE(Priv, '$.access') & (1 << 15), 'Y', 'N') AS Super_priv,
  IF(JSON_VALUE(Priv, '$.access') & (1 << 16), 'Y', 'N') AS Create_tmp_table_priv,
  IF(JSON_VALUE(Priv, '$.access') & (1 << 17), 'Y', 'N') AS Lock_tables_priv,
  IF(JSON_VALUE(Priv, '$.access') & (1 << 18), 'Y', 'N') AS Execute_priv,
  IF(JSON_VALUE(Priv, '$.access') & (1 << 19), 'Y', 'N') AS Repl_slave_priv,
  IF(JSON_VALUE(Priv, '$.access') & (1 << 20), 'Y', 'N') AS Binlog_monitor_priv,      -- Was Repl_client_priv prior to 10.5.2
  IF(JSON_VALUE(Priv, '$.access') & (1 << 21), 'Y', 'N') AS Create_view_priv,
  IF(JSON_VALUE(Priv, '$.access') & (1 << 22), 'Y', 'N') AS Show_view_priv,
  IF(JSON_VALUE(Priv, '$.access') & (1 << 23), 'Y', 'N') AS Create_routine_priv,
  IF(JSON_VALUE(Priv, '$.access') & (1 << 24), 'Y', 'N') AS Alter_routine_priv,
  IF(JSON_VALUE(Priv, '$.access') & (1 << 25), 'Y', 'N') AS Create_user_priv,
  IF(JSON_VALUE(Priv, '$.access') & (1 << 26), 'Y', 'N') AS Event_priv,
  IF(JSON_VALUE(Priv, '$.access') & (1 << 27), 'Y', 'N') AS Trigger_priv,
  IF(JSON_VALUE(Priv, '$.access') & (1 << 28), 'Y', 'N') AS Create_tablespace_priv,
  IF(JSON_VALUE(Priv, '$.access') & (1 << 29), 'Y', 'N') AS Delete_history_priv,      -- Added in 10.3.4
  IF(JSON_VALUE(Priv, '$.access') & (1 << 30), 'Y', 'N') AS Set_user_priv,            -- Added in 10.5.2
  IF(JSON_VALUE(Priv, '$.access') & (1 << 31), 'Y', 'N') AS Federated_admin_priv,     -- Added in 10.5.2
  IF(JSON_VALUE(Priv, '$.access') & (1 << 32), 'Y', 'N') AS Connection_admin_priv,    -- Added in 10.5.2
  IF(JSON_VALUE(Priv, '$.access') & (1 << 33), 'Y', 'N') AS Read_only_admin_priv,     -- Added in 10.5.2
  IF(JSON_VALUE(Priv, '$.access') & (1 << 34), 'Y', 'N') AS Repl_slave_admin_priv,    -- Added in 10.5.2
  IF(JSON_VALUE(Priv, '$.access') & (1 << 35), 'Y', 'N') AS Repl_master_admin_priv,   -- Added in 10.5.2
  IF(JSON_VALUE(Priv, '$.access') & (1 << 36), 'Y', 'N') AS Binlog_admin_priv,        -- Added in 10.5.2
  IF(JSON_VALUE(Priv, '$.access') & (1 << 37), 'Y', 'N') AS Binlog_replay_priv,       -- Added in 10.5.2 (replacing the value of Binlog_monitor_priv above)
  IF(JSON_VALUE(Priv, '$.access') & (1 << 38), 'Y', 'N') AS Slave_monitor_priv,       -- Added in 10.5.9
