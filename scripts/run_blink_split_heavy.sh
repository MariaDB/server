...
set_core() {
  local value=$1
  if mysql -NBe "SHOW GLOBAL VARIABLES LIKE 'innodb_blink_core_enabled'" |
      grep -q '^innodb_blink_core_enabled'; then
    mysql -e "SET GLOBAL innodb_blink_core_enabled='$value'"
  fi
}
...
run() {
  ...
  mysql -NBe "SHOW GLOBAL VARIABLES WHERE Variable_name IN
    ('innodb_buffer_pool_size','innodb_log_file_size',
     ...
     'performance_schema','innodb_blink_enabled','innodb_blink_core_enabled')" \
