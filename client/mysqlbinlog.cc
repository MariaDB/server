    ...
    else
    {
      ...
      else if (buf[EVENT_TYPE_OFFSET] == FORMAT_DESCRIPTION_EVENT)
      {
        ...
        if (!(new_description_event= (Format_description_log_event*) 
              Log_event::read_log_event(NULL, file, glob_description_event,
                                        opt_verify_binlog_checksum)))
      ...
      else if (buf[EVENT_TYPE_OFFSET] == ROTATE_EVENT)
      {
        Log_event *ev;
        my_b_seek(file, tmp_pos); /* seek back to event's start */
        if (!(ev= Log_event::read_log_event(NULL, file, glob_description_event,
                                            opt_verify_binlog_checksum)))
        {
          /* EOF can't be hit here normally, so it's a real error */
                                          ...
                                          const char* logname)
{
  ...
  for (;;)
  {
    ...
    Log_event* ev = Log_event::read_log_event(NULL, file, glob_description_event,
                                              opt_verify_binlog_checksum);
    if (!ev)
    {
      /*
