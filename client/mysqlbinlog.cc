...
static struct my_option my_options[] =
{
  ...
  */
  ...
  {"progress-log", 'L', "Write progress info to given file. "
   ...
   "[FINAL]: The last event applied before a stop condition was found. "
                                          ...
                                          const char* logname)
{
  ...
  if (logname && strcmp(logname, "-") != 0)
  {
    ...
    if (progress_file != NULL)
      fprintf(progress_file, "# OPEN %s with starting position %lu\n", logname, (unsigned long) start_position_mot);
...
err:
  ...
  if (progress_file != NULL)
  {
    if (retval == OK_STOP)
    {
      if (prev_ev_time)
      {
        res = localtime((time_t *)&prev_ev_time);
        fprintf(progress_file, "# FINAL applied event: logfile %s ", logname);
        fprintf(progress_file, "Time: %02d%02d%02d %2d:%02d:%02d",
            res->tm_year % 100,
            res->tm_mon+1,
            res->tm_mday,
            res->tm_hour,
            res->tm_min,
            res->tm_sec);
        fprintf(progress_file, " Pos: %lu\n", (unsigned long) prev_ev_pos);
      }
      else
      {
        fprintf(progress_file, "# No previous event\n");
      }
      res = localtime((time_t *)&this_ev_time);
