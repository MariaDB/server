...
static bool write_event_footer(FILE *outfile, my_bool do_checksum,
                               ha_checksum crc)
{
  if (do_checksum)
  {
...
/*
  Generates the output legacy binlog file name based on the prefix and the
  index
...
*/
static void generate_output_legacy_binlog_name(char *out_name,
                                               size_t out_name_len,
                                               ulonglong index)
{
                                          ...
                                          const char* logname)
{
  ...
  if (binlog_format == INNODB_BINLOG_FORMAT)
  {
    ...
    if (opt_convert_engine_binlog)
    {
      ...
      {
        error("Failed to read initial GTID state from the header of the "
              "innodb binlog file");
        goto err;
      }
    }
