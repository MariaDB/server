  ...
  while ((row = mysql_fetch_row(result)))
  {
	...
	else
	{
	  ...
	  if (opt_verbose > 1)
	  {
            ...
            my_snprintf(query, sizeof(query), "SELECT COUNT(*) FROM %sQ",
                        row[0]);
	    if (!(mysql_query(mysql,query)))
	    {
	      if ((rresult = mysql_store_result(mysql)))
...
list_table_status(MYSQL *mysql,const char *db,const char *wild)
{
  ...
  len= sizeof(query);
  len-= my_snprintf(query, len, "show table status from %sQ", db);
  if (wild && wild[0] && len)
	    ...
	    const char *wild)
{
  ...
  if (opt_count)
  {
    my_snprintf(query, sizeof(query), "select count(*) from %sQ", table);
    if (mysql_query(mysql,query) || !(result=mysql_store_result(mysql)))
    {
  ...
  len= sizeof(query);
  len-= my_snprintf(query, len, "show /*!32332 FULL */ columns from %sQ",
                    table);
  if (wild && wild[0] && len)
    strxnmov(query + strlen(query), len - 1, " like '", wild, "'", NullS);
  if (mysql_query(mysql,query) || !(result=mysql_store_result(mysql)))
  ...
  if (opt_show_keys)
  {
    ...
    result= NULL;
    my_snprintf(query, sizeof(query), "show keys from %sQ", table);
    if (mysql_query(mysql,query) || !(result=mysql_store_result(mysql)))
    {
