...
/*
  ...
  chmod/chown error are ignored after the first one, with a warning printed,
...
static void fix_permissions_and_owner(const char *dirpath)
{
  ...
  static bool fix_ownership_uid= true; // Try fixing user+grop ownership
  ...
  */
  ...
  if (fix_ownership_gid && chown(dirpath, -1, st_out_dir.st_gid))
  {
    ...
    {
      // Only warn,if "others" can't write, don't spam stderr.
      fprintf(stderr,
              "Warning: cannot set group ownership on directory %s, %d\n",
              dirpath, errno);
    }
