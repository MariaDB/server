...
static char *ENSUREFILE= (char*) "mysqld_err_ensure.h";
...
static struct my_option my_long_options[]=
{
  ...
  {"ensure_file", 'E', "ENSUREFILE file ", &ENSUREFILE,
   &ENSUREFILE, 0, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
...
static int create_sys_files(struct languages *lang_head,
			    struct errors *error_head, uint max_error,
                            uint error_count);
static int get_format_ensure_str(const char* msg, char *out, uint sz);
...
static int create_header_files(struct errors *error_head)
{
  char fmt_buf[MAX_ARGS];
  uint er_last= 0;
  uint section= 1;
  FILE *er_definef, *sql_statef, *er_namef, *er_ensuref;
  ...
  if (!(er_ensuref= my_fopen(ENSUREFILE, O_WRONLY, MYF(MY_WME))))
  {
  ...
  fprintf(er_ensuref, "/* Autogenerated file, please don't edit */\n\n");
  ...
  for (tmp_error= error_head; tmp_error; tmp_error= tmp_error->next_error)
  {
    ...
    */
    ...
    fprintf(er_ensuref, "#define ENSURE_%s \"", tmp_error->er_name);

    if (get_format_ensure_str(er_text, fmt_buf, sizeof fmt_buf))
      DBUG_RETURN(1);
    print_escaped_string(er_ensuref, fmt_buf);
    fprintf(er_ensuref, "\"\n");
  }
...
static int get_format_ensure_str(const char* msg, char *out, uint sz)
{
  ...
  while (*p)
  {
    ...
    if (letter == '%')
    {
      if (start != 0)
      {
        fprintf(stderr, "get_format_ensure_str: format specifier is broken: "
                        "\"%s\"", msg);
        return 1;
      }
      start= p+1; /* Entering format specifier */
    ...
    if (in_format)
    {
      if (pos >= sz - 1)
      {
        fprintf(stderr, "get_format_ensure_str: out is not big enough\n");
        return 1;
      }
      out[pos]= letter;
      pos++;
