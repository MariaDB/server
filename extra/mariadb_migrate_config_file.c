...
static size_t global_copied_lines=0, global_unsupported_lines= 0;
...
static void report_line_changes()
{
  if (!opt_print && !opt_silent && opt_edit_mode == EDIT_MODE_NONE)
  {
    if (global_mariadbd_additions)
      fprintf(stdout, "%lld line/lines would be copied to [mariadbd] group\n",
              (longlong) global_mariadbd_additions);
    if (global_copied_lines > 0)
      fprintf(stdout, "%lld line/lines would be copied to [%s] group\n",
              (longlong) global_copied_lines, opt_error_group);
    if (global_unsupported_lines)
      fprintf(stdout, "%lld line/lines would be copied to [%s] group\n",
              (longlong) global_unsupported_lines, opt_unsupported_group);
  }
}
