/* Copyright (c) 2000, 2017, Oracle and/or its affiliates. All rights reserved.
   Copyright (c) 2011, 2018, 2026 MariaDB Corporation
...
const char *client_server_group= "client-server";
...
struct RENAMED_OPTIONS
{
  const char *from, *to;
};

struct RENAMED_OPTIONS renamed_options[]=
{
...
LEX_CSTRING mysqld_name=  {STRING_WITH_LEN("mysqld")};
...
static my_bool opt_add_mariadb_replication_compatibility;
static my_bool opt_add_mysql_replication_compatibility;
static my_bool opt_add_client_server_group;
static my_bool opt_silent;
static my_bool opt_update_paths;
static my_bool opt_copy_mysqld_groups;
static my_bool add_new_options_once=0, new_options_added= 0;
static uint audit_plugin_warning= 0;
static const char **groups_to_use;
static const char *opt_mariadbd= "";
static ulong opt_edit_mode, opt_convert_from;
static int global_copied_lines=0, global_unsupported_lines= 0;
static int global_mariadbd_additions= 0;
...
static const char *mariadb_replication_compatibility_comment=
  "# The following options enable MySQL to replicate to MariaDB";
static const char *mysql_replication_compatibility_comment=
  "# The following options enable MariaDB to replicate to MySQL";

/* Options found that require special handling */
static int value_port;
static char *value_socket= 0;
static my_bool found_skip_slave_start= FALSE;
static my_bool found_innodb_log_files_in_group= 0;
static my_bool found_innodb_log_file_size= 0;
static my_bool found_character_set_server= 0;
static my_bool found_client_server_group= 0;
static my_bool found_mariadb_replication_compatibility= 0;
static my_bool found_mysql_replication_compatibility= 0;

static uint value_innodb_log_files_in_group= 0;
static ulonglong value_innodb_log_file_size= 0;
static char suffix_innodb_log_file_size;
...
/**
  Remove the -upgrade-config-orig backups of the updated files if
  successful and replace the updated files with the backups if not.
*/

static int finish_updated_files(struct upgrade_ctx *ctx, my_bool success)
{
  size_t i;
  if (!opt_silent && success && ctx->updated_files.elements)
    fputs("Updated files:\n", stdout);
  for (i= 0; i < ctx->updated_files.elements; i++)
  {
    ...
    char *orig = my_malloc(key_memory_upgrade_config, orig_len, MYF(MY_WME));
    if (!orig)
      return 1;
    ...
    {
      if (my_redel(name, orig, 0, MYF(MY_WME | MY_REDEL_MAKE_BACKUP)))
      {
        my_free(orig);
        return 1;
      }
                        ...
                        int defaults_args_count)
{
  ...
  dynstr_append(&ds_cmdline, "--silent-startup ");
  dynstr_append(&ds_cmdline, "--log-warnings=0 ");
...
static my_bool check_plugin_option(struct upgrade_ctx *ctx,
                                   const char *option,
                                   const char *value,
                                   char *name,
                                   uint line)
{
  ...
  if ((plugin_check_result= check_plugins(option, value)) != PLUGINS_OK)
  {
    if (!opt_print)
    {
      switch (plugin_check_result) {
      case PLUGINS_OK: break;               /* Impossible */
      case AUDIT_PLUGIN:
        if (!audit_plugin_warning++ && !opt_silent)
          fprintf(stdout, "In %s at line %d: Please replace audit_log "
                  "with the server_audit plugin\n", name, line);
      }
      return 1;
    }
  }
...
static char *find_end_comment(char *ptr)
{
...
struct generator
{
  ...
  */
  ...
  /* array of strings to be added to the client_server section */
  DYNAMIC_ARRAY client_server;

  struct upgrade_ctx *ctx;          /* Current context */
  ...
  */
  ...
  my_bool prev_empty;
  /* If we should copy [mysqld-xxx] options to [mariadbd] */
  my_bool copy_code_to_mariadb;
  /* If current file is valid */
  my_bool file_valid;
};
...
static void generator_init(struct generator *generator)
{
  ...
  generator->copy_code_to_mariadb= 0;
}
...
static my_bool change_output_group(struct generator *generator,
                                   const char *group)
{
  char *group_dupe= my_strdup(key_memory_upgrade_config, group, MYF(0));
  if (!group_dupe)
    return TRUE;
...
/*
  Generate output lines for one section
  Returns number of lines added
*/


static int generate_write_section(FILE *target_file,
                                   ...
                                   const char *group_name)
{
  ...
  if (array->elements > 0)
  {
    ...
    change_output_group(generator, group_name);
    for (uint i= 0; i < array->elements; i++)
    {
      char *element= *(char **)dynamic_array_ptr(array, i);
      if (element)
      {
...
static my_bool generator_write(MYSQL_FILE *f, struct generator *generator)
{
  ...
  else
  {
    ...
    if (count_array_elements(&generator->mariadbd_additions) > 0)
      lines_handled+= generate_write_section(target_file,
                                             generator,
                                             &generator->mariadbd_additions,
                                             "mariadbd");
  }
  generate_write_section(target_file, generator,
                         &generator->old_version, opt_error_group);
  generate_write_section(target_file, generator,
                         &generator->unsupported_version,
  ...
  new_lines= (lines_handled + generator->old_version.elements +
              generator->unsupported_version.elements +
              generator->client_server.elements);
  if (new_lines)
  {
    global_update_count+= new_lines;
    generator->copy_code_to_mariadb= 0;
  }
  return new_lines != 0;
}
...
static void generator_count_lines(struct generator *generator)
{
  ...
  global_mariadbd_additions+= mariadbd_additions;
  global_update_count+= (mariadbd_additions +
                         generator->old_version.elements +
                         generator->unsupported_version.elements +
                         generator->client_server.elements);
  global_copied_lines+= generator->old_version.elements;
  global_unsupported_lines+= generator->unsupported_version.elements;
}


static void report_line_changes()
{
  if (!opt_print && !opt_silent && opt_edit_mode == EDIT_MODE_NONE)
  {
    if (global_mariadbd_additions)
      fprintf(stdout, "%d line/lines would be copied to [mariadbd] group\n",
              global_mariadbd_additions);
    if (global_copied_lines > 0)
      fprintf(stdout, "%d line/lines would be copied to [%s] group\n",
              global_copied_lines, opt_error_group);
    if (global_unsupported_lines)
      fprintf(stdout, "%d line/lines would be copied to [%s] group\n",
              global_unsupported_lines, opt_unsupported_group);
  }
}
...
/* Called on include of new default file */

static void generator_reset(struct generator *generator)
{
  generator->main.elements= 0;
  generator->old_version.elements= 0;
  generator->unsupported_version.elements= 0;
  generator->mariadbd_additions.elements= 0;
  generator->mariadbd_group_end= -1;
}
                             ...
                             const char *format, ...)
{
  va_list args;
  my_bool res;
  /* Ensure that --print without edit doesn't loose space */
  my_bool delay_empty= (type == LINE_TYPE_EMPTY &&
                        opt_edit_mode != EDIT_MODE_NONE);

  /* generator->input_group is empty for comments at top of include files */
  if (!delay_empty && generator->input_group &&
       ...
       strcmp(generator->input_group, generator->output_group)))
  {
    if (add_line(&generator->alloc, &generator->main,
      ...
      return TRUE;
    if (change_output_group(generator, generator->input_group))
      return TRUE;
...
/*
  Change file paths from /mysql ending with /, \, . space or \n to /mariadbd
...
char *change_file_path(const char *line, LEX_CSTRING *from, LEX_CSTRING *to)
{
  ...
  {
    char end= pos[from->length];
    if (pos != line && (pos[-1] == '/' || pos[-1] == '\\') &&
        (end == pos[-1] || end == 0 || my_isspace(&my_charset_latin1, end) ||
         end == '.'))
    {
...
/* Change path with warning */

static char *path_changed(struct generator *generator, const char *line)
{
  char *new_line;
  if (!(new_line= change_file_path(line, &mysql_name, &mariadb_name)) &&
      !(new_line= change_file_path(line, &mysqld_name, &mariadb_name)))
    return 0;

  if (!opt_print && opt_edit_mode == EDIT_MODE_NONE)
  {
    fprintf(stdout, "In %s at line %d: '%.*s' would be "
            "replaced with '%.*s'\n", generator->name, generator->line,
            (int) strlen(line)-1, line,
            (int) strlen(new_line)-1, new_line);
    global_update_count++;
    // generator->ctx->failed= 1;
  }
  return new_line;
}
...
/*
  find new name of renamed option
*/

const char *renamed_option(const char *option)
{
  struct RENAMED_OPTIONS *options;
  for (options= renamed_options; options->from ; options++)
    if (!strcmp(option, options->from))
      return options->to;
  return 0;
}


void error_unknown_option(struct generator *generator,
                          ...
                          const char *org_line)
{
  ...
  if (!opt_print && opt_edit_mode == EDIT_MODE_NONE &&
      !generator->copy_code_to_mariadb)
  {
    if ((renamed= renamed_option(option)))
    {
      if (value)
        fprintf(stdout, "In %s at line %u: '%.*s' would be "
                "replaced with '%s=%.*s'\n", generator->name, generator->line,
                (int) strlen(org_line)-1, org_line, renamed,
    ...
    {
      fprintf(stdout, "In %s at line %u: Invalid option %s\n", generator->name,
              generator->line, option);
    }
    // ctx->failed= 1;
    global_update_count++;
  }
  if (!strncmp(option, STRING_WITH_LEN("audit_log")) &&
      !audit_plugin_warning++ && !opt_silent)
    fprintf(stdout, "Warning: Please replace audit_log with the server_audit "
            "plugin\n");
}


/*
  Store value for socket= option if used
  @return 1 if socket found
  @return 0 no socket.
*/

my_bool check_if_socket(const char *option, const char *value)
{
  if (!strcmp(option, "socket"))
  {
    const char *end;
    my_free(value_socket);

    /* The string may have an '\n' if paths has changed */
    if (!(end= strchr(value, '\n')))
      end= strend(value);
    value_socket= my_strndup(key_memory_upgrade_config, value, (end-value),
                             MYF(MY_WME));
                                         ...
                                         int recursion_level)
{
  ...
  char *value, *option, *option_value_start, *new_line, *end_of_option;
  const char *renamed;
  ...
  my_bool group_is_mariadbd= FALSE;
  my_bool group_is_mysqld= FALSE;
  uint option_start;
  enum { NONE, PARSE, SKIP } found_group= NONE;
  my_bool skip_prefix= 0, generator_inited= 0;
  size_t i;
  MY_DIR *search_dir;
  ...
  if (recursion_level == 0)
  {
    ...
    my_free(value_socket);
    value_socket= 0;
    found_skip_slave_start= FALSE;
    found_innodb_log_files_in_group= 0;
    found_innodb_log_file_size= 0;
    found_character_set_server= 0;
    value_innodb_log_files_in_group= 0;
    value_innodb_log_file_size= 0;
    suffix_innodb_log_file_size= 0;
    found_mariadb_replication_compatibility= 0;
    found_mysql_replication_compatibility= 0;
    found_client_server_group= 0;
  }
  ...
  while (mysql_file_fgets(buff, sizeof(buff) - 3, fp))
  {
    ...
    if (*ptr == '#' || *ptr == ';' || !*ptr)
    {
      if (is_prefix(ptr, mariadb_replication_compatibility_comment))
      {
        /* File is already updated */
        found_mariadb_replication_compatibility= 0;
      }
      if (is_prefix(ptr, mysql_replication_compatibility_comment))
      {
        /* File is already updated */
        found_mysql_replication_compatibility= 0;
      }
    ...
    if (*ptr == '!' || *ptr == '?')
    {
      /* Write out current cached items before include directive */
      my_bool prev_empty= generator.prev_empty;
      if (opt_print || opt_update)
      {
        if (generator_write(tmp_fp, &generator))
          generator.file_valid= FALSE;
      }
      else
        generator_count_lines(&generator);
      generator_reset(&generator);
      /* Ensure that when using --print we get !include next */
      if (prev_empty && !generator.prev_empty)
        generator_add_line(&generator, "\n", LINE_TYPE_EMPTY, 1);
      generator_add_line(&generator, buff, LINE_TYPE_PASSTHROUGH, 1);
          ...
          my_isspace(&my_charset_latin1, ptr[sizeof(includedir_keyword) - 1]))
      {
        ...
        for (i= 0; i < search_dir->number_of_files; i++)
        {
          ...
          if (*tmp_ext)
          {
            ...
            process_default_file_with_ext(ctx, "", "", tmp,
                                          recursion_level + 1);
            if (opt_print || (opt_edit_mode != EDIT_MODE_NONE && !opt_update))
              fprintf(stdout, "### Back in %s:\n", name);
          }
        }
	...
	goto err;
      ...
      group_is_mariadbd= !strcmp(curr_gr, "mariadbd");
      group_is_mysqld= (!strcmp(curr_gr, "mysqld") ||
                        !strcmp(curr_gr, "server"));
      found_client_server_group|= !strcmp(curr_gr, client_server_group);
      ...
      if (found_group == SKIP)
      {
        if (!strncmp(curr_gr, "mysqld-", 7) && opt_copy_mysqld_groups)
        {
          found_group= PARSE;
          /* Copy all valid code from the mysqld- group to the mariadbd group */
          generator.copy_code_to_mariadb= 1;
        }
      }

      generator.input_group= curr_gr;
      if (found_group == PARSE && group_is_mariadbd)
      {
        /**
           Force the mariadbd group to be emitted to allow potentially adding
           new lines to it.
        */
    ...
    case NONE:
      fprintf(stderr,
	      "error: Found option without preceding group in config "
              "file: %s at line: %d\n",
	      name, generator.line);
      goto err;
    case PARSE:
    ...
    end= end_of_option= find_end_comment(ptr);
    if ((value= strchr(ptr, '=')) && value < end_of_option)
      end= value;
    else
      value= 0;
	...
	value_end--;
      ...
      *ptr++= 0;
      ...
      tmp= mariadbd_option_exists(option);
      if (!tmp && skip_prefix && mariadbd_option_exists(conv_option))
      {
        /* Option only exists with the 'skip-' prefix */
        tmp= 1;
        option= conv_option;                    /* Option with skip- prefix */
      }
      if (!tmp)
      {
        if (generator.copy_code_to_mariadb)               /* [mysqld-#] group */
        {
          /* Changes will be detected in generator_write() */
        ...
        error_unknown_option(&generator, ctx, conv_option, value, buff);
        if (!opt_print && opt_edit_mode == EDIT_MODE_NONE)
          continue;
      }
	...
	if (*value == '\\' && value != value_end-1)
	{
      ...
      *ptr=0;
      if (!mariadbd_valid_enum_value(option, option_value_start))
      {
        ...
        if (!opt_print && opt_edit_mode == EDIT_MODE_NONE)
        {
          fprintf(stdout,
                  ...
                  name, generator.line, option_value_start, option);
          global_update_count++;
          // ctx->failed= 1;
          continue;
        }
                ...
                mariadbd_check_set_value(option, option_value_start)))
      {
        ...
        if (!opt_print && opt_edit_mode == EDIT_MODE_NONE)
        {
          fprintf(stdout, "In %s at line %d: Invalid set value '%s' at "
                  ...
                  name, generator.line, option_value_start, invalid_set_index,
                  option);
          global_update_count++;
          // ctx->failed= 1;
          continue;
      ...
      else if (check_plugin_option(ctx, option, option_value_start, name,
                                   generator.line))
      {
        line_valid= FALSE;
        generator.file_valid= FALSE;
      }
      else if (group_is_mysqld && line_valid &&
               opt_update_paths &&
               (new_line= path_changed(&generator, buff)))
      {
        line_valid= FALSE;

        /* We have alread validated that buff contains an '=' */
        check_if_socket(option, strchr(new_line, '=')+1);
        if ((opt_edit_mode == EDIT_MODE_INLINE ||
             opt_edit_mode == EDIT_MODE_LAST))
        {
          generator_add_line(&generator, buff, LINE_TYPE_OPTION, FALSE);
      ...
      else if (!group_is_mariadbd &&
               !strcmp(option, "key_buffer_size") &&
               (opt_no_myisam_options || opt_convert_from == FROM_MYSQL) &&
                ...
                opt_edit_mode == EDIT_MODE_LAST))
      {
        ...
        {
          {
            fprintf(stdout, "In %s at line %d: key_buffer_size needs "
                    "modifications. If you want to git rid of this warning and "
                    "keep as it now, move it to the [mariadbd] group\n",
                    name, generator.line);
            global_update_count++;
          }
          continue;
        ...
        if (opt_no_myisam_options)
        {
          ...
          add_line(&generator.alloc, &generator.mariadbd_additions, "%s\n",
                   conv_option);
          if (!opt_silent)
            fprintf(stdout, "Note: '%s' added to [mariadbd] group\n",
                    conv_option);
        }
        else if (opt_convert_from == FROM_MYSQL)
        {
          add_line(&generator.alloc, &generator.mariadbd_additions, "%s", buff);
          add_line(&generator.alloc, &generator.mariadbd_additions,
                   "#key_buffer_size=64K   "
                   "# Recommended value if no MyISAM tables\n");
          snprintf(conv_option, sizeof(conv_option)-1,
                   "aria_pagecache_buffer_size=%s",
                   option_value_start);
          add_line(&generator.alloc, &generator.mariadbd_additions, "%s\n",
        ...
        continue;
               ...
               !strcmp(option, "innodb_log_files_in_group"))
      {
        found_innodb_log_files_in_group= 1;
        value_innodb_log_files_in_group= atoi(option_value_start);
        if (!value_innodb_log_files_in_group)
          value_innodb_log_files_in_group= 1; /* Ensure it is not 0 */
      }
      else if (!strcmp(option, "innodb_log_file_size"))
      {
        if (group_is_mariadbd)
        {
          /* Already handled. Don't add it again  */
          found_innodb_log_file_size= 0;
          found_innodb_log_files_in_group= 0;
        }
        else
        {
          int error;
          char *int_end= 0;
          value_innodb_log_file_size= my_strtoll10(option_value_start,
                                                   &int_end, &error);
          found_innodb_log_file_size= (value_innodb_log_file_size != 0 &&
                                       error == 0);
          suffix_innodb_log_file_size= *int_end;
        }
      }
      else if (!strcmp(option, "character_set_server") ||
               !strcmp(option, "collation_server"))
      {
        found_character_set_server= 1;
      }
      else if (group_is_mysqld && !strcmp(option, "port"))
      {
        int error;
        char *int_end= 0;
        value_port= my_strtoll10(option_value_start, &int_end, &error);
      }
      else if (group_is_mysqld && check_if_socket(option, option_value_start))
      {
      }
      else if ((renamed= renamed_option(option)))
      {
        generator_add_line(&generator, buff, LINE_TYPE_OPTION, FALSE);
        add_line(&generator.alloc, &generator.mariadbd_additions,
                 "%s=%s\n", renamed, option_value_start);
        continue;
      }
    } /* !value */
    else if (!group_is_mariadbd && !strcmp(option, "skip_slave_start"))
    {
      found_skip_slave_start= TRUE;
    }
    else if (!mariadbd_option_exists(option) &&
             !(skip_prefix && mariadbd_option_exists(conv_option)))
    {
      line_valid= FALSE;
      generator.file_valid= FALSE;

      error_unknown_option(&generator, ctx, conv_option, NullS, buff);
  ...
  if (recursion_level == 0)
  {
    if (found_innodb_log_file_size && found_innodb_log_files_in_group)
    {
      generator.file_valid= FALSE;
      snprintf(conv_option, sizeof(conv_option)-1,
               "innodb_log_file_size=%llu%c",
               value_innodb_log_file_size * value_innodb_log_files_in_group,
               (my_islower(&my_charset_latin1, suffix_innodb_log_file_size) ||
                my_isupper(&my_charset_latin1, suffix_innodb_log_file_size)) ?
               suffix_innodb_log_file_size : ' ');
      add_line(&generator.alloc, &generator.mariadbd_additions, "%s\n", conv_option);
      if (!opt_silent)
        fprintf(stdout, "Note: '%s' added to [mariadbd] group based on "
                "innodb_log_file_size * innodb_log_files_in_group\n", conv_option);
      found_innodb_log_file_size= found_innodb_log_files_in_group= 0;
    }
    if (opt_add_client_server_group && !found_client_server_group)
    {
      if (value_port)
        add_line(&generator.alloc, &generator.client_server,
                 "port=%d\n", value_port);
      if (value_socket)
      {
        add_line(&generator.alloc, &generator.client_server,
    ...
    /* The following options should only be added once */
    if (!add_new_options_once || !new_options_added)
    {
      new_options_added= 1;

      /* Add generated options to mariadbd section */
      if (opt_add_skip_slave_start && !found_skip_slave_start)
      {
        generator.file_valid= FALSE;
        add_line(&generator.alloc, &generator.mariadbd_additions,
                 "skip_slave_start\n");
        found_skip_slave_start= 0;
      }
      if (opt_add_mariadb_replication_compatibility &&
          !found_mariadb_replication_compatibility)
      {
        generator.file_valid= FALSE;
        /* We do this line by line to get correct row numbers for reporting */
        add_line(&generator.alloc, &generator.old_version,
                 "%s\n", mariadb_replication_compatibility_comment);
        add_line(&generator.alloc, &generator.old_version,
                 "loose-binlog_row_value_options=\"\"\n");
        add_line(&generator.alloc, &generator.old_version,
                 "loose-binlog_transaction_compression=0\n");
        add_line(&generator.alloc, &generator.old_version,
                 "# Not required, but recommended for cross_replication\n");
        add_line(&generator.alloc, &generator.old_version,
                 "binlog_format=row\n");
      }
      if (opt_add_mysql_replication_compatibility &&
          !found_mysql_replication_compatibility)
      {
        add_line(&generator.alloc, &generator.mariadbd_additions,
                 "%s\n", mysql_replication_compatibility_comment);
        add_line(&generator.alloc, &generator.mariadbd_additions,
                 "encrypt_binlog=0\n");
        add_line(&generator.alloc, &generator.mariadbd_additions,
                 "log_bin_compress=0\n");
        ...
        add_line(&generator.alloc, &generator.mariadbd_additions,
                 "binlog_format=row\n");
        if (!found_character_set_server)
        {
          add_line(&generator.alloc, &generator.mariadbd_additions,
                   "# Character set to be able to replicate new tables to "
                   "MySQL 8.0\n");
          add_line(&generator.alloc, &generator.mariadbd_additions,
          ...
          if (!opt_silent)
            fprintf(stdout, "Note: Added MySQL default character sets and "
                    "collation to [mariadbd] to be able to replicate to "
                    "MySQL\n");
          found_character_set_server= 0;
        }
      }
                                ...
                                const char *config_file)
{
  ...
  {
    int error;
    if ((error= process_default_file_with_ext(ctx, dir, *ext, config_file,
                                              0)) < 0)
      return error;
  }
  return 0;
                            ...
                            int defaults_args_count)
{
  ...
  if (opt_mariadbd_testing)
  {
    ...
    if (opt_mariadbd)
      used_name= opt_mariadbd;
    else
    {
      used_name= mariadbd_name;
      if (!(error= (find_file_in_path(executable, mariadbd_name) != 0)))
        used_name= executable;
    }
    if (!error)
      error= access(used_name, X_OK);
    ...
    if (init_dynamic_string(&mariadbd_output, "", 32, 32))
      goto err;
    if (!test_mariadbd(used_name) &&
        (error= run_mariadbd(used_name, &mariadbd_output, defaults_args,
                             defaults_args_count)))
    {
      if (ctx.updated_files.elements)
        fprintf(stderr, "error: Failed to run %s with the updated files, "
                "reverting\n", used_name);
      else
        fprintf(stderr, "error: Failed to run %s with the original files.\n",
                used_name);
      if (error > 0)
  ...
  if (finish_updated_files(&ctx, error == 0))
    error= 1;
  free_root(&alloc, MYF(0));
  return error;
...
enum upgrade_config_options
{
  ...
  OPT_VERIFY,
  OPT_MARIADBD,
  OPT_ADD_MYSQL_REPLICATION_COMPATIBILITY,
  OPT_ADD_MARIADB_REPLICATION_COMPATIBILITY,
  OPT_COPY_MYSQLD_GROUPS,
  OPT_GENERATE_CLIENT_SERVER_GROUP
};
...
static struct my_option my_long_options[] =
{
  ...
  {"add-mariadb-replication-compatibility",
   OPT_ADD_MARIADB_REPLICATION_COMPATIBILITY,
   "Add options to [error-group] to enable MySQL to replicate to MariaDB. "
   "Options added are: "
   "binlog-row-value-options=\"\"  binlog_transaction_compression=0 and "
   "binlog_format=row.",
   &opt_add_mariadb_replication_compatibility,
   &opt_add_mariadb_replication_compatibility,
   0, GET_BOOL, NO_ARG, FALSE, 0, 0, 0, 0, 0},
  {"add-mysql-replication-compatibility",
   OPT_ADD_MYSQL_REPLICATION_COMPATIBILITY,
   "Add options to [mariadbd] section to enable MariaDB to replicate to MySQL.",
   &opt_add_mysql_replication_compatibility,
   &opt_add_mysql_replication_compatibility,
   0, GET_BOOL, NO_ARG, FALSE, 0, 0, 0, 0, 0},
  {"add-client-server-group",
   OPT_ADD_MYSQL_REPLICATION_COMPATIBILITY,
   "Add a [client-server] group if it does not exists. This will contain the "
   "last port and socket found in the [mysqld] or [server] groups.",
   &opt_add_client_server_group,
   &opt_add_client_server_group,
   0, GET_BOOL, NO_ARG, FALSE, 0, 0, 0, 0, 0},
  {"backup", OPT_BACKUP,
   "Backup the updated configuration files. The backup file name are of the "
   "type: 'original_name'-timestamp.BAK",
   &opt_backup, &opt_backup, 0, GET_BOOL, NO_ARG, FALSE, 0, 0, 0, 0, 0},
  {"copy-mysqld-groups", OPT_COPY_MYSQLD_GROUPS,
   "Copy valid MariaDB options from [mysqld-###] groups to the [mariadbd] "
   "group.",
   &opt_copy_mysqld_groups, &opt_copy_mysqld_groups, 0, GET_BOOL, NO_ARG, TRUE,
   0, 0, 0, 0, 0},
  ...
  {"edit", OPT_EDIT,
   ...
   "inline' adds [error_group] before unknown options. "
   "'last' moves incompatible or mariadbd specific options to the end of "
   "the given file under [error_group], [unsupported_group] or [mariadbd].",
   &opt_edit_mode, &opt_edit_mode, &edit_mode_typelib, GET_ENUM,
   REQUIRED_ARG, EDIT_MODE_NONE, 0, 0, 0, 0, 0},
  {"fix-all", OPT_FIX_ALL,
   "Same as --no-myisam --from=mysql --add-skip-slave-start "
   "--add-mysql-replication-compatibility "
   "--add-mariadb-replication-compatibility --add-client-server-group "
   "--edit=last",
   0, 0, 0, GET_NO_ARG, NO_ARG, FALSE, 0, 0, 0, 0, 0},
  {"from", OPT_CONVERT_FROM,
   "Options are MySQL (default) or MariaDB. Used to specify if your are "
   "upgrading from MySQL or an earlier MariaDB version. "
   ...
   "is the preferred tool! This tool is in this case mainly useful to check "
   "if there is any incompatibilities in the MariaDB configuration files.",
   &opt_convert_from, &opt_convert_from, &from_mode_typelib, GET_ENUM,
   REQUIRED_ARG, FROM_MYSQL, 0, 0, 0, 0, 0},
  {"help", '?', "Display this help message and exit.",
   0, 0, 0, GET_NO_ARG, NO_ARG, 0, 0, 0, 0, 0, 0},
  {"mariadbd", OPT_MARIADBD,
   "Path to the mariadbd server. Used to verify if the config file changes "
   "succeeded.",
   &opt_mariadbd, &opt_mariadbd, 0, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
  {"no-myisam", OPT_NO_MYISAM_OPTIONS,
   "Assume no usage of MyISAM tables.  Makes the MyISAM buffers very small "
   "to save memory. "
   "Useful when moving to MariaDB when there are no user data in MyISAM "
   "tables.  This option only has an effect if the config file contains "
   "settings of the key-buffer-size variable.",
  ...
  {"update-paths", OPT_UPDATE_PATHS,
   "When used with --from=mysql, change file paths having /mysql or /mysqld "
   "ending with '/', '.', space, or new line to /mariadbd. This is intended "
   "to be used when you want to run MySQL and MariaDB on the same system.",
  ...
  {"unsupported-group", OPT_UNSUPPORTED_GROUP,
   ...
   0, GET_STR, REQUIRED_ARG, 0,  0, 0, 0, 0, 0},
  {"verify", OPT_VERIFY,
   "Verify updates by testing the used config files through mariadbd. "
   "Disabled if --print is used. "
   "Note that verify only works if one uses --defaults-file=# or if one does "
   "not specify any configuration files on the command line. "
   "When using verify the mariadbd option has also to be used.",
...
static void print_to_width(const char *line, uint width)
{
  const char *start= line, *space= 0;
  fputs("- ", stdout);
...
static void usage()
{
  ...
  printf("Usage: %s [--print | --update] [OPTIONS] [OPTION_FILES]\n\n",
         my_progname);
  puts("If OPTION_FILES is not provided, all default MariaDB configuration\n"
  ...
  puts("");
  puts("If moving from MYSQL, the recommended usage is to use the following "
       "3 steps:");
  puts("");
  puts("- Check if there is anything that needs changing:");
  puts("mariadb-migrate-config-file --from=mysql");
  puts("");
  puts("- Check how the new configuration files would look like:");
  puts("mariadb-migrate-config-file --from=mysql --edit=last --print");
  puts("");
  puts("- Modify all configuration files keeping backups of the originals:");
  puts("mariadb-migrate-config-file --from=mysql --edit=last --update "
       "--backup --error-group=mysqld-5.7\n");

  puts("Examples:");
  pe("", "Go through all my.cnf and included files and print not supported "
     "options. The groups checked are [server], [mysqld] and [mysqld-###]. "
     "Options valid for MariaDB in [mysqld-###] groups are copied to "
     "the [mariadbd] group");
  pe("--from=mariadb",
     "Go through all my.cnf and include files and print options not supported "
     "by the current MariaDB version. All MariaDB groups read by mariadbd are "
     "checked");
  pe("~/.my.cnf", "Print not supported options in ~/.my.cnf");
  pe("--defaults-file=/etc/my.cnf --verify=1 --mariadbd=#path-to-mariadbd#",
     "Go through /etc/my.cnf and print all not supported options. Check also "
     "the by running it through the MariaDB server");
  pe("--print", "Print the current content of all my.cnf and included files "
     "(no changes)");
  pe("--print --edit=last --update-paths",
     "As print, but change all paths from /mysql to /mariadbd. Useful to "
     "check the effect of --update-paths");
  pe("--print --edit=last",
     "Print modified version of all my.cnf and included files where supported "
     "options from [mysqld-###] groups are copied to a [mariadbd] group at "
     "the end of the file. Not supported options from [mysqld] and [server] "
     "are moved to [mysqld-5.7] (default value of 'error_group')");
  pe("--print --edit=inline --error-group=mysqld-8.0",
     "Print modified version of all my.cnf and included files where supported "
     "options from [mysqld-###] groups are copied inline to [mariadbd] "
     "groups. Not supported options in the [mysqld] and [server] groups are "
     "moved to [mysqld-8.0]");
  pe("--print --edit=remove",
     "Print modified version of all my.cnf and includedfiles where supported "
     "options from [mysqld-###] groups are copied to a [mariadbd] group. "
     "Not supported options from [mysqld] and [server] are removed");
  pe("--print --edit=comment",
     "As '--remove', but not supported options are commented");
  pe("--edit=update --edit=inline --backup",
     "Modify all my.cnf and included files. Supported options from "
     "[mysqld-###] groups are copied to [mariadbd] groups. Not supported "
     "options in the [mysqld] and [server] groups are moved to a "
     "[mysqld-5.7] group. "
               ...
               const char *filename __attribute__((unused)))
{
  ...
  case OPT_FIX_ALL:
    ...
    opt_add_mariadb_replication_compatibility= 1;
    opt_add_mysql_replication_compatibility= 1;
    opt_add_client_server_group= 1;
    break;
  }
  return 0;
}
...
static int get_options(int *argc,char ***argv)
{
  ...
  if (!opt_print && !opt_update && opt_edit_mode != EDIT_MODE_NONE &&
      !opt_silent)
  {
    fputs("Note: edit_mode used without print/update. --print assumed\n",
          stderr);
    opt_print= 1;
  }
  if (opt_print && opt_edit_mode == EDIT_MODE_NONE && opt_update_paths)
  {
    fputs("Note: --print --update-paths without edit mode set will not "
          "show updated paths\n", stderr);
  }
  if (opt_print)
  {
    opt_silent= 1;                /* Don't print notes with --print */
...
int main(int argc, char **argv)
{
  ...
  */
  ...
  if (argc)
  {
    ...
    for (;  argc-- ; org_argv++)
    {
      ...
      if ((error= my_load_defaults(*org_argv, groups_to_use,
                                   &count, &arguments, 0)))
        goto handle_error;
      error= process_defaults(*org_argv, groups_to_use,
                              0,
                              (const char **)(argv + 1),
                              args_used - 1);
  ...
  else
  {
    ...
    */
    error= process_defaults(config_file, groups_to_use,
                            default_directories,
                            (const char **)(argv + 1),
                            args_used - 1);
    free_defaults(arguments);
  ...
  {
    if (global_update_count)
    {
      if (opt_update)
        fprintf(stdout, "%d issue/issues fixed\n", (int) global_update_count);
      else
  ...
  cleanup_and_exit(error != 0 ||
                   (!opt_update && !opt_print && global_update_count != 0) ?
                   2 : 0);
