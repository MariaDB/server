...
static unsigned int 		tablespace_flags= 0;
...
static void init_page_size_from_flags(const unsigned flags)
{
...
{
  ut_ad(cur_space == 0);
  /* dblwr_1, dblwr_2 wasn't initialized yet */
  if (cur_page_num <= 5 && dblwr_1 == 0 && dblwr_2 == 0) {
	return false;
  }

  return ((cur_page_num >= dblwr_1 &&
           cur_page_num < dblwr_1 + extent_size) ||
          (cur_page_num >= dblwr_2 &&
           cur_page_num < dblwr_2 + extent_size));
}
...
static struct my_option innochecksum_options[] = {
  ...
  {"tablespace-flags", 0, "InnoDB tablespace flags",
   &tablespace_flags, &tablespace_flags, 0, GET_ULONG, REQUIRED_ARG,
   0, 0, 0, 0, 0, 0},
...
and page size.
@param[in]      fil_in        File pointer
@param[in,out]  buf           Buffer to read page into
@return true if page 0 was successfully read
and validated, false otherwise */
static bool read_and_validate_page0(FILE *fil_in, byte *buf)
{
  /* Read the minimum page size first */
  ulint initial_page_size= UNIV_ZIP_SIZE_MIN;
  if (tablespace_flags)
  {
    if (!fil_space_t::is_valid_flags(tablespace_flags,
                                     cur_space != 0))
    {
      fprintf(stderr, "Warning: Provided tablespace_flags "
	      "is not valid. So skipping --tablespace_flags option");
      tablespace_flags= 0;
    }
    else
    {
  ...
  ulint bytes= fread(buf, 1, initial_page_size, fil_in);

  if (bytes != initial_page_size)
  {
    fprintf(stderr, "Error: Was not able to read the "
            "minimum page size of " ULINTPF "bytes. Bytes read "
  ...
  else
  {
    if (cur_page_num)
    {
      fprintf(stderr, "Error: First page of the tablespace file "
              "should be 0, but encountered page number %lu.\n"
    ...
    /* Read the rest of the page if it's larger than
    the minimum size */
    if (physical_page_size > UNIV_ZIP_SIZE_MIN)
    {
      ...
      if (bytes != physical_page_size)
      {
        fprintf(stderr, "Error: Was not able to read the "
                "rest of the page ");
        fprintf(stderr, "of " ULINTPF " bytes.  Bytes read was "
                ULINTPF "\n",
