...
# backup (streaming)
# The server runs mariadb-backup-pipe, which writes this stream's
# tar into a fifo we created in the server @@datadir and
# drain to stdout, so nothing lands on local disk.
# The server only accepts a bare command name, and resolves it
# in the PATH of the mariadbd process as mariadb-backup-<name>.
#
...
if [ -n "$STREAM" ]; then
    ...
    # mariadb-backup-pipe names the fifo relative to its working
    # directory, which is the server @@datadir, so we must create
    # it there. This is why --stream only works on the server host,
    # and needs write access to the data directory.
    datadir=$(ask "SELECT @@global.datadir")
    [ -n "$datadir" ] ||
        { cleanup_scratch; die "cannot determine the server datadir"; }
    fifo=${datadir%/}/.mariadb-backup-pipe-1.tar

    # Never reuse or remove a fifo we did not create: a leftover one
    # means another backup is running, or an earlier one was killed.
    [ -e "$fifo" ] &&
        { cleanup_scratch; die "$fifo already exists; remove it and retry"; }
    mkfifo -m 600 "$fifo" ||
        { cleanup_scratch
          die "cannot create $fifo (needs write access to the datadir)"; }

    drain=
    cleanup_stream() {
        # unlinking does not release a reader still blocked on open(2)
        [ -z "$drain" ] || kill "$drain" 2>/dev/null
        rm -f "$fifo" "$scratch/backup-prepare.cnf"
        cleanup_scratch
    }
    ...
    # Client output goes to stderr to keep stdout clean for the tar.
    $MARIADB $MARIADB_OPTS -e "BACKUP SERVER WITH 'pipe'" >&2 \
