...
static bool check_if_fts_table(const char *space_name) {
	...
	if (!starts_with(table_name_start,"FTS_"))
		...
		table_name_start + strlen(table_name_start);
        ...
        /* Skip <table_id>_ */
        table_name_suffix = strchr(table_name_start, '_');
        if (!table_name_suffix ||
        ...
        /* If the remaining character length doesn't exceed the
        maximum common table name length (BEING_DELETED_CACHE) */
        if (table_name_end - table_name_suffix <= 19) {
