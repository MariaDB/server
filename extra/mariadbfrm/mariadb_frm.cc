...
extern void init_frm_mock_hooks();
...
{
  /*
    Do not explicitly delete type_handler_data here.  In SAFEMALLOC debug
    builds (SAFEMALLOC + SAFE_MUTEX), the Dynamic_array buffers inside
    Type_handler_data interact with the safemalloc heap-tracking linked
    list in a way that produces heap corruption ("corrupted size vs.
    prev_size") when free() is called on the st_irem metadata block.
    This is a side-effect of the partial initialisation environment of
    mariadb-frm (mocked hooks, no full server plugin/PSI init).
    Since mariadb-frm is a short-lived CLI tool, we leave the object
    alive and rely on the OS to reclaim memory at process exit.
    sf_leaking_memory was set to 1 in init_early_variables(), so
    safemalloc's atexit handler will not report this as a leak.
  */
...
{
  init_frm_mock_hooks();
  global_frm_parser_mode= true;
#ifdef WITH_WSREP
  Wsrep_server_state::init_once("mariadb-frm", "", "", "", wsrep::gtid(), 0);
#endif
  int exit_code{};
  const char *frm_file;
