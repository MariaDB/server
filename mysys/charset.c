...
get_collation_number_internal(const char *name)
{
  ...
  for (cs= all_charsets;
       ...
       cs++)
  {
    if (cs[0] && cs[0]->col_name.str &&
        !my_strcasecmp(&my_charset_latin1, cs[0]->col_name.str, name))
      return cs[0]->number;
  }  
  return 0;
}
...
static int cs_copy_data(struct charset_info_st *to, CHARSET_INFO *from)
{
  ...
  if (from->col_name.str)
  {
    if (!(to->col_name.str= my_once_memdup(from->col_name.str,
                                           from->col_name.length + 1,
                                           MYF(MY_WME))))
      goto err;
    to->col_name.length= from->col_name.length;
  }
...
{
  return  cs->number && cs->cs_name.str && cs->col_name.str &&
...
static int add_collation(struct charset_info_st *cs)
{
  if (cs->col_name.str &&
      (cs->number ||
       (cs->number=get_collation_number_internal(cs->col_name.str))) &&
      cs->number < array_elements(all_charsets))
  {
	...
	if (!(newcs->comment= my_once_strdup(cs->comment,MYF(MY_WME))))
      ...
      if (cs->col_name.str)
      {
	if (!(newcs->col_name.str= my_once_memdup(cs->col_name.str,
                                                  cs->col_name.length+1,
                                                  MYF(MY_WME))))
	  return MY_XML_ERROR;
        newcs->col_name.length= cs->col_name.length;
      }
    }
    cs->number= 0;
    cs->primary_number= 0;
...
const char *get_charset_name(uint charset_number)
{
  ...
  if (charset_number < array_elements(all_charsets))
  {
    ...
    if (cs && (cs->number == charset_number) && cs->col_name.str)
      return cs->col_name.str;
  }
