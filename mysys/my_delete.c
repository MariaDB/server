...
static int my_win_unlink(const char *name, myf MyFlags)
{
  ...
  for (retries= FILE_SHARING_VIOLATION_RETRIES; ; retries--)
  {
    ...
    /* With MY_NOSYMLINKS, skip this and fall through to the strategies
       below: they open 'name' normally (following the reparse point) and
       win_unlink_check_nosymlinks() then rejects, since the opened
       handle's resolved path won't match 'name'. */
    if ((attributes & FILE_ATTRIBUTE_REPARSE_POINT) &&
        !(MyFlags & MY_NOSYMLINKS))
    {
