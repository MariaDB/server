...
if subprocess.run('lscpu | grep avx512f', shell=True).returncode == 0:
    funcs = ['memcpy_manual_avx512_loop_aligned'] + funcs
...
def replace_macro_and_alignment(func_name):
    ...
    mult = (16 if 'avx512' in func_name else
            4 if align8 else
            8 if 'avx' in func_name else
            1 if align8 else
            2)
...
def start_mariadbd():
    ...
    cmd = (f"{env} taskset -c 0-15 {dbg} ./bld_gemini/sql/mariadbd --no-defaults "
           f"--datadir=/dev/shm/mariadb_data_tmpfs --innodb-buffer-pool-size=4G --innodb-log-file-size=4G "
           f"--innodb-page-size=64k --socket=/tmp/mariadb_bench.sock --skip-grant-tables"
           f" > /dev/shm/mariadb_data_tmpfs/mariadbd.log 2>&1 &")
    subprocess.Popen(cmd, shell=True,
                     stdout=sys.stdout if debug else subprocess.PIPE,
                     stderr=sys.stderr if debug else subprocess.PIPE)
    
    for _ in range(10):
        if run_cmd("./bld_gemini/client/mariadb -u root "
                   "--socket=/tmp/mariadb_bench.sock -e 'SELECT 1'", critical=False) == 0:
            return True
        time.sleep(1)
    return False

def init_datadir():
    run_cmd("rm -rf /dev/shm/mariadb_data_tmpfs/*", critical=False)
    run_cmd("../scripts/mariadb-install-db --no-defaults --builddir=. "
            "--datadir=/dev/shm/mariadb_data_tmpfs --innodb-page-size=64k --force --verbose",
            cwd="bld_gemini")
...
if __name__ == "__main__":
    ...
    for func in funcs:
        ...
        if not start_mariadbd():
            print(f"  Server failed to start for {func}. Skipping.", flush=True)
            sys.exit(1)
        ...
        for engine in engines:
            for size in sizes:
                ...
                run_cmd(f"./bld_gemini/client/mariadb -u root --socket=/tmp/mariadb_bench.sock"
                        f" -e \"DROP DATABASE IF EXISTS bench_db; CREATE DATABASE bench_db;\"")
                run_cmd(f"./bld_gemini/client/mariadb -u root --socket=/tmp/mariadb_bench.sock bench_db"
                        f" -e \"CREATE TABLE t1 (id INT PRIMARY KEY AUTO_INCREMENT, val VARCHAR({size}))"
                        f" ENGINE={engine} /*WITH SYSTEM VERSIONING*/;\"")

                # Insert data
                txt = 'a'*size
                run_cmd(f'./bld_gemini/client/mariadb -u root --socket=/tmp/mariadb_bench.sock bench_db'
                        f' -e "insert t1 select seq, \'{txt}\' from seq_1_to_1000000"')
                ...
                with open("/tmp/bench_script.sql", "w") as f_sql:
                    ...
                    f_sql.write("SET GLOBAL innodb_flush_log_at_trx_commit = 2;\n")
                    f_sql.write("SET GLOBAL innodb_doublewrite = 0;\n")
                    f_sql.write("SET SESSION sql_log_bin = 0;\n")
                    f_sql.write(f"UPDATE t1 set val= '{'0'*size}';\n") # Warmup
                    f_sql.write("SELECT CONCAT(@@memcpy_time, ' ', @@memcpy_calls) AS m1;\n")
                    for loop in range(2 // len(str(size)) + 1):
                        for c in string.ascii_lowercase + string.hexdigits:
                            if size == 1:
                                loop = c
                            f_sql.write(f"UPDATE t1 set val= '{loop}{c*(size-1)}';\n")
                ...
                rc, out = run_cmd("./bld_gemini/client/mariadb -u root --socket=/tmp/mariadb_bench.sock -s -N"
                                  " < /tmp/bench_script.sql",
                ...
                if success and end_calls > start_calls:
                    ...
                    print(f"Bench failed for {engine}/{size}."
                          f" (success={success} end_calls={end_calls} start_calls={start_calls})", flush=True)
