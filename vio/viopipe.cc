...
size_t Named_pipe_vio::read(uchar *buf, size_t count)
{
  DWORD transferred;
  size_t ret= (size_t) -1;
  DBUG_ENTER("Named_pipe_vio::read");
  if (m_shutdown_flag)
  {
    vio_set_was_timeout(FALSE);
    return ret;
...
size_t Named_pipe_vio::write(const uchar *buf, size_t count)
{
  DWORD transferred;
  size_t ret= (size_t) -1;
  DBUG_ENTER("Named_pipe_vio::write");
...
{
  DBUG_ENTER("Named_pipe_vio::close");

  if (!CloseHandle(m_overlapped.hEvent))
  {
    DBUG_PRINT("error", ("CloseHandle failed with error %lu", GetLastError()));
    DBUG_ASSERT(!"CloseHandle failed");
  }
