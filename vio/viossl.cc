                  ...
                  ssl_handshake_func_t func, unsigned long *errptr)
{
  ...
  /* Handshake succeeded: hand the caller the SSL-wrapping VIO. */
  *vio_ptr= ssl_vio;
...
my_bool Ssl_vio::is_connected()
{
  /* Decrypted data may be buffered at the SSL layer even when the
     underlying transport has none left to report. */
  return m_underlying->is_connected() ||
         SSL_pending(static_cast<SSL *>(m_ssl)) > 0;
}
