...
static int ssl_handshake_loop(Vio *vio, SSL *ssl, int (*func)(SSL*));
...
size_t vio_ssl_write(Vio *vio, const uchar *buf, size_t size)
{
  ...
  if (!SSL_is_init_finished(ssl))
  {
    ...
    if (!SSL_is_init_finished(ssl) &&
        ssl_handshake_loop(vio, ssl, SSL_accept) < 1)
      DBUG_RETURN(-1);
...
static int ssl_handshake_loop(Vio *vio, SSL *ssl, ssl_handshake_func_t func)
{
  ...
  /* vio->ssl_arg stays, it is the caller's to keep or to clear */
  return ret;
}
