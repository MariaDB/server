...
{
  if ((st->flags & MY_XML_FLAG_ASSERT_WELL_FORMED) &&
      st->attr.end == st->attr.start)
  {
    /*
      Values are not allowed on the root level of XML.
      Though spaces are ok.
    */
    while (len > 0)
    {
      if (!my_xml_is_space(str[--len]))
        return MY_XML_ERROR;
    }
    return MY_XML_OK;
  }
...
static int my_xml_enter(MY_XML_PARSER *st, const char *str, size_t len)
{
  ...
  else if ((st->flags & MY_XML_FLAG_ASSERT_WELL_FORMED) &&
           st->current_node_type == MY_XML_NODE_TAG)
  {
    /* level 0 nesting of the XML document .*/
    if (st->root_node_counter++)
      return MY_XML_ERROR; /* no multiple root nodes allowed. */
    st->item_counter++;
  }
...
int my_xml_parse(MY_XML_PARSER *p,const char *str, size_t len)
{
  ...
  while ( p->cur < p->end )
  {
    ...
    if (p->cur[0] == '<')
    {
      ...
      {
        size_t tag_len= (size_t) (a.end - a.beg);
        if ((p->flags & MY_XML_FLAG_ASSERT_WELL_FORMED) &&
            tag_len == 3 && memcmp(a.beg, "xml", 3) == 0)
        {
          /*
            The <?xml ..?> processing instruction has special meaning.
            If appears it must be first line of the XML document.
          */
          if (p->item_counter)
            return MY_XML_ERROR;
        }
...
gt:
  ...
  if (p->attr.start[0] ||
      ((p->flags & MY_XML_FLAG_ASSERT_WELL_FORMED) &&
       p->root_node_counter == 0))
  {
