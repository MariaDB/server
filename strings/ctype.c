                      ...
                      uint *errors)
{
  ...
  while (1)
  {
    ...
    {
      /* Handle malformed byte sequences by forcing advancement to prevent infinite loops when 0 bytes are consumed. */
      error_count++;
      from++;
      wc= '?';
    }
...
# endif
 nonascii:
  ...
  for (; ; *to++= *from++, length--)
  {
    if (!length)
    {
      /* Catch silent truncation if the destination buffer is too small. */
