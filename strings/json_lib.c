...
static int mark_object(json_engine_t *j)
{
  ...
  if (j->stack_p < j->stack.max_element)
    *((int*)(j->stack.buffer)+j->stack_p)= val;
  else
    mem_root_dynamic_array_resize_and_set_val(&(j->stack), &val, j->stack_p);
                    ...
                    CHARSET_INFO *i_cs, const uchar *str, const uchar *end)
{
  ...
  do
  {
    ...
    case PS_KEY:
      ...
      if (p->last_step_idx < p->steps.max_element)
      {
                        ...
                        MEM_ROOT_DYNAMIC_ARRAY *array_counters)
{
      ...
      je->value_type & JSON_VALUE_OBJECT)
  {
    do
    {
      ...
      int idx= next_step-initial_step;

      if (idx < array_counters->max_element)
        *((int*)(array_counters->buffer) + idx)= val;
      else
        mem_root_dynamic_array_resize_and_set_val(array_counters, &val, (next_step - initial_step));
  ...
  if (((next_step-initial_step)) < array_counters->max_element)
    *((int*)(array_counters->buffer) + (next_step-initial_step))= value;
  else
    mem_root_dynamic_array_resize_and_set_val(array_counters, &value, (next_step - initial_step));
  ...
  if (next_step->type & JSON_PATH_ARRAY)
  {
    ...
    if (((next_step-initial_step)) < array_counters->max_element)
    *((int*)(array_counters->buffer) + (next_step-initial_step))= array_size;
  else
    mem_root_dynamic_array_resize_and_set_val(array_counters, &array_size, (next_step - initial_step));
