...
/*
  ...
  - index: Which child we are on. Example for object,
  {"a":1, "b":2 } then a is child 0 and b is child 1. For arrays [1, 2, 3]
  1 is child 0, 2 is child 1, 3 is child 2.
  - visited: Which phase of processing we are on. Mainly used for printing.
     0: First time we saw this node. If it is an object or array,
         set to 1 and push the frame in the stack.
     1: You are seeing the node after processing the children
     2: print object key for index
     3: print array comma for index
